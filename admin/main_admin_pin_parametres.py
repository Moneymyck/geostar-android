...
def _make_ssl_context():
    """Crée un contexte SSL qui fonctionne sur Android (sans certifi)."""
    import ssl
    try:
        import certifi
...
class SettingsScreen(Screen):
    ...
    def _build(self):
        ...
        b_secours = make_btn("MODIFIER CODE PIN", bg=(0.3, 0.3, 0.55, 1),
...
# ============================================================
# PATCH ADMIN — PIN 4 CHIFFRES + NOTES SUPABASE
# ============================================================

def _geostar_admin_pin_supabase_patch():
    ...
    def load_pin():
        ...
        if not pin_hash:
            return hash_password("1253")
        return pin_hash

    def save_pin(pin):
        if not pin.isdigit() or len(pin) != 4:
            return False
        return save_json(PIN_FILE, {"pin_hash": hash_password(pin), "updated_at": datetime.now().strftime("%Y-%m-%d %H:%M")})

    def verify_pin(pin):
        return hash_password(pin) == load_pin()
    ...
    def supabase_request(method, path, body=None):
        ...
        req = urllib.request.Request(
            ...
            headers={
                ...
                "User-Agent": "GEOSTAR-Admin-Supabase/1.0"
            }
        )
        ctx = _ssl_ctx()
        if ctx:
    ...
    def fetch_notes_supabase(statut="attente"):
        q = "/rest/v1/notes_communaute?select=*&order=date_soumission.desc"
        if statut:
            q += "&statut=eq." + statut
        return supabase_request("GET", q, None) or []
    ...
    def update_note_status(note_id, statut):
        ...
        return supabase_request("PATCH", "/rest/v1/notes_communaute?id=eq." + str(note_id), body)

    # ---------- LOGIN : ajout PIN + changer PIN ----------
    old_login_build = LoginScreen._build

    def login_build_with_pin(self):
        old_login_build(self)

        try:
            root = self.children[0]

            self.inp_pin = TextInput(
                hint_text="Code PIN admin 4 chiffres",
                password=True,
                multiline=False,
                size_hint_y=None,
                height=dp(52),
            ...
            b_pin = make_btn("SE CONNECTER AVEC PIN", bg=C_BLUE, height=dp(52), bold=True, font_size=dp(15))
            b_change = make_btn("MODIFIER PIN", bg=(0.3, 0.25, 0.45, 1), height=dp(44), font_size=dp(13))

            # Mettre les boutons juste après connexion normale si possible
            root.add_widget(self.inp_pin)
            root.add_widget(b_pin)
            root.add_widget(b_change)

            def do_pin(_):
                pin = self.inp_pin.text.strip()
                if verify_pin(pin):
                    self.lbl_err.text = ""
                    App.get_running_app().current_admin = "PIN_ADMIN"
                    try:
                        log_action("PIN_ADMIN", "Connexion PIN", "")
                    except Exception:
                        pass
                    self.manager.transition = SlideTransition(direction="left")
                    self.manager.current = "main"
                else:
                    self.lbl_err.text = "PIN incorrect"

            def do_change(_):
                box = BoxLayout(orientation="vertical", padding=dp(14), spacing=dp(8))
                oldp = TextInput(hint_text="Ancien PIN", password=True, multiline=False, input_filter="int", size_hint_y=None, height=dp(48))
                newp = TextInput(hint_text="Nouveau PIN 4 chiffres", password=True, multiline=False, input_filter="int", size_hint_y=None, height=dp(48))
                newp2 = TextInput(hint_text="Confirmer nouveau PIN", password=True, multiline=False, input_filter="int", size_hint_y=None, height=dp(48))
                msg = make_label("", color=C_RED, height=dp(28), halign="center")
                bok = make_btn("ENREGISTRER PIN", bg=C_GREEN, height=dp(48), bold=True)
                for w in [oldp, newp, newp2, msg, bok]:
                    box.add_widget(w)
                pop = Popup(title="Modifier PIN admin", content=box, size_hint=(0.9, None), height=dp(330))

                def save_new_pin(__):
                    if not verify_pin(oldp.text.strip()):
                        msg.text = "Ancien PIN incorrect"
                        return
                    if newp.text.strip() != newp2.text.strip():
                        msg.text = "Les PIN ne correspondent pas"
                        return
                    if not newp.text.strip().isdigit() or len(newp.text.strip()) != 4:
                        msg.text = "Le PIN doit avoir 4 chiffres"
                        return
                    save_pin(newp.text.strip())
                    pop.dismiss()
                    show_toast("PIN modifié")

                bok.bind(on_release=save_new_pin)
                pop.open()

            b_pin.bind(on_release=do_pin)
            b_change.bind(on_release=do_change)

        except Exception as e:
            print("PATCH PIN ERREUR", e)

    LoginScreen._build = login_build_with_pin

    # ---------- NOTES ADMIN : lecture Supabase ----------
    def show_notes_supabase(self):
        box = BoxLayout(orientation="vertical", padding=dp(10), spacing=dp(8))

        header = make_label("Chargement des notes Supabase...", color=C_GOLD, height=dp(36), halign="center")
        box.add_widget(header)

        filtre_row = BoxLayout(size_hint_y=None, height=dp(40), spacing=dp(6))
        grid = GridLayout(cols=1, spacing=dp(6), size_hint_y=None, padding=dp(4))
        grid.bind(minimum_height=grid.setter("height"))
        sv = ScrollView()
        sv.add_widget(grid)

        current_filter = {"statut": "attente"}

        def refresh(statut="attente"):
            current_filter["statut"] = statut
            grid.clear_widgets()
            header.text = "Chargement " + statut + "..."
            try:
                notes = fetch_notes_supabase(statut)
            except Exception as e:
                header.text = "Erreur Supabase"
                grid.add_widget(make_label(str(e)[:400], color=C_RED, height=dp(120), halign="center"))
                return

            header.text = "Notes " + statut + " : " + str(len(notes))

            if not notes:
                grid.add_widget(make_label("Aucune note " + statut, color=C_SUBTEXT, height=dp(60), halign="center"))
                return

            for note in notes:
                card = BoxLayout(orientation="vertical", size_hint_y=None, height=dp(150), padding=dp(8), spacing=dp(4))
                theme = note.get("theme_key", "?")
                auteur = note.get("auteur", "Anonyme")
                date = note.get("date_soumission", "")
                texte = note.get("texte", "") or ""
                nid = note.get("id", "")

                card.add_widget(make_label("[b]" + str(auteur) + "[/b] | " + str(theme), color=C_GOLD, height=dp(24), font_size=dp(12)))
                card.add_widget(make_label(str(date)[:19], color=C_SUBTEXT, height=dp(20), font_size=dp(10)))
                card.add_widget(make_label(texte[:180] + ("..." if len(texte) > 180 else ""), color=C_TEXT, height=dp(54), font_size=dp(11)))

                row = BoxLayout(size_hint_y=None, height=dp(38), spacing=dp(6))
                if statut == "attente":
                    b_ok = make_btn("VALIDER", bg=C_GREEN, height=dp(36), font_size=dp(11))
                    b_no = make_btn("REJETER", bg=C_RED, height=dp(36), font_size=dp(11))

                    def do_valider(_, note_id=nid):
                        try:
                            update_note_status(note_id, "valide")
                            show_toast("Note validée")
                            refresh("attente")
                        except Exception as e:
                            show_toast("Erreur validation : " + str(e)[:80])

                    def do_rejeter(_, note_id=nid):
                        try:
                            update_note_status(note_id, "rejete")
                            show_toast("Note rejetée")
                            refresh("attente")
                        except Exception as e:
                            show_toast("Erreur rejet : " + str(e)[:80])
                    ...
                    b_back = make_btn("REMETTRE EN ATTENTE", bg=C_ORANGE, height=dp(36), font_size=dp(10))
                    def do_attente(_, note_id=nid):
                        try:
                            update_note_status(note_id, "attente")
                            show_toast("Note remise en attente")
                            refresh(statut)
                        except Exception as e:
                            show_toast("Erreur : " + str(e)[:80])
                    b_back.bind(on_release=do_attente)
                    row.add_widget(b_back)

                card.add_widget(row)
                grid.add_widget(card)

        for label, key, clr in [("Attente", "attente", C_ORANGE), ("Validées", "valide", C_GREEN), ("Rejetées", "rejete", C_RED)]:
            b = make_btn(label, bg=clr, height=dp(36), font_size=dp(11))
            b.bind(on_release=lambda _, k=key: refresh(k))
            filtre_row.add_widget(b)

        box.add_widget(filtre_row)
        box.add_widget(sv)

        close = make_btn("FERMER", bg=C_GREY, height=dp(44))
        box.add_widget(close)

        pop = Popup(title="Notes communauté Supabase", content=box, size_hint=(0.97, 0.93))
        close.bind(on_release=lambda x: pop.dismiss())
        pop.open()

        Clock.schedule_once(lambda dt: refresh("attente"), 0.2)

    MainScreen._show_notes_attente = show_notes_supabase


_geostar_admin_pin_supabase_patch()


# ============================================================
# PATCH ADMIN — REMPLACE SECOURS EMAIL PAR MODIFIER PIN
# ============================================================

def _geostar_admin_settings_pin_patch():
    from kivy.uix.popup import Popup
    from kivy.uix.boxlayout import BoxLayout
    from kivy.uix.textinput import TextInput
    from kivy.metrics import dp
    ...
    def verify_pin_local(pin):
        return hash_password(pin) == load_pin_hash()

    def save_pin_local(pin):
        if not pin.isdigit() or len(pin) != 4:
            return False
        return save_json(PIN_FILE, {"pin_hash": hash_password(pin)})

    def show_change_pin_popup():
        box = BoxLayout(orientation="vertical", padding=dp(14), spacing=dp(8))

        old_pin = TextInput(hint_text="Ancien PIN", password=True, multiline=False, input_filter="int", size_hint_y=None, height=dp(48))
        new_pin = TextInput(hint_text="Nouveau PIN 4 chiffres", password=True, multiline=False, input_filter="int", size_hint_y=None, height=dp(48))
        confirm_pin = TextInput(hint_text="Confirmer nouveau PIN", password=True, multiline=False, input_filter="int", size_hint_y=None, height=dp(48))

        msg = make_label("", color=C_RED, height=dp(30), halign="center")
        btn_save = make_btn("ENREGISTRER LE PIN", bg=C_GREEN, height=dp(48), bold=True)
        btn_reset = make_btn("REMETTRE PIN PAR DÉFAUT 1253", bg=C_ORANGE, height=dp(44))
        btn_close = make_btn("FERMER", bg=C_GREY, height=dp(44))

        for w in [old_pin, new_pin, confirm_pin, msg, btn_save, btn_reset, btn_close]:
            box.add_widget(w)

        pop = Popup(title="Modifier code PIN admin", content=box, size_hint=(0.9, None), height=dp(430))

        def save_new_pin(_):
            ancien = old_pin.text.strip()
            nouveau = new_pin.text.strip()
            confirm = confirm_pin.text.strip()

            if not verify_pin_local(ancien):
                msg.text = "Ancien PIN incorrect"
                return
            if not nouveau.isdigit() or len(nouveau) != 4:
                msg.text = "Le nouveau PIN doit avoir 4 chiffres"
                return
            if nouveau != confirm:
                msg.text = "Les deux PIN ne correspondent pas"
                return

            save_pin_local(nouveau)
            pop.dismiss()
            show_toast("PIN modifié avec succès")

        def reset_pin(_):
            save_pin_local("1253")
            pop.dismiss()
            show_toast("PIN remis à 1253")

        btn_save.bind(on_release=save_new_pin)
        btn_reset.bind(on_release=reset_pin)
        btn_close.bind(on_release=lambda x: pop.dismiss())
        pop.open()

    try:
        SettingsScreen._show_email_recovery = lambda self, *args: show_change_pin_popup()
    except Exception:
        pass

    try:
        old_settings_build = SettingsScreen._build

        def settings_build_with_pin(self):
            old_settings_build(self)
            try:
                def scan(widget):
                    if hasattr(widget, "children"):
                        for child in widget.children:
                            if getattr(child, "text", "") in ["MODIFIER CODE PIN", "MODIFIER CODE PIN"]:
                                child.text = "MODIFIER CODE PIN"
                                child.bind(on_release=lambda x: show_change_pin_popup())
                            scan(child)
                scan(self)
            except Exception:
                pass

        SettingsScreen._build = settings_build_with_pin
    except Exception:
        pass

_geostar_admin_settings_pin_patch()
