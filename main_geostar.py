...
class MoneyRoot(FloatLayout):
    ...
    def popup_search_by_figure(self):
        ...
        def on_calculer(_):
            ...
            if fig_code is None:
                title = "Toutes figures M" + "+M".join(str(p) for p in positions) + mode_txt
            else:
                ...
                for idx, sol in enumerate(solutions, 1):
                    ...
                    def on_sol_click(btn, s=sol, sl=solutions):
                        ...
                        if not stricte:
                            ...
                            SF = _get_saved_file()
                            sv = read_json(SF, [])
                            fig_bits2 = s.get("figure")
                            fig_code2 = BIN_TO_CODE.get(fig_bits2, "")
                            fig_nom2 = FIGURES.get(fig_code2, {}).get("africain", data_fig(fig_bits2).get("africain", "Figure"))
...
def _money_v103_patch():
    ...
    def open_solution_window_v103(self, title, positions, rare):
        ...
        close.bind(on_release=lambda x: pop.dismiss())
        def save_search_to_combo(_):
            item = {
                "kind":"advanced",
                "name": title,
                "fig_code": fig_code,
                "positions": list(positions or []),
                "stricte": bool(stricte),
                "search_type": search_type or "auto",
                "rare": bool(rare)
            }
            _add_combo(item)
            self.message("COMBINAISONS", "Recherche ajoutée dans l'onglet COMBINAISONS.")
        combo_search_btn.bind(on_release=save_search_to_combo)
        pop.open()
...
# ============================================================
# PATCH GEOSTAR STABLE — NOTE PAR THÈME SUPABASE + VOCAL + SANS FAVORIS
# ============================================================

def _geostar_stable_note_patch():
    import json, ssl, urllib.request, urllib.parse, threading
    from datetime import datetime
    from kivy.uix.popup import Popup
    from kivy.uix.boxlayout import BoxLayout
    from kivy.uix.gridlayout import GridLayout
    ...
    def http(method, url, payload=None, prefer="return=representation"):
        data = json.dumps(payload).encode("utf-8") if payload is not None else None
        req = urllib.request.Request(url, data=data, method=method, headers=headers(prefer))
    ...
    def theme_key(self):
        try:
            return "-".join(str(x) for x in self.mother_codes)
        except Exception:
            try:
    ...
    def fetch_notes(theme):
        url = SUPABASE_URL + "/rest/v1/notes_communaute?select=*&statut=eq.valide&theme_key=eq." + urllib.parse.quote(str(theme)) + "&order=date_validation.desc"
        data = http("GET", url)
        return data if isinstance(data, list) else []

    def send_note(theme, auteur, texte, audio):
        payload = {"theme_key": str(theme), "auteur": auteur or "Anonyme", "code_licence": "", "langue": "fr", "texte": texte or "", "audio_url": audio or "", "statut": "attente", "date_soumission": datetime.now().isoformat()}
        return http("POST", SUPABASE_URL + "/rest/v1/notes_communaute", payload, prefer="return=minimal")

    def request_delete(note_id):
        payload = {"statut": "demande_suppression", "date_validation": datetime.now().isoformat()}
        url = SUPABASE_URL + "/rest/v1/notes_communaute?id=eq." + urllib.parse.quote(str(note_id))
        return http("PATCH", url, payload, prefer="return=minimal")

    def msg(self, title, body):
        try:
            self.message(title, body)
        except Exception:
            Popup(title=title, content=Label(text=str(body)), size_hint=(0.85,0.45)).open()

    def popup_notes(self):
        theme = theme_key(self)
        main = BoxLayout(orientation="vertical", padding=dp(8), spacing=dp(7))
        main.add_widget(Label(text="[b]NOTES PUBLIQUES DU THÈME[/b]\n" + str(theme), markup=True, color=(1,1,1,1), size_hint=(1,None), height=dp(58)))

        top = BoxLayout(size_hint=(1,None), height=dp(46), spacing=dp(6))
        b_refresh = Button(text="ACTUALISER", background_color=(0.05,0.25,0.50,1))
        b_add = Button(text="AJOUTER NOTE / VOCAL", background_color=(0,0.42,0.10,1))
        top.add_widget(b_refresh); top.add_widget(b_add); main.add_widget(top)

        sv = ScrollView()
        grid = GridLayout(cols=1, spacing=dp(7), padding=dp(4), size_hint_y=None)
        grid.bind(minimum_height=grid.setter("height"))
        sv.add_widget(grid); main.add_widget(sv)

        close = Button(text="FERMER", size_hint=(1,None), height=dp(46), background_color=(0.25,0.25,0.25,1))
        main.add_widget(close)
        pop = Popup(title="NOTE du thème", content=main, size_hint=(0.96,0.94))

        def refresh(*_):
            grid.clear_widgets()
            grid.add_widget(Label(text="Chargement Supabase...", color=(1,1,1,1), size_hint_y=None, height=dp(70)))
            def worker():
                try:
                    notes = fetch_notes(theme); err = ""
                except Exception as e:
                    notes = []; err = str(e)[:350]
                def done(dt):
                    grid.clear_widgets()
                    if err:
                        grid.add_widget(Label(text="Erreur Supabase :\n" + err, color=(1,0.3,0.3,1), size_hint_y=None, height=dp(120))); return
                    if not notes:
                        grid.add_widget(Label(text="Aucune note validée pour ce thème.", color=(.82,.82,.82,1), size_hint_y=None, height=dp(80))); return
                    for n in notes:
                        note_id = n.get("id")
                        auteur = str(n.get("auteur","Anonyme"))
                        texte = str(n.get("texte",""))
                        audio = str(n.get("audio_url","") or "")
                        date = str(n.get("date_validation","") or n.get("date_soumission",""))[:19]
                        card = BoxLayout(orientation="vertical", padding=dp(7), spacing=dp(3), size_hint_y=None, height=dp(180))
                        card.add_widget(Label(text="[b]" + auteur + "[/b]  " + date, markup=True, color=(1,.82,.1,1), size_hint=(1,None), height=dp(26)))
                        card.add_widget(Label(text=texte[:300], color=(1,1,1,1), size_hint=(1,None), height=dp(64)))
                        if audio:
                            card.add_widget(Label(text="VOCAL / LIEN : " + audio[:100], color=(.75,.85,1,1), size_hint=(1,None), height=dp(28)))
                        bd = Button(text="DEMANDER SUPPRESSION", size_hint=(1,None), height=dp(38), background_color=(.45,.1,.1,1))
                        bd.bind(on_release=lambda x, nid=note_id: ask_del(nid))
                        card.add_widget(bd); grid.add_widget(card)
        ...
        def ask_del(nid):
            try:
                request_delete(nid)
                msg(self, "SUPPRESSION", "Demande envoyée.\nUn admin doit accepter ou refuser.")
                refresh()
            except Exception as e:
                msg(self, "ERREUR", str(e)[:300])

        def add_note(*_):
            box = BoxLayout(orientation="vertical", padding=dp(10), spacing=dp(8))
            box.add_widget(Label(text="Ajouter une note au thème\n" + str(theme), color=(1,1,1,1), size_hint=(1,None), height=dp(52)))
            auteur = TextInput(hint_text="Nom ou pseudo", multiline=False, size_hint=(1,None), height=dp(44))
            texte = TextInput(hint_text="Écris ta note ici", multiline=True, size_hint=(1,1))
            audio = TextInput(hint_text="Lien vocal ou mémo vocal optionnel", multiline=False, size_hint=(1,None), height=dp(44))
            send = Button(text="ENVOYER POUR VALIDATION", size_hint=(1,None), height=dp(46), background_color=(0,0.42,0.10,1))
            cancel = Button(text="FERMER", size_hint=(1,None), height=dp(42), background_color=(.25,.25,.25,1))
            for w in [auteur, texte, audio, send, cancel]: box.add_widget(w)
            p2 = Popup(title="Nouvelle note", content=box, size_hint=(.92,.78))
            def do_send(*__):
                if not texte.text.strip() and not audio.text.strip():
                    msg(self, "NOTE", "Écris une note ou ajoute un vocal avant d’envoyer."); return
                send.disabled = True; send.text = "ENVOI..."
                def worker():
                    try:
                        send_note(theme, auteur.text.strip() or "Anonyme", texte.text.strip(), audio.text.strip()); err = ""
                    except Exception as e:
                        err = str(e)[:350]
                    def done(dt):
                        send.disabled = False; send.text = "ENVOYER POUR VALIDATION"
                        if err: msg(self, "ERREUR", err)
                        else:
                            p2.dismiss(); msg(self, "NOTE", "Note envoyée.\nElle apparaîtra après validation admin.")
                    Clock.schedule_once(done, 0)
                threading.Thread(target=worker, daemon=True).start()
            send.bind(on_release=do_send); cancel.bind(on_release=lambda x: p2.dismiss())
            p2.open()

        b_refresh.bind(on_release=refresh); b_add.bind(on_release=add_note); close.bind(on_release=lambda x: pop.dismiss())
        pop.open(); refresh()

    MoneyRoot.popup_notes_current = popup_notes
    MoneyRoot.ouvrir_communaute = lambda self, *args: self.popup_notes_current()

    try:
        old_build_ui = MoneyRoot.build_ui
        def build_ui_clean(self):
            old_build_ui(self)
            try:
                for child in list(self.children):
                    if isinstance(child, BoxLayout):
                        for w in list(child.children):
                            if getattr(w, "text", "") in ["COMM.", "FAVORIS"]:
                                child.remove_widget(w)
            except Exception:
                pass
        MoneyRoot.build_ui = build_ui_clean
    except Exception:
        pass

_geostar_stable_note_patch()


# ============================================================
# PATCH GEOSTAR — RETIRER BOUTONS INUTILES
# ============================================================

def _geostar_remove_unwanted_buttons_patch():
    from kivy.uix.boxlayout import BoxLayout
    from kivy.clock import Clock

    def _remove_buttons(root):
        try:
            ...
            for child in list(root.children):
                ...
                if txt in [
                    "ALÉATOIRE SIMPLE\n(ancienne méthode)",
                    "ALÉATOIRE SIMPLE",
                    "+ AJOUTER RECHERCHE PERSONNALISÉE",
                                    ]:
                    try:
                ...
                # retire aussi les boutons contenant ces textes
                if isinstance(txt, str):
                    if "ALÉATOIRE SIMPLE" in txt or "AJOUTER RECHERCHE PERSONNALISÉE" in txt:
                        try:
                            root.remove_widget(child)
                            continue
                        except Exception:
    ...
    try:
        ...
        def build_ui_clean_buttons(self):
            old_build_ui(self)
            Clock.schedule_once(lambda dt: _remove_buttons(self), 0.1)

        MoneyRoot.build_ui = build_ui_clean_buttons
    except Exception:
        pass

    # Nettoyer aussi à l’ouverture des popups, pour les boutons créés après.
    try:
        from kivy.uix.popup import Popup
        old_open = Popup.open

        def open_clean(self, *args, **kwargs):
            result = old_open(self, *args, **kwargs)
            Clock.schedule_once(lambda dt: _remove_buttons(self), 0.1)
            Clock.schedule_once(lambda dt: _remove_buttons(self), 0.5)
            return result

        Popup.open = open_clean
    except Exception:
        pass

_geostar_remove_unwanted_buttons_patch()
...
# ============================================================
# PATCH GEOSTAR — MICRO DIRECT SUPABASE + FAVORIS OFF
# ============================================================

def _geostar_micro_direct_supabase_patch():
    import os, json, ssl, urllib.request, urllib.parse, threading, time
    from datetime import datetime
    from kivy.app import App
    from kivy.clock import Clock
    from kivy.uix.popup import Popup
    ...
    def headers(prefer="return=representation"):
        return {
            ...
            "Content-Type": "application/json",
            "Prefer": prefer,
            "User-Agent": "GEOSTAR-MICRO-DIRECT"
        }

    def http_json(method, url, payload=None, prefer="return=representation"):
        data = json.dumps(payload).encode("utf-8") if payload is not None else None
        req = urllib.request.Request(url, data=data, method=method, headers=headers(prefer))
    ...
    def msg(self, title, body):
        try:
            ...
            Popup(title=title, content=Label(text=str(body)), size_hint=(0.86,0.46)).open()

    def fetch_valid_notes(theme):
        url = SUPABASE_URL + "/rest/v1/notes_communaute?select=*&statut=eq.valide&theme_key=eq." + urllib.parse.quote(str(theme)) + "&order=date_validation.desc"
        data = http_json("GET", url)
        return data if isinstance(data, list) else []

    def send_note(theme, auteur, texte, audio_url):
        payload = {
            "theme_key": str(theme),
            "auteur": auteur or "Anonyme",
            "code_licence": getattr(self, "activation_code", "") if "self" in locals() else "",
            "langue": "fr",
            "texte": texte or "",
        ...
        return http_json("POST", SUPABASE_URL + "/rest/v1/notes_communaute", payload, prefer="return=minimal")

    def ask_delete(note_id):
        payload = {"statut": "demande_suppression", "date_validation": datetime.now().isoformat()}
        url = SUPABASE_URL + "/rest/v1/notes_communaute?id=eq." + urllib.parse.quote(str(note_id))
        return http_json("PATCH", url, payload, prefer="return=minimal")

    def upload_audio(path, theme):
        if not path or not os.path.exists(path):
            return ""
        object_path = str(theme).replace("/", "_") + "/" + os.path.basename(path)
        url = SUPABASE_URL + "/storage/v1/object/" + AUDIO_BUCKET + "/" + urllib.parse.quote(object_path)
        ...
        req = urllib.request.Request(
            ...
            headers={
                ...
                "x-upsert": "true",
                "User-Agent": "GEOSTAR-AUDIO-UPLOAD"
            }
        )
        with urllib.request.urlopen(req, timeout=60, context=ctx()) as r:
            r.read()
        return SUPABASE_URL + "/storage/v1/object/public/" + AUDIO_BUCKET + "/" + urllib.parse.quote(object_path)
    ...
    class MicroRecorder:
        ...
        def start(self):
            ...
            try:
                if platform != "android":
                    return False, "Micro direct disponible seulement dans l’APK Android."
                from jnius import autoclass
                MediaRecorder = autoclass("android.media.MediaRecorder")
                AudioSource = autoclass("android.media.MediaRecorder$AudioSource")
                OutputFormat = autoclass("android.media.MediaRecorder$OutputFormat")
                ...
                if not os.path.exists(folder):
                    os.makedirs(folder)
                self.path = os.path.join(folder, "geostar_vocal_" + str(int(time.time())) + ".m4a")
                ...
                self.rec = None
                self.recording = False
                return False, "Erreur micro : " + str(e)[:240]
        ...
        def stop(self):
            try:
                ...
                self.rec = None
                self.recording = False
                return False, "Erreur stop : " + str(e)[:240]
    ...
    def remove_favoris_widgets(root):
        try:
            for child in list(getattr(root, "children", [])):
                txt = getattr(child, "text", "")
                if isinstance(txt, str) and txt.strip().upper() == "FAVORIS":
                    try:
                        root.remove_widget(child)
                        continue
                    except Exception:
                        try:
                            ...
                            pass
                remove_favoris_widgets(child)
        except Exception:
            pass

    def popup_notes(self):
        theme = theme_key(self)
        recorder = MicroRecorder()
        state = {"audio_path": ""}

        main = BoxLayout(orientation="vertical", padding=dp(10), spacing=dp(8))
        main.add_widget(Label(text="[b]NOTES DU THÈME[/b]\n" + str(theme), markup=True, color=(1,1,1,1), size_hint=(1,None), height=dp(58)))
        ...
        sv = ScrollView()
        grid = GridLayout(cols=1, spacing=dp(10), padding=dp(6), size_hint_y=None)
        grid.bind(minimum_height=grid.setter("height"))
        sv.add_widget(grid); main.add_widget(sv)
        ...
        def refresh(*_):
            grid.clear_widgets()
            grid.add_widget(Label(text="Chargement...", color=(1,1,1,1), size_hint_y=None, height=dp(70)))
            def worker():
                try:
                    notes = fetch_valid_notes(theme); err = ""
                except Exception as e:
                    notes = []; err = str(e)[:350]
                def done(dt):
                    grid.clear_widgets()
                    if err:
                        grid.add_widget(Label(text="Erreur Supabase :\n" + err, color=(1,0.3,0.3,1), size_hint_y=None, height=dp(120)))
                        return
                    if not notes:
                        grid.add_widget(Label(text="Aucune note validée pour ce thème.", color=(0.82,0.82,0.82,1), size_hint_y=None, height=dp(80)))
                        return
                    for n in notes:
                        note_id = n.get("id")
                        auteur = str(n.get("auteur","Anonyme"))
                        ...
                        audio = str(n.get("audio_url","") or "")
                        date = str(n.get("date_validation","") or n.get("date_soumission",""))[:19]
                        card = BoxLayout(orientation="vertical", padding=dp(10), spacing=dp(5), size_hint_y=None, height=dp(200))
                        card.add_widget(Label(text="[b]" + auteur + "[/b]\n" + date, markup=True, color=(1,0.82,0.1,1), size_hint=(1,None), height=dp(42)))
                        card.add_widget(Label(text=texte[:330] if texte else "(Vocal uniquement)", color=(1,1,1,1), size_hint=(1,None), height=dp(58)))
                        if audio:
                            card.add_widget(Label(text="🎙 Vocal stocké Supabase", color=(0.75,0.85,1,1), size_hint=(1,None), height=dp(25)))
                        bd = Button(text="DEMANDER SUPPRESSION", size_hint=(1,None), height=dp(38), background_color=(0.45,0.10,0.10,1))
                        bd.bind(on_release=lambda x, nid=note_id: ask_del(nid))
                        card.add_widget(bd)
        ...
        def add_note(*_):
            ...
            auteur = TextInput(hint_text="Nom ou pseudo", multiline=False, size_hint=(1,None), height=dp(44))
            texte = TextInput(hint_text="Écris ta note ici", multiline=True, size_hint=(1,1))
            info = Label(text="Aucun vocal enregistré", color=(0.85,0.85,0.85,1), size_hint=(1,None), height=dp(34))

            row = BoxLayout(size_hint=(1,None), height=dp(46), spacing=dp(5))
            b_start = Button(text="DÉMARRER MICRO", background_color=(0,0.35,0.05,1))
            b_stop = Button(text="STOP MICRO", background_color=(0.55,0.28,0,1))
            b_del = Button(text="SUPPRIMER VOCAL", background_color=(0.45,0.05,0.05,1))
            row.add_widget(b_start); row.add_widget(b_stop); row.add_widget(b_del)

            send = Button(text="ENVOYER POUR VALIDATION", size_hint=(1,None), height=dp(46), background_color=(0,0.42,0.10,1))
            cancel = Button(text="FERMER", size_hint=(1,None), height=dp(42), background_color=(0.25,0.25,0.25,1))
            for w in [auteur, texte, info, row, send, cancel]:
                box.add_widget(w)

            p2 = Popup(title="Nouvelle note", content=box, size_hint=(0.94,0.86))

            def start_rec(*__):
                ok, res = recorder.start()
                if ok:
                    state["audio_path"] = res
                    info.text = "Enregistrement en cours..."
                else:
                    info.text = res

            def stop_rec(*__):
                ok, res = recorder.stop()
                if ok:
                    state["audio_path"] = res
                    info.text = "Vocal enregistré : " + os.path.basename(res)
                else:
                    info.text = res
            ...
            def del_rec(*__):
                recorder.delete()
                state["audio_path"] = ""
                info.text = "Vocal supprimé"

            def do_send(*__):
                if not texte.text.strip() and not state.get("audio_path"):
                    msg(self, "NOTE", "Écris une note ou enregistre un vocal avant d’envoyer.")
                    return
                send.disabled = True
                send.text = "ENVOI..."
                def worker():
                    try:
                        audio_url = upload_audio(state["audio_path"], theme) if state.get("audio_path") else ""
                        send_note(theme, auteur.text.strip() or "Anonyme", texte.text.strip(), audio_url)
                        err = ""
                    except Exception as e:
                        err = str(e)[:500]
                    def done(dt):
                        ...
                        if err:
                            ...
                            p2.dismiss()
                            msg(self, "NOTE", "Note envoyée.\nElle apparaîtra après validation admin.")
                    Clock.schedule_once(done, 0)
                threading.Thread(target=worker, daemon=True).start()

            b_start.bind(on_release=start_rec)
            b_stop.bind(on_release=stop_rec)
            b_del.bind(on_release=del_rec)
            send.bind(on_release=do_send)
            cancel.bind(on_release=lambda x: p2.dismiss())
            p2.open()

        b_refresh.bind(on_release=refresh)
        b_add.bind(on_release=add_note)
        close.bind(on_release=lambda x: pop.dismiss())
        pop.open()
        refresh()
    ...
    try:
        old_build_ui = MoneyRoot.build_ui
        def build_ui_no_favoris(self):
            old_build_ui(self)
            Clock.schedule_once(lambda dt: remove_favoris_widgets(self), 0.1)
            Clock.schedule_once(lambda dt: remove_favoris_widgets(self), 0.8)
        MoneyRoot.build_ui = build_ui_no_favoris
    except Exception:
        pass

    try:
        from kivy.uix.popup import Popup as _Popup
        old_open = _Popup.open
        def open_no_favoris(self, *args, **kwargs):
            r = old_open(self, *args, **kwargs)
            Clock.schedule_once(lambda dt: remove_favoris_widgets(self), 0.1)
            return r
        _Popup.open = open_no_favoris
    except Exception:
        pass

_geostar_micro_direct_supabase_patch()



# ============================================================
# PATCH FINAL GEOSTAR — AUDIO ANDROID DIRECT + ECOUTE AVANT ENVOI
# ============================================================

def _geostar_audio_final_patch():
    import os, json, ssl, time, threading, urllib.request, urllib.parse
    from datetime import datetime
    from kivy.app import App
    from kivy.clock import Clock
    from kivy.metrics import dp
    ...
    SUPABASE_URL = "https://kvgjghvcptryghggzuui.supabase.co"
    SUPABASE_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXAiLCJyZWYiOiJrdmdqZ2h2Y3B0cnlnaGdnenV1aSIsInJvbGUiOiJhbm9uIiwiaWF0IjoxNzc5NzA5MTMzLCJleHAiOjIwOTUyODUxMzN9.9nQK19ixZzc8A8P6ca-u7nTXfS5wAmYf8ur3V4biA9E"
    # Si ton ancien anon key fonctionne déjà, il sera conservé automatiquement ci-dessous.
    try:
        old_consts = _geostar_micro_direct_supabase_patch.__code__.co_consts
        for c in old_consts:
            if isinstance(c, str) and c.startswith('eyJ') and len(c) > 80:
                SUPABASE_KEY = c
    except Exception:
        pass
    AUDIO_BUCKET = "geostar-audio"
    ...
    try:
        old_build = MoneyApp.build
        def build_with_audio_permission(self):
            ask_audio_permission()
            return old_build(self)
        MoneyApp.build = build_with_audio_permission
    except Exception:
        pass

    def message(root, title, text):
        try:
            root.message(title, text)
        except Exception:
            Popup(title=title, content=Label(text=str(text)), size_hint=(0.86, 0.45)).open()

    def app_audio_dir():
        try:
            folder = App.get_running_app().user_data_dir
        except Exception:
            folder = _get_data_dir()
    ...
    class AndroidAudio:
        ...
        def start(self):
            ask_audio_permission()
            if platform != "android":
                return False, "Le micro direct fonctionne dans l’APK Android."
            try:
                from jnius import autoclass
                MediaRecorder = autoclass("android.media.MediaRecorder")
                AudioSource = autoclass("android.media.MediaRecorder$AudioSource")
                OutputFormat = autoclass("android.media.MediaRecorder$OutputFormat")
                AudioEncoder = autoclass("android.media.MediaRecorder$AudioEncoder")
                self.path = os.path.join(app_audio_dir(), "geostar_vocal_%s.m4a" % int(time.time()))
                rec = MediaRecorder()
                rec.setAudioSource(AudioSource.MIC)
                rec.setOutputFormat(OutputFormat.MPEG_4)
                rec.setAudioEncoder(AudioEncoder.AAC)
                ...
                self.rec = None
                self.recording = False
                return False, "Erreur micro : " + str(e)[:300]
        ...
        def stop(self):
            try:
                ...
                if self.path and os.path.exists(self.path):
                    return True, self.path
                return False, "Aucun fichier vocal créé."
            except Exception as e:
                self.rec = None
                self.recording = False
                return False, "Erreur stop : " + str(e)[:300]

        def play(self):
            if not self.path or not os.path.exists(self.path):
                return False, "Aucun vocal à écouter."
            try:
                from jnius import autoclass
                MediaPlayer = autoclass("android.media.MediaPlayer")
                if self.player:
                    try:
                        self.player.stop(); self.player.release()
                    except Exception:
                        pass
                p = MediaPlayer()
                p.setDataSource(self.path)
                p.prepare()
                p.start()
                self.player = p
                return True, "Lecture du vocal."
            except Exception as e:
                return False, "Erreur lecture : " + str(e)[:300]
    ...
    def theme_key(root):
        try:
            return "-".join(str(x) for x in root.mother_codes)
        except Exception:
            try:
                return str(root.current_key())
            except Exception:
                return "general"
    ...
    def upload_audio(path, theme):
        if not path or not os.path.exists(path):
            return ""
        safe_theme = re.sub(r"[^A-Za-z0-9_\-]", "_", str(theme)) if "re" in globals() else str(theme).replace("/", "_").replace(" ", "_")
        object_path = safe_theme + "/" + os.path.basename(path)
        # Important : garder les / dans le chemin Storage. Les encoder en %2F provoque souvent HTTP 400.
        quoted = urllib.parse.quote(object_path, safe="/")
        url = SUPABASE_URL + "/storage/v1/object/" + AUDIO_BUCKET + "/" + quoted
        with open(path, "rb") as f:
            data = f.read()
        ...
        try:
            ...
            raise Exception("Upload audio Supabase erreur " + str(e.code) + " : " + detail)
        return SUPABASE_URL + "/storage/v1/object/public/" + AUDIO_BUCKET + "/" + quoted

    def send_note(root, theme, auteur, texte, audio_url):
        code = getattr(root, "activation_code", "") or getattr(root, "code_activation", "") or getattr(root, "licence_code", "") or ""
        audio_name = os.path.basename(audio_url.split("?")[0]) if audio_url else ""
        payload = {
            "theme_key": str(theme),
            "auteur": auteur or "Anonyme",
            "code_licence": code,
        ...
        try:
            ...
            raise Exception("Envoi note Supabase erreur " + str(e.code) + " : " + detail)
        ...
        def refresh(*_):
            ...
            def worker():
                ...
                def done(dt):
                    grid.clear_widgets()
                    if err:
                        grid.add_widget(Label(text="Erreur Supabase :\n" + err, color=(1,0.35,0.35,1), size_hint_y=None, height=dp(120)))
                        return
                    if not notes:
                        grid.add_widget(Label(text="Aucune note validée pour ce thème.", color=(0.82,0.82,0.82,1), size_hint_y=None, height=dp(80)))
                        return
                    for n in notes:
                        ...
                        audio = str(n.get("audio_url", "") or "")
                        date = str(n.get("date_validation", "") or n.get("date_soumission", ""))[:19]
                        card = BoxLayout(orientation="vertical", padding=dp(10), spacing=dp(5), size_hint_y=None, height=dp(225 if audio else 170))
                        card.add_widget(Label(text="[b]" + auteur + "[/b]\n" + date, markup=True, color=(1,0.82,0.1,1), size_hint=(1,None), height=dp(42)))
                        card.add_widget(Label(text=texte[:330] if texte else "(Vocal uniquement)", color=(1,1,1,1), size_hint=(1,None), height=dp(58)))
                        if audio:
                            card.add_widget(Label(text="🎙 Vocal envoyé", color=(0.75,0.85,1,1), size_hint=(1,None), height=dp(25)))
                            b_audio = Button(text="▶ ÉCOUTER VOCAL", size_hint=(1,None), height=dp(44), background_color=(0.05,0.25,0.60,1))
                            def open_audio(btn, url=audio):
                                try:
                                    if platform == "android":
                                        from jnius import autoclass
                                        MediaPlayer = autoclass("android.media.MediaPlayer")
                                        ...
                                        mp.prepareAsync()
                                        boxp = BoxLayout(orientation="vertical", padding=dp(12), spacing=dp(10))
                                        info_p = Label(text="Chargement du vocal depuis Supabase...", color=(1,1,1,1), size_hint=(1,None), height=dp(80))
                                        rowp = BoxLayout(orientation="horizontal", spacing=dp(6), size_hint=(1,None), height=dp(48))
                                        bplay = Button(text="▶ LIRE", background_color=(0,0.45,0.12,1))
                                        bstop = Button(text="■ STOP", background_color=(0.55,0.05,0.05,1))
                                        rowp.add_widget(bplay); rowp.add_widget(bstop)
                                        bclose = Button(text="FERMER", size_hint=(1,None), height=dp(46), background_color=(0.25,0.25,0.25,1))
                                        boxp.add_widget(info_p); boxp.add_widget(rowp); boxp.add_widget(bclose)
                                        pp = Popup(title="Lecture vocale", content=boxp, size_hint=(0.9,0.42))
                                        def on_prepared(player):
                                            info_p.text = "Audio prêt. Appuie sur LIRE."
                                        mp.setOnPreparedListener(on_prepared)
                                        bplay.bind(on_release=lambda x: mp.start())
                                        bstop.bind(on_release=lambda x: mp.pause() if mp.isPlaying() else None)
                                        def close_audio(x):
                                            try:
                                                ...
                                                pass
                                            pp.dismiss()
                                        bclose.bind(on_release=close_audio)
                                        pp.open()
                                    else:
                                        import webbrowser
                                        webbrowser.open(url)
                                except Exception as e:
                                    message(self, "Audio", "Impossible de lire directement le vocal Supabase : " + str(e)[:350])
                            b_audio.bind(on_release=open_audio)
                            card.add_widget(b_audio)
                        grid.add_widget(card)
                Clock.schedule_once(done, 0)
            threading.Thread(target=worker, daemon=True).start()

        def add_note(*_):
            recorder = AndroidAudio()
            state = {"audio_path": ""}
            box = BoxLayout(orientation="vertical", padding=dp(10), spacing=dp(8))
            box.add_widget(Label(text="Nouvelle note\nTu peux enregistrer puis écouter avant d’envoyer.", color=(1,1,1,1), size_hint=(1,None), height=dp(60)))
            auteur = TextInput(hint_text="Nom ou pseudo", multiline=False, size_hint=(1,None), height=dp(44))
            texte = TextInput(hint_text="Écris ta note ici", multiline=True, size_hint=(1,1))
            info = Label(text="Aucun vocal enregistré", color=(0.85,0.85,0.85,1), size_hint=(1,None), height=dp(38))
            row1 = BoxLayout(size_hint=(1,None), height=dp(46), spacing=dp(5))
            b_start = Button(text="DÉMARRER MICRO", background_color=(0,0.35,0.05,1))
            b_stop = Button(text="STOP MICRO", background_color=(0.55,0.28,0,1))
            row1.add_widget(b_start); row1.add_widget(b_stop)
            row2 = BoxLayout(size_hint=(1,None), height=dp(46), spacing=dp(5))
            b_play = Button(text="▶ ÉCOUTER VOCAL", background_color=(0.05,0.25,0.60,1))
            b_del = Button(text="SUPPRIMER VOCAL", background_color=(0.45,0.05,0.05,1))
            row2.add_widget(b_play); row2.add_widget(b_del)
            send = Button(text="ENVOYER POUR VALIDATION", size_hint=(1,None), height=dp(46), background_color=(0,0.42,0.10,1))
            cancel = Button(text="FERMER", size_hint=(1,None), height=dp(42), background_color=(0.25,0.25,0.25,1))
            for w in [auteur, texte, info, row1, row2, send, cancel]:
                box.add_widget(w)
            p2 = Popup(title="Note + vocal", content=box, size_hint=(0.94,0.90))

            def start_rec(*__):
                ok, res = recorder.start()
                if ok:
                    state["audio_path"] = res
                    info.text = "Enregistrement en cours... Appuie sur STOP MICRO."
                else:
                    info.text = res
            ...
            def stop_rec(*__):
                ...
                if ok:
                    state["audio_path"] = res
                    info.text = "Vocal prêt : " + os.path.basename(res) + " — tu peux l’écouter."
                else:
                    info.text = res

            def play_rec(*__):
                ok, res = recorder.play()
                info.text = res

            def del_rec(*__):
                ok, res = recorder.delete()
                state["audio_path"] = ""
                info.text = res

            def do_send(*__):
                if not texte.text.strip() and not state.get("audio_path"):
                    message(self, "NOTE", "Écris une note ou enregistre un vocal avant d’envoyer.")
                    return
                send.disabled = True
                send.text = "ENVOI..."
                def worker():
                    try:
                        audio_url = upload_audio(state["audio_path"], theme) if state.get("audio_path") else ""
                        send_note(self, theme, auteur.text.strip() or "Anonyme", texte.text.strip(), audio_url)
                        err = ""
                    except Exception as e:
                        err = str(e)[:500]
                    def done(dt):
                        ...
                        send.text = "ENVOYER POUR VALIDATION"
                        if err:
                            message(self, "ERREUR", err)
                        else:
                            p2.dismiss()
                            message(self, "NOTE", "Note envoyée.\nElle apparaîtra après validation admin.")
                            refresh()
                    Clock.schedule_once(done, 0)
                threading.Thread(target=worker, daemon=True).start()
            ...
            b_start.bind(on_release=start_rec)
            b_stop.bind(on_release=stop_rec)
            b_play.bind(on_release=play_rec)
            b_del.bind(on_release=del_rec)
            send.bind(on_release=do_send)
            cancel.bind(on_release=lambda x: p2.dismiss())
            p2.open()
    ...
    try:
        MoneyRoot.popup_notes_current = popup_notes_final
        MoneyRoot.ouvrir_communaute = lambda self, *args: self.popup_notes_current()
    except Exception:
        pass

_geostar_audio_final_patch()


# ============================================================
# PATCH FINAL — RECHERCHE RÉPÉTITIONS PAR MAISONS + AUDIO 400
# ============================================================
def _geostar_repetition_search_final_patch():
    from kivy.uix.togglebutton import ToggleButton
    from kivy.uix.boxlayout import BoxLayout
    from kivy.uix.gridlayout import GridLayout
    from kivy.uix.scrollview import ScrollView
    ...
    def popup_search_repetition(self):
        box = BoxLayout(orientation="vertical", spacing=dp(6), padding=dp(8))
        box.add_widget(Label(
            text="1. Figure facultative : laisse vide pour chercher une répétition dans les maisons choisies",
            color=(1,1,1,1), size_hint=(1,None), height=dp(44), font_size=dp(12)
        ))
        selected_figure = [None]
        fig_grid = GridLayout(cols=2, spacing=dp(4), size_hint=(1,None))
        fig_grid.bind(minimum_height=fig_grid.setter("height"))
        for code, data in FIGURES.items():
            tb = ToggleButton(text=f"{data['africain']} / {data['occidental']}", group="figure_choice_final", size_hint=(1,None), height=dp(38), font_size=dp(11))
            def on_fig(btn, c=code):
                selected_figure[0] = c if btn.state == "down" else None
            tb.bind(on_press=on_fig)
            fig_grid.add_widget(tb)
        sv_fig = ScrollView(size_hint=(1,None), height=dp(165))
        sv_fig.add_widget(fig_grid)
        box.add_widget(sv_fig)

        box.add_widget(Label(text="2. Choisis les maisons. Exemple : M3 + M1 + M6", color=(1,1,1,1), size_hint=(1,None), height=dp(32)))
        maison_grid = GridLayout(cols=4, spacing=dp(4), size_hint=(1,None), height=dp(160))
        maison_buttons = {}
        for i in range(1, 17):
            tb = ToggleButton(text=f"M{i}", size_hint=(1,None), height=dp(36), font_size=dp(12))
        ...
        box.add_widget(maison_grid)
        info = Label(
            text="Sans figure : l'app cherche les thèmes où ces maisons ont la même figure, et seulement ces maisons.",
            color=(0.85,0.85,0.85,1), size_hint=(1,None), height=dp(42), font_size=dp(11)
        )
        box.add_widget(info)
        btn_calc = Button(text="CALCULER LES SOLUTIONS", size_hint=(1,None), height=dp(48), background_color=(0.0,0.45,0.18,1))
        box.add_widget(btn_calc)
        pop = Popup(title="Recherche par répétition", content=box, size_hint=(0.97,0.95))

        def on_calculer(_):
            positions = sorted([i for i, tb in maison_buttons.items() if tb.state == "down"])
            if len(positions) < 2:
                self.message("Attention", "Choisis au moins 2 maisons. Exemple : M3, M1, M6.")
                return
            fig_code = selected_figure[0]
            pop.dismiss()
            if fig_code:
                title = FIGURES[fig_code]["africain"] + " exactement en M" + "+M".join(str(p) for p in positions)
            else:
                title = "Répétition exacte en M" + "+M".join(str(p) for p in positions)
            self.open_search_by_figure_window(title, fig_code, positions, True)

        btn_calc.bind(on_release=on_calculer)
        pop.open()

    def open_repetition_window(self, title, fig_code, positions, stricte=True):
        content = BoxLayout(orientation="vertical", padding=dp(6), spacing=dp(6))
        header = Label(text=title + "\nCalcul en cours...", color=(1,1,1,1), size_hint=(1,None), height=dp(58))
        content.add_widget(header)
        search_item = {"kind":"advanced", "name":title, "fig_code":fig_code, "positions":positions or [], "stricte":bool(stricte), "search_type":search_type or "auto", "rare":bool(rare)}
        b_move_search = Button(text="➜ METTRE CETTE RECHERCHE DANS COMBINAISONS", size_hint=(1,None), height=dp(42), background_color=(0.30,0.18,0.45,1))
        b_move_search.bind(on_release=lambda x, it=search_item: (_add_combo(it), _remove_adv_search(it), self.message("COMBINAISONS", "Recherche déplacée dans COMBINAISONS.")))
        content.add_widget(b_move_search)
        sv = ScrollView()
        grid = GridLayout(cols=1, spacing=dp(4), size_hint_y=None)
        grid.bind(minimum_height=grid.setter("height"))
        sv.add_widget(grid)
        content.add_widget(sv)
        combo_search_btn = Button(text="➜ AJOUTER CETTE RECHERCHE AUX COMBINAISONS", size_hint=(1,None), height=dp(42), background_color=(0.30,0.18,0.45,1))
        content.add_widget(combo_search_btn)
        close = Button(text="FERMER", size_hint=(1,None), height=dp(44), background_color=(0.25,0.25,0.25,1))
        content.add_widget(close)
        pop = Popup(title="Solutions", content=content, size_hint=(0.98,0.94))
        close.bind(on_release=lambda x: pop.dismiss())
        pop.open()
        ...
        def fill(dt):
            solutions = []
            seen = set()
            wanted_bits = code_vers_bin(fig_code) if fig_code else None
                ...
                for m1 in TOUTES_LES_FIGURES:
                    for m2 in TOUTES_LES_FIGURES:
                        for m3 in TOUTES_LES_FIGURES:
                            for m4 in TOUTES_LES_FIGURES:
                                theme = developper_theme(m1, m2, m3, m4)
                                vals = [theme[p] for p in positions]
                                if not all(v == vals[0] for v in vals):
                                    continue
                                rep_bits = vals[0]
                                if wanted_bits is not None and rep_bits != wanted_bits:
                                    continue
                                all_pos = [i for i in range(1,17) if theme[i] == rep_bits]
                                # Recherche exacte : la répétition ne doit apparaître que dans les maisons choisies.
                                if set(all_pos) != pos_set:
                                    continue
                                key = (m1,m2,m3,m4,rep_bits)
                                if key in seen:
                                    continue
                                seen.add(key)
                                solutions.append({"m1":m1,"m2":m2,"m3":m3,"m4":m4,"positions":list(positions),"figure":rep_bits,"secondary":"","all_positions":all_pos})

                self.active_solutions = solutions
                self.active_solution_index = -1
                header.text = title + "\n" + str(len(solutions)) + " solution(s)"
                if not solutions:
                    grid.add_widget(Label(text="Aucune solution trouvée.", color=(1,0.35,0.35,1), size_hint_y=None, height=dp(70)))
                    return
                for idx, sol in enumerate(solutions, 1):
                    fig_name = data_fig(sol["figure"])["africain"]
                    pos_txt = "M" + "M".join(str(p) for p in sol.get("all_positions", positions))
                    txt = f"{idx}. {fig_name} en {pos_txt}\nM1={sol['m1']} M2={sol['m2']} M3={sol['m3']} M4={sol['m4']}"
                    b = Button(text=txt, font_size=dp(10), size_hint_y=None, height=dp(58), halign="left", background_color=(0.1,0.32,0.50,1))
                    def apply_it(btn, s=sol, sl=solutions):
                        pop.dismiss()
                        self.apply_solution_from_list(s, sl, None)
                    b.bind(on_release=apply_it)
                    grid.add_widget(b)
            except Exception as e:
                header.text = "Erreur"
                grid.add_widget(Label(text=str(e)[:500], color=(1,0.4,0.4,1), size_hint_y=None, height=dp(100)))

        Clock.schedule_once(fill, 0.1)

    MoneyRoot.popup_search_by_figure = popup_search_repetition
    MoneyRoot.open_search_by_figure_window = open_repetition_window

_geostar_repetition_search_final_patch()


# ============================================================
# PATCH AJOUT — RECHERCHE RÉPÉTITIONS PAR MAISONS (SANS RETIRER L'EXISTANT)
# - Ajoute un bouton dans SOLUTIONS
# - Exemple possible : 3 1 6
# - Cherche une répétition exacte uniquement dans ces maisons
# ============================================================
def _geostar_add_repetition_by_houses_only_patch():
    from kivy.uix.boxlayout import BoxLayout
    from kivy.uix.gridlayout import GridLayout
    from kivy.uix.scrollview import ScrollView
    from kivy.uix.button import Button
    ...
    def _parse_maisons(txt):
        nums = [int(x) for x in re.findall(r"\d+", txt or "")]
        out = []
        for n in nums:
            if 1 <= n <= 16 and n not in out:
                out.append(n)
        return out

    def popup_repetition_maisons(self):
        box = BoxLayout(orientation="vertical", spacing=dp(8), padding=dp(10))
        box.add_widget(Label(
            text="Recherche répétition exacte dans certaines maisons",
            color=(1,1,1,1), size_hint=(1,None), height=dp(34), bold=True
        ))
        box.add_widget(Label(
            text="Exemple : écris 3 1 6 pour trouver les thèmes où la même figure se répète seulement en M1, M3 et M6.",
            color=(0.85,0.85,0.85,1), size_hint=(1,None), height=dp(70), font_size=dp(12)
        ))
        inp_pos = TextInput(
            hint_text="Maisons : exemple 3 1 6",
            multiline=False, size_hint=(1,None), height=dp(48),
            input_filter="int"
        )
        # input_filter=int bloque les espaces sur certains Android, donc on le retire immédiatement.
        try:
            inp_pos.input_filter = None
        except Exception:
            pass
        inp_fig = TextInput(
            hint_text="Figure facultative : exemple Oumar ou 2122. Laisse vide = toutes",
            multiline=False, size_hint=(1,None), height=dp(48)
        )
        box.add_widget(inp_pos)
        box.add_widget(inp_fig)
        btn_calc = Button(text="CALCULER", size_hint=(1,None), height=dp(50), background_color=(0,0.45,0.16,1))
        btn_close = Button(text="FERMER", size_hint=(1,None), height=dp(44), background_color=(0.25,0.25,0.25,1))
        box.add_widget(btn_calc)
        box.add_widget(btn_close)
        pop = Popup(title="Répétitions par maisons", content=box, size_hint=(0.94,0.62))

        def go(_):
            positions = sorted(_parse_maisons(inp_pos.text))
            if len(positions) < 2:
                self.message("Attention", "Écris au moins 2 maisons. Exemple : 3 1 6")
                return
            fig_code = None
            fig_txt = (inp_fig.text or "").strip()
            if fig_txt:
                try:
                    fig_code = entree_vers_code(fig_txt)
                except Exception:
                    self.message("Erreur", "Figure inconnue : " + fig_txt)
                    return
            pop.dismiss()
            self.open_repetition_maisons_result(positions, fig_code)

        btn_calc.bind(on_release=go)
        btn_close.bind(on_release=lambda x: pop.dismiss())
        pop.open()

    def open_repetition_maisons_result(self, positions, fig_code=None):
        title = "Répétition exacte M" + "+M".join(str(p) for p in positions)
        if fig_code:
            title = FIGURES[fig_code]["africain"] + " — " + title
        content = BoxLayout(orientation="vertical", padding=dp(6), spacing=dp(6))
        header = Label(text=title + "\nCalcul en cours...", color=(1,1,1,1), size_hint=(1,None), height=dp(58))
        content.add_widget(header)
        search_item = {"kind":"advanced", "name":title, "fig_code":fig_code, "positions":positions or [], "stricte":bool(stricte), "search_type":search_type or "auto", "rare":bool(rare)}
        ...
        close = Button(text="FERMER", size_hint=(1,None), height=dp(44), background_color=(0.25,0.25,0.25,1))
        content.add_widget(close)
        pop = Popup(title="Solutions répétitions", content=content, size_hint=(0.98,0.94))
        close.bind(on_release=lambda x: pop.dismiss())
        pop.open()
        ...
        def fill(dt):
            ...
            try:
                    ...
                    for m2 in TOUTES_LES_FIGURES:
                        for m3 in TOUTES_LES_FIGURES:
                            for m4 in TOUTES_LES_FIGURES:
                                h = developper_theme(m1, m2, m3, m4)
                                vals = [h[p] for p in positions]
                                if not vals or not all(v == vals[0] for v in vals):
                                    continue
                                rep_bits = vals[0]
                                if wanted_bits is not None and rep_bits != wanted_bits:
                                    continue
                                all_pos = [i for i in range(1, 17) if h[i] == rep_bits]
                                # Exact : la figure ne doit pas apparaître ailleurs.
                                if set(all_pos) != pos_set:
                                    continue
                                key = (m1, m2, m3, m4, rep_bits)
                                if key in seen:
                                ...
                                solutions.append({
                                    "m1":m1, "m2":m2, "m3":m3, "m4":m4,
                                    "positions":list(positions), "figure":rep_bits,
                                    "secondary":"", "all_positions":all_pos
                                })
                self.active_solutions = solutions
                self.active_solution_index = -1
                header.text = title + "\n" + str(len(solutions)) + " solution(s)"
                ...
                for idx, sol in enumerate(solutions, 1):
                    fig_name = data_fig(sol["figure"])["africain"]
                    pos_txt = "M" + "M".join(str(p) for p in sol.get("all_positions", positions))
                    txt = f"{idx}. {fig_name} seulement en {pos_txt}\nM1={sol['m1']} M2={sol['m2']} M3={sol['m3']} M4={sol['m4']}"
                    b = Button(text=txt, font_size=dp(10), size_hint_y=None, height=dp(58), halign="left", background_color=(0.1,0.32,0.50,1))
                    def apply_it(btn, s=sol, sl=solutions):
                        pop.dismiss()
                        self.apply_solution_from_list(s, sl, None)
    ...
    def popup_solutions_plus_repetition(self):
        box = BoxLayout(orientation="vertical", spacing=dp(6), padding=dp(8))
        box.add_widget(Label(text="Solutions intégrées + recherches personnalisées.", color=(1,1,1,1), size_hint=(1,None), height=dp(40)))
        options = [
            ("5-10-16 toutes", [5,10,16], False),
        ...
        add.bind(on_release=lambda x: self.popup_add_custom_search())
        box.add_widget(add)
        if getattr(self, "custom_searches", None):
            box.add_widget(Label(text="Recherches perso :", color=(1,1,1,1), size_hint=(1,None), height=dp(28)))
            for item in self.custom_searches:
                b = Button(text=f"{item.get('name')} : {item.get('positions')}", size_hint=(1,None), height=dp(38))
                b.bind(on_release=lambda btn, it=item: self.open_solution_window(it.get("name","Perso"), it.get("positions",[]), False))
        ...
        btn_fig.bind(on_release=lambda x: self.popup_search_by_figure())
        box.add_widget(btn_fig)
        btn_rep = Button(text="🔁 RÉPÉTITIONS PAR MAISONS", size_hint=(1,None), height=dp(42), background_color=(0.48,0.20,0.65,1))
        btn_rep.bind(on_release=lambda x: self.popup_repetition_maisons())
        box.add_widget(btn_rep)
        manage = Button(text="SUPPRIMER UNE RECHERCHE PERSO", size_hint=(1,None), height=dp(42))
        manage.bind(on_release=lambda x: self.popup_delete_custom_search())
        box.add_widget(manage)
        sv_main = ScrollView()
        ...
        outer = BoxLayout(orientation="vertical", spacing=dp(4), padding=dp(4))
        outer.add_widget(sv_main)
        btn_close_sols = Button(text="FERMER", size_hint=(1,None), height=dp(44), background_color=(0.3,0.3,0.3,1))
        outer.add_widget(btn_close_sols)
        pop_sols = Popup(title="SOLUTIONS", content=outer, size_hint=(0.94,0.92))
        btn_close_sols.bind(on_release=lambda x: pop_sols.dismiss())
        pop_sols.open()

    MoneyRoot.popup_repetition_maisons = popup_repetition_maisons
    MoneyRoot.open_repetition_maisons_result = open_repetition_maisons_result
    MoneyRoot.popup_solutions = popup_solutions_plus_repetition

_geostar_add_repetition_by_houses_only_patch()

# ============================================================
# PATCH FINAL — RECHERCHE AVANCEE UNIQUE + COMBINAISONS
# Ajoute sans retirer :
# - Une seule page de recherche avancee
# - Figure facultative
# - Maisons facultatives
# - STRICTE / NON STRICTE
# - Repetition / figure dans maisons / toutes solutions
# - Deplacer une recherche vers COMBINAISONS => disparait de RECHERCHES
# - Bouton FERMER sur chaque fenetre creee ici
# ============================================================
def _geostar_recherche_avancee_unique_patch():
    from kivy.uix.togglebutton import ToggleButton
    from kivy.uix.boxlayout import BoxLayout
    from kivy.uix.gridlayout import GridLayout
    from kivy.uix.scrollview import ScrollView
    ...
    ADV_COMBO_FILE = _os.path.join(_base_dir(), "geostar_solution_combos.json")
    ADV_SEARCH_FILE = _os.path.join(_base_dir(), "geostar_advanced_searches.json")
    SAVED_FILE = _get_saved_file()

    PREDEF_SEARCHES = [
        {"kind":"predef", "name":"5-10-16 toutes", "positions":[5,10,16], "rare":False},
        {"kind":"predef", "name":"5-10-16 rares 7-13/7-15", "positions":[5,10,16], "rare":True},
        {"kind":"predef", "name":"3-10-15 toutes", "positions":[3,10,15], "rare":False},
        {"kind":"predef", "name":"10-11-15", "positions":[10,11,15], "rare":False},
        {"kind":"predef", "name":"3-11-15", "positions":[3,11,15], "rare":False},
        {"kind":"predef", "name":"2-3-13", "positions":[2,3,13], "rare":False},
        {"kind":"predef", "name":"10-13-15", "positions":[10,13,15], "rare":False},
        {"kind":"predef", "name":"2-10-15", "positions":[2,10,15], "rare":False},
    ]

    def _combo_items():
        data = read_json(ADV_COMBO_FILE, [])
        return data if isinstance(data, list) else []
    ...
    def _adv_items():
        data = read_json(ADV_SEARCH_FILE, [])
        return data if isinstance(data, list) else []

    def _adv_key(item):
        return json.dumps(item, ensure_ascii=False, sort_keys=True)

    def _add_adv_search(item):
        data = _adv_items()
        if not any(_adv_key(x) == _adv_key(item) for x in data):
            data.append(item)
            write_json(ADV_SEARCH_FILE, data)
        return data

    def _remove_adv_search(item):
        data = [x for x in _adv_items() if _adv_key(x) != _adv_key(item)]
        write_json(ADV_SEARCH_FILE, data)
        return data

    def _is_moved(item):
        key = _combo_key(item)
        return any(_combo_key(x) == key for x in _combo_items())

    def _add_combo(item):
        data = _combo_items()
        if not any(_combo_key(x) == _combo_key(item) for x in data):
            data.append(item)
            write_json(ADV_COMBO_FILE, data)
        return data

    def _remove_combo(item):
        data = [x for x in _combo_items() if _combo_key(x) != _combo_key(item)]
        write_json(ADV_COMBO_FILE, data)
        return data

    def _entry_title(item):
        name = item.get("name") or "Recherche"
        fig = item.get("fig_code")
        if fig:
            try:
                name += " | " + FIGURES[fig]["africain"]
            except Exception:
                pass
        pos = item.get("positions") or []
        if pos:
            name += " | M" + "+M".join(str(p) for p in pos)
        mode = "STRICTE" if item.get("stricte", item.get("rare", False)) else "NON STRICTE"
        return name + " [" + mode + "]"

    def _search_solutions(fig_code=None, positions=None, stricte=True, search_type="auto", rare=False, limit=None):
        positions = sorted(set([int(p) for p in (positions or []) if 1 <= int(p) <= 16]))
        pos_set = set(positions)
        target_bits = code_vers_bin(fig_code) if fig_code else None
        out = []
        seen = set()
            ...
            for m2 in TOUTES_LES_FIGURES:
                for m3 in TOUTES_LES_FIGURES:
                    for m4 in TOUTES_LES_FIGURES:
                        ...
                        if search_type in ("repetition", "auto"):
                            if positions:
                                vals = [h[p] for p in positions]
                                if vals and all(v == vals[0] for v in vals):
                                    rep_bits = vals[0]
                                    if target_bits is None or rep_bits == target_bits:
                                        all_pos = [i for i in range(1,17) if h[i] == rep_bits]
                                        ok = (set(all_pos) == pos_set) if stricte else pos_set.issubset(set(all_pos))
                            else:
                                reps = analyser_repetitions(h)
                                for bits, pp in reps.items():
                        ...
                        if not ok and search_type in ("figure", "all", "auto"):
                            if target_bits is not None:
                                all_pos = [i for i in range(1,17) if h[i] == target_bits]
                                if positions:
                                    ok = (set(all_pos) == pos_set) if stricte else pos_set.issubset(set(all_pos))
                                else:
                                    ok = len(all_pos) >= 1
                                rep_bits = target_bits
                            elif positions:
                                vals = [h[p] for p in positions]
                                if vals:
                                    if stricte:
                                        # Sans figure en stricte : les maisons choisies doivent porter la meme figure et seulement ces maisons.
                                        if all(v == vals[0] for v in vals):
                                            rep_bits = vals[0]
                                            all_pos = [i for i in range(1,17) if h[i] == rep_bits]
                                            ok = set(all_pos) == pos_set
                                    else:
                                        # Sans figure non stricte : il suffit qu'une figure soit presente dans toutes les maisons choisies.
                                        if all(v == vals[0] for v in vals):
                                            rep_bits = vals[0]
                                            all_pos = [i for i in range(1,17) if h[i] == rep_bits]
                                            ok = True
                            else:
                                # Recherche globale : toutes les combinaisons.
                                rep_bits = h[15]
                                all_pos = [15]
                                ok = True
                        ...
                        if not ok:
                            continue
                        if rare:
                            if not (h[7] == h[13] or h[7] == h[15]):
                                continue

                        key = (m1,m2,m3,m4,rep_bits,tuple(all_pos),search_type,stricte)
                        if key in seen:
                            continue
                        seen.add(key)
                        out.append({
                            "m1":m1, "m2":m2, "m3":m3, "m4":m4,
                            "figure":rep_bits or h[15],
                            "positions":positions[:],
    ...
    def _save_theme_solution(self, sol, mode="S"):
        saved = read_json(SAVED_FILE, [])
        fig_bits = sol.get("figure", "1111")
        fig_nom = data_fig(fig_bits).get("africain", "?")
        pos = sol.get("all_positions") or sol.get("positions") or []
        pos_str = "M" + "M".join(str(p) for p in pos) if pos else "GLOBAL"
        entry = {
            "figure_nom": fig_nom,
            "figure_bits": fig_bits,
            "positions_str": pos_str,
        ...
        if entry not in saved:
            saved.append(entry)
            write_json(SAVED_FILE, saved)
            self.message("Sauvegarde", "Ajoute dans SAUVEGARDE / COMBINAISONS.")
        else:
            self.message("Sauvegarde", "Cette combinaison existe deja.")

    def _open_results(self, title, fig_code=None, positions=None, stricte=True, search_type="auto", rare=False):
        content = BoxLayout(orientation="vertical", padding=dp(6), spacing=dp(6))
        header = Label(text=title + "\nCalcul en cours...", color=(1,1,1,1), size_hint=(1,None), height=dp(58))
        content.add_widget(header)
        search_item = {"kind":"advanced", "name":title, "fig_code":fig_code, "positions":positions or [], "stricte":bool(stricte), "search_type":search_type or "auto", "rare":bool(rare)}
        ...
        close = Button(text="FERMER", size_hint=(1,None), height=dp(44), background_color=(0.25,0.25,0.25,1))
        content.add_widget(close)
        pop = Popup(title="Résultats", content=content, size_hint=(0.98,0.95))
        try:
            self.register_solution_popup(pop)
        except Exception:
            pass
        ...
        def fill(dt):
            try:
                sols = _search_solutions(fig_code, positions, stricte, search_type, rare)
                self.active_solutions = sols
                self.active_solution_index = -1
                header.text = title + "\n" + str(len(sols)) + " solution(s)"
                if not sols:
                    grid.add_widget(Label(text="Aucune solution trouvee.", color=(1,0.35,0.35,1), size_hint_y=None, height=dp(70)))
                    return
                for idx, sol in enumerate(sols, 1):
                    fig = data_fig(sol.get("figure", "1111"))["africain"]
                    pos = sol.get("all_positions") or sol.get("positions") or []
                    pos_txt = "M" + "M".join(str(p) for p in pos) if pos else "global"
                    txt = f"{idx}. {fig} | {pos_txt}\nM1={sol['m1']} M2={sol['m2']} M3={sol['m3']} M4={sol['m4']}"
                    row = BoxLayout(orientation="horizontal", spacing=dp(3), size_hint_y=None, height=dp(62))
                    b_open = Button(text=txt, font_size=dp(9.5), halign="left", size_hint=(0.70,1), background_color=(0.10,0.30,0.50,1))
                    b_save = Button(text="SAVE", size_hint=(0.15,1), font_size=dp(10), background_color=(0.10,0.45,0.20,1))
                    b_combo = Button(text="COMBO", size_hint=(0.15,1), font_size=dp(9), background_color=(0.30,0.20,0.50,1))
                    def open_it(btn, s=sol, sl=sols):
                        pop.dismiss()
                        self.apply_solution_from_list(s, sl, None)
                    def save_it(btn, s=sol):
                        _save_theme_solution(self, s, "S" if stricte else "NS")
                    def combo_it(btn, s=sol):
                        _add_combo(search_item)
                        _remove_adv_search(search_item)
                        self.message("COMBINAISONS", "Recherche déplacée dans COMBINAISONS.")
                    b_open.bind(on_release=open_it)
                    b_save.bind(on_release=save_it)
                    b_combo.bind(on_release=combo_it)
                    row.add_widget(b_open); row.add_widget(b_save); row.add_widget(b_combo)
                    grid.add_widget(row)
            except Exception as e:
                header.text = "Erreur"
                grid.add_widget(Label(text=str(e)[:700], color=(1,0.4,0.4,1), size_hint_y=None, height=dp(120)))
        Clock.schedule_once(fill, 0.1)

    def _open_entry(self, item):
        if item.get("kind") == "predef":
            return self.open_solution_window(item.get("name","Recherche"), item.get("positions",[]), item.get("rare",False))
        return _open_results(
            self,
            _entry_title(item),
            item.get("fig_code"),
            item.get("positions", []),
            bool(item.get("stricte", True)),
            item.get("search_type", "auto"),
            bool(item.get("rare", False)),
        )

    def popup_recherche_avancee(self):
        box = BoxLayout(orientation="vertical", spacing=dp(6), padding=dp(8))
        box.add_widget(Label(
            text="RECHERCHE AVANCÉE UNIQUE\nFigure et maisons sont facultatives. Tout se fait ici.",
            color=(1,1,1,1), size_hint=(1,None), height=dp(55), font_size=dp(13), bold=True
        ))
        selected_fig = [None]
        selected_type = ["auto"]

        box.add_widget(Label(text="1. Figure facultative", color=(0.9,0.9,0.9,1), size_hint=(1,None), height=dp(26)))
        fig_grid = GridLayout(cols=2, spacing=dp(4), size_hint=(1,None))
        fig_grid.bind(minimum_height=fig_grid.setter("height"))
        b_all_fig = ToggleButton(text="TOUTES LES FIGURES", group="adv_fig", state="down", size_hint=(1,None), height=dp(36), background_color=(0.25,0.25,0.25,1))
        def all_fig(_):
            selected_fig[0] = None
        b_all_fig.bind(on_press=all_fig)
        fig_grid.add_widget(b_all_fig)
        for code, data in FIGURES.items():
            tb = ToggleButton(text=f"{data['africain']} / {data['occidental']}", group="adv_fig", size_hint=(1,None), height=dp(36), font_size=dp(10))
            tb.bind(on_press=lambda btn, c=code: selected_fig.__setitem__(0, c))
            fig_grid.add_widget(tb)
        sv_fig = ScrollView(size_hint=(1,None), height=dp(145))
        sv_fig.add_widget(fig_grid)
        box.add_widget(sv_fig)

        box.add_widget(Label(text="2. Maisons facultatives", color=(0.9,0.9,0.9,1), size_hint=(1,None), height=dp(26)))
        maison_grid = GridLayout(cols=4, spacing=dp(4), size_hint=(1,None), height=dp(150))
        maison_buttons = {}
        for i in range(1,17):
            tb = ToggleButton(text="M"+str(i), size_hint=(1,None), height=dp(34), font_size=dp(11))
            maison_buttons[i] = tb
            maison_grid.add_widget(tb)
        box.add_widget(maison_grid)

        box.add_widget(Label(text="3. Mode", color=(0.9,0.9,0.9,1), size_hint=(1,None), height=dp(24)))
        mode_row = BoxLayout(orientation="horizontal", spacing=dp(5), size_hint=(1,None), height=dp(40))
        b_strict = ToggleButton(text="STRICTE", group="adv_mode", state="down", background_color=(0.10,0.50,0.70,1))
        b_non = ToggleButton(text="NON STRICTE", group="adv_mode", background_color=(0.35,0.35,0.35,1))
        mode_row.add_widget(b_strict); mode_row.add_widget(b_non)
        box.add_widget(mode_row)

        selected_type[0] = "auto"
        info = Label(
            text="AUTO actif : l'app choisit seule entre recherche de figure, répétition ou globale selon tes sélections. STRICTE = seulement les maisons choisies. NON STRICTE = maisons choisies + possible ailleurs.",
            color=(0.75,0.75,0.75,1), size_hint=(1,None), height=dp(64), font_size=dp(10)
        )
        box.add_widget(info)
        b_calc = Button(text="CALCULER LES SOLUTIONS", size_hint=(1,None), height=dp(48), background_color=(0.0,0.45,0.18,1))
        b_close = Button(text="FERMER", size_hint=(1,None), height=dp(42), background_color=(0.25,0.25,0.25,1))
        box.add_widget(b_calc); box.add_widget(b_close)
        pop = Popup(title="Recherche avancée", content=box, size_hint=(0.97,0.97))
        try:
            self.register_solution_popup(pop)
        except Exception:
            pass
        def go(_):
            pos = sorted([i for i,tb in maison_buttons.items() if tb.state == "down"])
            fig_code = selected_fig[0]
            stricte = b_strict.state == "down"
            stype = selected_type[0]
            if not pos and not fig_code:
                # Autorise la recherche globale, mais prévient que c'est beaucoup.
                title = "Recherche globale toutes solutions"
            else:
                fig_txt = FIGURES[fig_code]["africain"] if fig_code else "Toutes figures"
                pos_txt = "M" + "+M".join(str(p) for p in pos) if pos else "Toutes maisons"
                title = f"{fig_txt} | {pos_txt} | {'STRICTE' if stricte else 'NON STRICTE'}"
            adv_item = {"kind":"advanced", "name":title, "fig_code":fig_code, "positions":pos, "stricte":bool(stricte), "search_type":stype, "rare":False}
            _add_adv_search(adv_item)
            pop.dismiss()
            _open_results(self, title, fig_code, pos, stricte, stype, False)
        b_calc.bind(on_release=go)
        b_close.bind(on_release=lambda x: pop.dismiss())
        pop.open()

    def popup_solutions_unified(self):
        from kivy.uix.togglebutton import ToggleButton as TB
        self.close_all_solution_popups() if hasattr(self, "close_all_solution_popups") else None
        main = BoxLayout(orientation="vertical", spacing=dp(5), padding=dp(6))
        tabs = BoxLayout(orientation="horizontal", spacing=dp(4), size_hint=(1,None), height=dp(44))
        t_rech = TB(text="RECHERCHES", group="soltabs_uni", state="down", background_color=(0.10,0.55,0.75,1))
        t_save = TB(text="SAUVEGARDE", group="soltabs_uni")
        t_combo = TB(text="COMBINAISONS", group="soltabs_uni")
        tabs.add_widget(t_rech); tabs.add_widget(t_save); tabs.add_widget(t_combo)
        main.add_widget(tabs)
        zone = BoxLayout(size_hint=(1,1))
        main.add_widget(zone)
        close = Button(text="FERMER", size_hint=(1,None), height=dp(44), background_color=(0.1,0.1,0.1,1))
        main.add_widget(close)
        pop = Popup(title="SOLUTIONS", content=main, size_hint=(0.95,0.95))
        try:
            self.register_solution_popup(pop)
        ...
        def make_search_tab():
            sv = ScrollView(); grid = GridLayout(cols=1, spacing=dp(6), size_hint_y=None); grid.bind(minimum_height=grid.setter("height")); sv.add_widget(grid)
            b_adv = Button(text="🔎 RECHERCHE AVANCÉE UNIQUE", size_hint_y=None, height=dp(52), background_color=(0.0,0.38,0.28,1))
            b_adv.bind(on_release=lambda x: self.popup_recherche_avancee())
            grid.add_widget(b_adv)
            advs = _adv_items()
            if advs:
                grid.add_widget(Label(text="Recherches avancées enregistrées", color=(0.9,0.9,0.9,1), size_hint_y=None, height=dp(30)))
                for item in advs:
                    if _is_moved(item):
                        continue
                    row = BoxLayout(orientation="horizontal", spacing=dp(4), size_hint_y=None, height=dp(52))
                    b = Button(text=_entry_title(item), size_hint=(0.62,1), font_size=dp(9.5), halign="left", background_color=(0.12,0.25,0.35,1))
                    b_move = Button(text="➜ COMBO", size_hint=(0.20,1), font_size=dp(9), background_color=(0.30,0.18,0.45,1))
                    b_del = Button(text="X", size_hint=(0.10,1), background_color=(0.65,0.08,0.08,1))
                    b.bind(on_release=lambda x, it=item: _open_entry(self, it))
                    b_move.bind(on_release=lambda x, it=item: (_add_combo(it), _remove_adv_search(it), switch_tab(None)))
                    b_del.bind(on_release=lambda x, it=item: (_remove_adv_search(it), switch_tab(None)))
                    row.add_widget(b); row.add_widget(b_move); row.add_widget(b_del); grid.add_widget(row)
            grid.add_widget(Label(text="Recherches prédéfinies", color=(0.9,0.9,0.9,1), size_hint_y=None, height=dp(30)))
            for item in PREDEF_SEARCHES:
                if _is_moved(item):
                    continue
                row = BoxLayout(orientation="horizontal", spacing=dp(4), size_hint_y=None, height=dp(48))
                b = Button(text=item["name"], size_hint=(0.78,1))
                b_move = Button(text="➜ COMBO", size_hint=(0.22,1), font_size=dp(10), background_color=(0.30,0.18,0.45,1))
                b.bind(on_release=lambda x, it=item: _open_entry(self, it))
                def mv(btn, it=item):
                    _add_combo(it); switch_tab(None)
                b_move.bind(on_release=mv)
                row.add_widget(b); row.add_widget(b_move); grid.add_widget(row)
            if getattr(self, "custom_searches", None):
                grid.add_widget(Label(text="Recherches perso", color=(0.9,0.9,0.9,1), size_hint_y=None, height=dp(30)))
                for it0 in self.custom_searches:
                    item = {"kind":"custom", "name":it0.get("name","Perso"), "positions":it0.get("positions",[]), "fig_code":it0.get("target_code"), "stricte":False, "search_type":"auto"}
                    if _is_moved(item):
                        continue
                    row = BoxLayout(orientation="horizontal", spacing=dp(4), size_hint_y=None, height=dp(48))
                    b = Button(text=item["name"] + " : " + str(item.get("positions", [])), size_hint=(0.78,1), font_size=dp(10))
                    b_move = Button(text="➜ COMBO", size_hint=(0.22,1), font_size=dp(10), background_color=(0.30,0.18,0.45,1))
                    b.bind(on_release=lambda x, it=item: _open_entry(self, it))
                    b_move.bind(on_release=lambda x, it=item: (_add_combo(it), switch_tab(None)))
                    row.add_widget(b); row.add_widget(b_move); grid.add_widget(row)
            b_add = Button(text="+ AJOUTER RECHERCHE PERSONNALISÉE", size_hint_y=None, height=dp(44), background_color=(0.05,0.25,0.45,1))
            b_del = Button(text="SUPPRIMER UNE RECHERCHE PERSO", size_hint_y=None, height=dp(44), background_color=(0.35,0.08,0.08,1))
            b_add.bind(on_release=lambda x: self.popup_add_custom_search())
            b_del.bind(on_release=lambda x: self.popup_delete_custom_search())
            grid.add_widget(b_add); grid.add_widget(b_del)
            return sv

        def make_saved_tab():
            sv = ScrollView(); grid = GridLayout(cols=1, spacing=dp(5), size_hint_y=None); grid.bind(minimum_height=grid.setter("height")); sv.add_widget(grid)
            saved = read_json(SAVED_FILE, [])
            if not saved:
                grid.add_widget(Label(text="Aucun thème sauvegardé.", color=(0.7,0.7,0.7,1), size_hint_y=None, height=dp(70)))
                return sv
            for idx, item in enumerate(saved, 1):
                fig_n = item.get("figure_nom","?"); pos_str = item.get("positions_str",""); mode = item.get("mode","")
                txt = f"{idx}. {fig_n} {pos_str} [{mode}]\nM1={item.get('m1','?')} M2={item.get('m2','?')} M3={item.get('m3','?')} M4={item.get('m4','?')}"
                row = BoxLayout(orientation="horizontal", spacing=dp(3), size_hint_y=None, height=dp(58))
                b = Button(text=txt, font_size=dp(9.5), halign="left", size_hint=(0.75,1))
                b_del = Button(text="X", size_hint=(0.12,1), background_color=(0.65,0.08,0.08,1))
                b_go = Button(text="GO", size_hint=(0.13,1), background_color=(0.1,0.35,0.7,1))
                def go(btn, it=item):
                    pop.dismiss(); self.apply_solution({"m1":it["m1"],"m2":it["m2"],"m3":it["m3"],"m4":it["m4"],"figure":it.get("figure_bits","1111"),"positions":[],"secondary":""})
                def dele(btn, it=item):
                    sv2 = [x for x in read_json(SAVED_FILE, []) if x != it]
                    write_json(SAVED_FILE, sv2); switch_tab(None)
                b.bind(on_release=go); b_go.bind(on_release=go); b_del.bind(on_release=dele)
                row.add_widget(b); row.add_widget(b_go); row.add_widget(b_del); grid.add_widget(row)
            return sv
        ...
        def make_combo_tab():
            ...
            combos = _combo_items()
            if not combos:
                grid.add_widget(Label(text="Aucune recherche déplacée ici.\nDepuis RECHERCHES, utilise ➜ COMBO.", color=(0.7,0.7,0.7,1), size_hint_y=None, height=dp(80)))
                return sv
            for idx, item in enumerate(combos, 1):
                row = BoxLayout(orientation="horizontal", spacing=dp(3), size_hint_y=None, height=dp(58))
                b = Button(text=str(idx)+". "+_entry_title(item), font_size=dp(10), halign="left", size_hint=(0.75,1), background_color=(0.18,0.28,0.45,1))
                b_del = Button(text="X", size_hint=(0.12,1), background_color=(0.65,0.08,0.08,1))
                b_go = Button(text="GO", size_hint=(0.13,1), background_color=(0.1,0.45,0.3,1))
                b.bind(on_release=lambda x, it=item: _open_entry(self, it))
                b_go.bind(on_release=lambda x, it=item: _open_entry(self, it))
                def delc(btn, it=item):
                    _remove_combo(it); switch_tab(None)
                b_del.bind(on_release=delc)
                row.add_widget(b); row.add_widget(b_go); row.add_widget(b_del); grid.add_widget(row)
            return sv

        def switch_tab(btn):
            zone.clear_widgets()
            if t_rech.state == "down": zone.add_widget(make_search_tab())
            elif t_save.state == "down": zone.add_widget(make_saved_tab())
            else: zone.add_widget(make_combo_tab())
        t_rech.bind(on_press=switch_tab); t_save.bind(on_press=switch_tab); t_combo.bind(on_press=switch_tab)
        close.bind(on_release=lambda x: pop.dismiss())
        zone.add_widget(make_search_tab())
        pop.open()

    MoneyRoot.popup_recherche_avancee = popup_recherche_avancee
    MoneyRoot.popup_search_repetition = popup_recherche_avancee
    MoneyRoot.popup_repetition_maisons = popup_recherche_avancee
    MoneyRoot.popup_search_by_figure = popup_recherche_avancee
    MoneyRoot.popup_solutions = popup_solutions_unified

_geostar_recherche_avancee_unique_patch()



# ============================================================
# PATCH FINAL — RECHERCHES PERSISTANTES + COMBINAISONS PROPRES + AFFICHAGE RAPIDE
# ============================================================
def _geostar_final_search_fast_patch():
    from kivy.uix.togglebutton import ToggleButton
    from kivy.uix.boxlayout import BoxLayout
    from kivy.uix.gridlayout import GridLayout
    from kivy.uix.scrollview import ScrollView
    ...
    PREDEF_SEARCHES = [
        {"kind":"predef", "name":"5-10-16 toutes", "positions":[5,10,16], "rare":False, "stricte":True, "search_type":"auto"},
        {"kind":"predef", "name":"5-10-16 rares 7-13/7-15", "positions":[5,10,16], "rare":True, "stricte":True, "search_type":"auto"},
        {"kind":"predef", "name":"3-10-15 toutes", "positions":[3,10,15], "rare":False, "stricte":True, "search_type":"auto"},
        {"kind":"predef", "name":"10-11-15", "positions":[10,11,15], "rare":False, "stricte":True, "search_type":"auto"},
        {"kind":"predef", "name":"3-11-15", "positions":[3,11,15], "rare":False, "stricte":True, "search_type":"auto"},
        {"kind":"predef", "name":"2-3-13", "positions":[2,3,13], "rare":False, "stricte":True, "search_type":"auto"},
        {"kind":"predef", "name":"10-13-15", "positions":[10,13,15], "rare":False, "stricte":True, "search_type":"auto"},
        {"kind":"predef", "name":"2-10-15", "positions":[2,10,15], "rare":False, "stricte":True, "search_type":"auto"},
    ]
    ...
    def _key(item):
        return _json.dumps(item, ensure_ascii=False, sort_keys=True)
    def _adv_items(): return _items(ADV_SEARCH_FILE)
    def _combo_items(): return _items(ADV_COMBO_FILE)
    def _add_adv(item):
        data = _adv_items()
        if not any(_key(x) == _key(item) for x in data):
            data.append(item); _save(ADV_SEARCH_FILE, data)
    def _remove_adv(item):
        _save(ADV_SEARCH_FILE, [x for x in _adv_items() if _key(x) != _key(item)])
    def _add_combo(item):
        data = _combo_items()
        if not any(_key(x) == _key(item) for x in data):
            data.append(item); _save(ADV_COMBO_FILE, data)
    def _remove_combo(item):
        _save(ADV_COMBO_FILE, [x for x in _combo_items() if _key(x) != _key(item)])
    def _move_to_combo(item):
        _add_combo(item)
        if item.get("kind") == "advanced":
            _remove_adv(item)

    def _title(item):
        if item.get("kind") == "predef":
            return item.get("name", "Combinaison")
        fig = item.get("fig_code")
        fig_txt = FIGURES.get(fig, {}).get("africain") if fig else "Toutes figures"
        pos = item.get("positions") or []
        pos_txt = "M" + "+M".join(str(p) for p in pos) if pos else "Toutes maisons"
        mode = "STRICTE" if item.get("stricte", True) else "NON STRICTE"
        name = item.get("name") or (fig_txt + " | " + pos_txt)
        if name.startswith("Toutes figures") or name.startswith("Recherche globale") or " | M" in name:
            return name + " [" + mode + "]"
        return name + " | " + fig_txt + " | " + pos_txt + " [" + mode + "]"

    def _search_solutions(fig_code=None, positions=None, stricte=True, rare=False):
        positions = sorted(set([int(p) for p in (positions or []) if 1 <= int(p) <= 16]))
        pos_set = set(positions)
        target_bits = code_vers_bin(fig_code) if fig_code else None
        out = []
        ...
        for m1 in TOUTES_LES_FIGURES:
            for m2 in TOUTES_LES_FIGURES:
                for m3 in TOUTES_LES_FIGURES:
                    for m4 in TOUTES_LES_FIGURES:
                        h = developper_theme(m1, m2, m3, m4)
                        ok = False; fig_bits = target_bits; all_pos = []
                        if positions:
                            vals = [h[p] for p in positions]
                            if all(v == vals[0] for v in vals):
                                fig_bits = vals[0]
                                if target_bits is None or fig_bits == target_bits:
                                    all_pos = [i for i in range(1,17) if h[i] == fig_bits]
                                    ok = (set(all_pos) == pos_set) if stricte else pos_set.issubset(set(all_pos))
                        else:
                            if target_bits is not None:
                                all_pos = [i for i in range(1,17) if h[i] == target_bits]
                                ok = len(all_pos) > 0
                                fig_bits = target_bits
                            else:
                                # Recherche globale : toutes les 65536 combinaisons, recalculées à neuf.
                                fig_bits = h[15]
                                all_pos = [15]
                                ok = True
                        if ok and rare:
                            ok = (h[7] == h[13] or h[7] == h[15])
                        if not ok: continue
                        key = (m1,m2,m3,m4,fig_bits,tuple(all_pos))
                        if key in seen: continue
                        seen.add(key)
                        out.append({"m1":m1,"m2":m2,"m3":m3,"m4":m4,"figure":fig_bits or h[15],"positions":positions[:],"all_positions":all_pos[:],"secondary":""})
        return out

    def _open_results(self, item):
        title = _title(item)
        fig_code = item.get("fig_code")
        positions = item.get("positions") or []
        stricte = bool(item.get("stricte", True))
        rare = bool(item.get("rare", False))
        content = BoxLayout(orientation="vertical", padding=dp(6), spacing=dp(6))
        header = Label(text=title + "\nCalcul en cours...", color=(1,1,1,1), size_hint=(1,None), height=dp(58))
        content.add_widget(header)
        if item.get("kind") == "advanced":
            bmove = Button(text="➜ DÉPLACER CETTE RECHERCHE VERS COMBINAISONS", size_hint=(1,None), height=dp(42), background_color=(0.30,0.18,0.45,1))
            content.add_widget(bmove)
        sv = ScrollView(); grid = GridLayout(cols=1, spacing=dp(4), size_hint_y=None); grid.bind(minimum_height=grid.setter("height")); sv.add_widget(grid); content.add_widget(sv)
        footer = BoxLayout(orientation="horizontal", spacing=dp(4), size_hint=(1,None), height=dp(44))
        more = Button(text="AFFICHER PLUS", background_color=(0.10,0.25,0.45,1))
        close = Button(text="FERMER", background_color=(0.20,0.20,0.20,1))
        footer.add_widget(more); footer.add_widget(close); content.add_widget(footer)
        pop = Popup(title="Résultats", content=content, size_hint=(0.98,0.95))
        try:
            self.register_solution_popup(pop)
        except Exception:
        ...
        close.bind(on_release=lambda x: pop.dismiss())
        if item.get("kind") == "advanced":
            bmove.bind(on_release=lambda x, it=item: (_move_to_combo(it), self.message("COMBINAISONS", "Recherche déplacée. Elle disparaît de RECHERCHES.")))
        pop.open()
        state = {"sols": [], "shown": 0, "page": 180}
        def add_rows(count=None):
            sols = state["sols"]
            start = state["shown"]
            end = min(len(sols), start + (count or state["page"]))
            for idx in range(start, end):
                sol = sols[idx]
                fig = data_fig(sol.get("figure","1111"))["africain"]
                pos = sol.get("all_positions") or sol.get("positions") or []
                pos_txt = "M" + "M".join(str(p) for p in pos) if pos else "global"
                txt = f"{idx+1}. {fig} | {pos_txt}\nM1={sol['m1']} M2={sol['m2']} M3={sol['m3']} M4={sol['m4']}"
                row = BoxLayout(orientation="horizontal", spacing=dp(3), size_hint_y=None, height=dp(62))
                bopen = Button(text=txt, font_size=dp(9.5), halign="left", size_hint=(0.72,1), background_color=(0.10,0.30,0.50,1))
                bsave = Button(text="SAVE", size_hint=(0.14,1), background_color=(0.10,0.45,0.20,1))
                bcombo = Button(text="COMBO", size_hint=(0.14,1), background_color=(0.30,0.20,0.50,1))
                bopen.bind(on_release=lambda x, s=sol, sl=sols: (pop.dismiss(), self.apply_solution_from_list(s, sl, None)))
                def save_theme(btn, s=sol):
                    saved = read_json(SAVED_FILE, [])
                    fig_bits = s.get("figure","1111"); fig_nom = data_fig(fig_bits).get("africain","?")
                    pp = s.get("all_positions") or s.get("positions") or []
                    entry = {"figure_nom":fig_nom,"figure_bits":fig_bits,"positions_str":"M"+"M".join(str(p) for p in pp) if pp else "GLOBAL","mode":"S" if stricte else "NS","m1":s["m1"],"m2":s["m2"],"m3":s["m3"],"m4":s["m4"]}
                    if entry not in saved:
                        saved.append(entry); write_json(SAVED_FILE, saved)
                    self.message("Sauvegarde", "Thème sauvegardé.")
                bsave.bind(on_release=save_theme)
                bcombo.bind(on_release=lambda x, it=item: (_move_to_combo(it), self.message("COMBINAISONS", "Recherche déplacée vers COMBINAISONS.")))
                row.add_widget(bopen); row.add_widget(bsave); row.add_widget(bcombo); grid.add_widget(row)
            state["shown"] = end
            more.disabled = end >= len(sols)
            more.text = "TOUT AFFICHÉ" if more.disabled else "AFFICHER PLUS (" + str(end) + "/" + str(len(sols)) + ")"
        def fill(dt):
            try:
                sols = _search_solutions(fig_code, positions, stricte, rare)
                state["sols"] = sols
                self.active_solutions = sols; self.active_solution_index = -1
                header.text = title + "\n" + str(len(sols)) + " solution(s) recalculée(s)"
                if not sols:
                    grid.add_widget(Label(text="Aucune solution trouvée.", color=(1,0.35,0.35,1), size_hint_y=None, height=dp(80))); more.disabled=True; return
                add_rows()
            except Exception as e:
                header.text = "Erreur"; grid.add_widget(Label(text=str(e)[:700], color=(1,0.4,0.4,1), size_hint_y=None, height=dp(120))); more.disabled=True
        more.bind(on_release=lambda x: add_rows())
        Clock.schedule_once(fill, 0.05)

    def popup_recherche_avancee(self):
        box = BoxLayout(orientation="vertical", spacing=dp(6), padding=dp(8))
        box.add_widget(Label(text="RECHERCHE AVANCÉE UNIQUE\nAUTO : l'app choisit le bon type selon figure/maisons.", color=(1,1,1,1), size_hint=(1,None), height=dp(55), font_size=dp(13), bold=True))
        selected_fig = [None]
        box.add_widget(Label(text="1. Figure facultative", color=(0.9,0.9,0.9,1), size_hint=(1,None), height=dp(26)))
        fig_grid = GridLayout(cols=2, spacing=dp(4), size_hint=(1,None)); fig_grid.bind(minimum_height=fig_grid.setter("height"))
        allb = ToggleButton(text="TOUTES LES FIGURES", group="adv_fig2", state="down", size_hint=(1,None), height=dp(36)); allb.bind(on_press=lambda x: selected_fig.__setitem__(0, None)); fig_grid.add_widget(allb)
        for code, data in FIGURES.items():
            tb = ToggleButton(text=f"{data['africain']} / {data['occidental']}", group="adv_fig2", size_hint=(1,None), height=dp(36), font_size=dp(10)); tb.bind(on_press=lambda x, c=code: selected_fig.__setitem__(0, c)); fig_grid.add_widget(tb)
        svf = ScrollView(size_hint=(1,None), height=dp(145)); svf.add_widget(fig_grid); box.add_widget(svf)
        box.add_widget(Label(text="2. Maisons facultatives", color=(0.9,0.9,0.9,1), size_hint=(1,None), height=dp(26)))
        mg = GridLayout(cols=4, spacing=dp(4), size_hint=(1,None), height=dp(150)); maison_buttons={}
        for i in range(1,17):
            tb=ToggleButton(text="M"+str(i), size_hint=(1,None), height=dp(34), font_size=dp(11)); maison_buttons[i]=tb; mg.add_widget(tb)
        box.add_widget(mg)
        box.add_widget(Label(text="3. Mode", color=(0.9,0.9,0.9,1), size_hint=(1,None), height=dp(24)))
        mr = BoxLayout(orientation="horizontal", spacing=dp(5), size_hint=(1,None), height=dp(40))
        bs = ToggleButton(text="STRICTE", group="adv_mode2", state="down", background_color=(0.10,0.50,0.70,1)); bn = ToggleButton(text="NON STRICTE", group="adv_mode2", background_color=(0.35,0.35,0.35,1))
        mr.add_widget(bs); mr.add_widget(bn); box.add_widget(mr)
        box.add_widget(Label(text="STRICTE = uniquement ces maisons. NON STRICTE = ces maisons + possible ailleurs. Sans maison = recherche globale.", color=(0.75,0.75,0.75,1), size_hint=(1,None), height=dp(54), font_size=dp(10)))
        bc = Button(text="CALCULER ET ENREGISTRER LA RECHERCHE", size_hint=(1,None), height=dp(48), background_color=(0.0,0.45,0.18,1)); bf = Button(text="FERMER", size_hint=(1,None), height=dp(42), background_color=(0.25,0.25,0.25,1))
        box.add_widget(bc); box.add_widget(bf)
        pop=Popup(title="Recherche avancée", content=box, size_hint=(0.97,0.97)); bf.bind(on_release=lambda x: pop.dismiss())
        def go(_):
            pos = sorted([i for i,t in maison_buttons.items() if t.state == "down"])
            fig = selected_fig[0]; strict = bs.state == "down"
            fig_txt = FIGURES[fig]["africain"] if fig else "Toutes figures"
            pos_txt = "M" + "+M".join(str(p) for p in pos) if pos else "Toutes maisons"
            item = {"kind":"advanced", "name": fig_txt + " | " + pos_txt, "fig_code":fig, "positions":pos, "stricte":bool(strict), "search_type":"auto", "rare":False}
            _add_adv(item); pop.dismiss(); _open_results(self, item)
        bc.bind(on_release=go); pop.open()

    def popup_solutions_final(self):
        from kivy.uix.togglebutton import ToggleButton as TB
        main = BoxLayout(orientation="vertical", spacing=dp(5), padding=dp(6))
        tabs = BoxLayout(orientation="horizontal", spacing=dp(4), size_hint=(1,None), height=dp(44))
        t_rech = TB(text="RECHERCHES", group="soltabs_final", state="down", background_color=(0.10,0.55,0.75,1)); t_save = TB(text="SAUVEGARDE", group="soltabs_final"); t_combo = TB(text="COMBINAISONS", group="soltabs_final")
        tabs.add_widget(t_rech); tabs.add_widget(t_save); tabs.add_widget(t_combo); main.add_widget(tabs)
        zone=BoxLayout(size_hint=(1,1)); main.add_widget(zone)
        close=Button(text="FERMER", size_hint=(1,None), height=dp(44), background_color=(0.1,0.1,0.1,1)); main.add_widget(close)
        pop=Popup(title="SOLUTIONS", content=main, size_hint=(0.95,0.95)); close.bind(on_release=lambda x: pop.dismiss())
        def make_rech():
            sv=ScrollView(); grid=GridLayout(cols=1, spacing=dp(6), size_hint_y=None); grid.bind(minimum_height=grid.setter("height")); sv.add_widget(grid)
            b_adv=Button(text="🔎 RECHERCHE AVANCÉE UNIQUE", size_hint_y=None, height=dp(52), background_color=(0.0,0.38,0.28,1)); b_adv.bind(on_release=lambda x: popup_recherche_avancee(self)); grid.add_widget(b_adv)
            advs=_adv_items()
            if advs:
                grid.add_widget(Label(text="Recherches avancées enregistrées", color=(0.9,0.9,0.9,1), size_hint_y=None, height=dp(30)))
                for it in advs:
                    row=BoxLayout(orientation="horizontal", spacing=dp(4), size_hint_y=None, height=dp(52))
                    b=Button(text=_title(it), size_hint=(0.66,1), font_size=dp(9.5), background_color=(0.12,0.25,0.35,1)); bm=Button(text="➜ COMBO", size_hint=(0.22,1), font_size=dp(9), background_color=(0.30,0.18,0.45,1)); bd=Button(text="X", size_hint=(0.12,1), background_color=(0.65,0.08,0.08,1))
                    b.bind(on_release=lambda x, item=it: _open_results(self, item)); bm.bind(on_release=lambda x, item=it: (_move_to_combo(item), switch(None))); bd.bind(on_release=lambda x, item=it: (_remove_adv(item), switch(None)))
                    row.add_widget(b); row.add_widget(bm); row.add_widget(bd); grid.add_widget(row)
            if getattr(self,"custom_searches",None):
                grid.add_widget(Label(text="Recherches perso", color=(0.9,0.9,0.9,1), size_hint_y=None, height=dp(30)))
                for it0 in self.custom_searches:
                    it={"kind":"advanced", "name":it0.get("name","Recherche perso"), "fig_code":it0.get("target_code"), "positions":it0.get("positions",[]), "stricte":False, "search_type":"auto", "rare":False}
                    row=BoxLayout(orientation="horizontal", spacing=dp(4), size_hint_y=None, height=dp(48))
                    b=Button(text=_title(it), size_hint=(0.78,1), font_size=dp(10)); bm=Button(text="➜ COMBO", size_hint=(0.22,1), background_color=(0.30,0.18,0.45,1))
                    b.bind(on_release=lambda x, item=it: _open_results(self,item)); bm.bind(on_release=lambda x, item=it: (_add_combo(item), switch(None)))
                    row.add_widget(b); row.add_widget(bm); grid.add_widget(row)
            b_add=Button(text="+ AJOUTER RECHERCHE PERSONNALISÉE", size_hint_y=None, height=dp(44), background_color=(0.05,0.25,0.45,1)); b_del=Button(text="SUPPRIMER UNE RECHERCHE PERSO", size_hint_y=None, height=dp(44), background_color=(0.35,0.08,0.08,1))
            b_add.bind(on_release=lambda x:self.popup_add_custom_search()); b_del.bind(on_release=lambda x:self.popup_delete_custom_search())
            grid.add_widget(b_add); grid.add_widget(b_del); return sv
        def make_save():
            sv=ScrollView(); grid=GridLayout(cols=1, spacing=dp(5), size_hint_y=None); grid.bind(minimum_height=grid.setter("height")); sv.add_widget(grid); saved=read_json(SAVED_FILE, [])
            if not saved: grid.add_widget(Label(text="Aucun thème sauvegardé.", color=(0.7,0.7,0.7,1), size_hint_y=None, height=dp(70))); return sv
            for idx,it in enumerate(saved,1):
                txt=f"{idx}. {it.get('figure_nom','?')} {it.get('positions_str','')} [{it.get('mode','')}]\nM1={it.get('m1','?')} M2={it.get('m2','?')} M3={it.get('m3','?')} M4={it.get('m4','?')}"
                row=BoxLayout(orientation="horizontal", spacing=dp(3), size_hint_y=None, height=dp(58)); b=Button(text=txt, font_size=dp(9.5), size_hint=(0.75,1)); bg=Button(text="GO", size_hint=(0.13,1), background_color=(0.1,0.35,0.7,1)); bd=Button(text="X", size_hint=(0.12,1), background_color=(0.65,0.08,0.08,1))
                def go(btn,item=it): pop.dismiss(); self.apply_solution({"m1":item["m1"],"m2":item["m2"],"m3":item["m3"],"m4":item["m4"],"figure":item.get("figure_bits","1111"),"positions":[],"secondary":""})
                def dele(btn,item=it): write_json(SAVED_FILE,[x for x in read_json(SAVED_FILE,[]) if x!=item]); switch(None)
                b.bind(on_release=go); bg.bind(on_release=go); bd.bind(on_release=dele); row.add_widget(b); row.add_widget(bg); row.add_widget(bd); grid.add_widget(row)
            return sv
        def make_combo():
            sv=ScrollView(); grid=GridLayout(cols=1, spacing=dp(5), size_hint_y=None); grid.bind(minimum_height=grid.setter("height")); sv.add_widget(grid)
            grid.add_widget(Label(text="Combinaisons pré-enregistrées", color=(0.9,0.9,0.9,1), size_hint_y=None, height=dp(30)))
            for it in PREDEF_SEARCHES:
                row=BoxLayout(orientation="horizontal", spacing=dp(3), size_hint_y=None, height=dp(50)); b=Button(text=it["name"], size_hint=(0.88,1)); bgo=Button(text="GO", size_hint=(0.12,1), background_color=(0.1,0.45,0.3,1)); b.bind(on_release=lambda x,item=it:_open_results(self,item)); bgo.bind(on_release=lambda x,item=it:_open_results(self,item)); row.add_widget(b); row.add_widget(bgo); grid.add_widget(row)
            combos=_combo_items()
            if combos: grid.add_widget(Label(text="Mes combinaisons", color=(0.9,0.9,0.9,1), size_hint_y=None, height=dp(30)))
            for idx,it in enumerate(combos,1):
                row=BoxLayout(orientation="horizontal", spacing=dp(3), size_hint_y=None, height=dp(58)); b=Button(text=str(idx)+". "+_title(it), font_size=dp(10), size_hint=(0.75,1), background_color=(0.18,0.28,0.45,1)); bg=Button(text="GO", size_hint=(0.13,1), background_color=(0.1,0.45,0.3,1)); bd=Button(text="X", size_hint=(0.12,1), background_color=(0.65,0.08,0.08,1))
                b.bind(on_release=lambda x,item=it:_open_results(self,item)); bg.bind(on_release=lambda x,item=it:_open_results(self,item)); bd.bind(on_release=lambda x,item=it:(_remove_combo(item, self), switch(None)))
                row.add_widget(b); row.add_widget(bg); row.add_widget(bd); grid.add_widget(row)
            return sv
        def switch(btn):
            zone.clear_widgets()
            if t_rech.state == "down": zone.add_widget(make_rech())
            elif t_save.state == "down": zone.add_widget(make_save())
            else: zone.add_widget(make_combo())
        t_rech.bind(on_press=switch); t_save.bind(on_press=switch); t_combo.bind(on_press=switch); zone.add_widget(make_rech()); pop.open()
    MoneyRoot.popup_recherche_avancee = popup_recherche_avancee
    MoneyRoot.popup_search_repetition = popup_recherche_avancee
    MoneyRoot.popup_repetition_maisons = popup_recherche_avancee
    MoneyRoot.popup_search_by_figure = popup_recherche_avancee
    MoneyRoot.popup_solutions = popup_solutions_final

_geostar_final_search_fast_patch()


# ============================================================
# PATCH DEFINITIF — COMBINAISONS = CRITERES DE RECHERCHE
# RECHERCHES = OUTILS UNIQUEMENT / RECALCUL A CHAQUE OUVERTURE
# ============================================================
def _geostar_combos_criteria_final_patch():
    from kivy.uix.togglebutton import ToggleButton
    from kivy.uix.boxlayout import BoxLayout
    from kivy.uix.gridlayout import GridLayout
    from kivy.uix.scrollview import ScrollView
    ...
    PREDEF_SEARCHES = [
        {"kind":"predef", "name":"5-10-16 toutes", "positions":[5,10,16], "rare":False, "stricte":True, "fig_code":None},
        {"kind":"predef", "name":"5-10-16 rares 7-13/7-15", "positions":[5,10,16], "rare":True, "stricte":True, "fig_code":None},
        {"kind":"predef", "name":"3-10-15 toutes", "positions":[3,10,15], "rare":False, "stricte":True, "fig_code":None},
        {"kind":"predef", "name":"10-11-15", "positions":[10,11,15], "rare":False, "stricte":True, "fig_code":None},
        {"kind":"predef", "name":"3-11-15", "positions":[3,11,15], "rare":False, "stricte":True, "fig_code":None},
        {"kind":"predef", "name":"2-3-13", "positions":[2,3,13], "rare":False, "stricte":True, "fig_code":None},
        {"kind":"predef", "name":"10-13-15", "positions":[10,13,15], "rare":False, "stricte":True, "fig_code":None},
        {"kind":"predef", "name":"2-10-15", "positions":[2,10,15], "rare":False, "stricte":True, "fig_code":None},
    ]
    ...
    def _key(item):
        keep = {k:item.get(k) for k in ["kind","name","fig_code","positions","stricte","rare"]}
        return _json.dumps(keep, ensure_ascii=False, sort_keys=True)

    def _combo_items():
        return _items(ADV_COMBO_FILE)
    ...
    def _title(item):
        ...
        mode = "STRICTE" if item.get("stricte", True) else "NON STRICTE"
        if item.get("kind") == "predef":
            return item.get("name", "Recherche")
        return (item.get("name") or (fig_txt + " | " + pos_txt)) + "  [" + mode + "]"
                    ...
                    for m4 in TOUTES_LES_FIGURES:
                        ...
                        if positions:
                            ...
                            if target_bits is not None:
                                ...
                                # Recherche globale : tous les themes, recalcul a neuf.
                                fig_bits = h[15]
                                all_pos = [15]
                                ok = True
                        if ok and rare:
    ...
    def _open_results(self, item):
        ...
        content.add_widget(header)
        if item.get("kind") == "predef":
            bcombo = Button(text="➜ AJOUTER CETTE RECHERCHE DANS COMBINAISONS", size_hint=(1,None), height=dp(42), background_color=(0.30,0.18,0.45,1))
            bcombo.bind(on_release=lambda x, it=item: (_add_combo(it), self.message("COMBINAISONS", "Critère ajouté dans COMBINAISONS.")))
            content.add_widget(bcombo)
        sv = ScrollView()
        grid = GridLayout(cols=1, spacing=dp(4), size_hint_y=None)
        grid.bind(minimum_height=grid.setter("height"))
        sv.add_widget(grid)
        ...
        def add_rows(count=None):
            ...
            for idx in range(start, end):
                ...
                txt = f"{idx+1}. {fig} | {pos_txt}\nM1={sol['m1']} M2={sol['m2']} M3={sol['m3']} M4={sol['m4']}"
                row = BoxLayout(orientation="horizontal", spacing=dp(3), size_hint_y=None, height=dp(62))
                bopen = Button(text=txt, font_size=dp(9.5), halign="left", size_hint=(0.78,1), background_color=(0.10,0.30,0.50,1))
                bsave = Button(text="SAVE", size_hint=(0.22,1), background_color=(0.10,0.45,0.20,1))
                bopen.bind(on_release=lambda x, s=sol, sl=sols: (pop.dismiss(), self.apply_solution_from_list(s, sl, None)))
                def save_theme(btn, s=sol):
                    saved = read_json(SAVED_FILE, [])
                    fig_bits = s.get("figure","1111")
                    fig_nom = data_fig(fig_bits).get("africain","?")
                    pp = s.get("all_positions") or s.get("positions") or []
                    entry = {"figure_nom":fig_nom,"figure_bits":fig_bits,"positions_str":"M"+"M".join(str(p) for p in pp) if pp else "GLOBAL","mode":"S" if item.get("stricte", True) else "NS","m1":s["m1"],"m2":s["m2"],"m3":s["m3"],"m4":s["m4"]}
                    if entry not in saved:
                        saved.append(entry); write_json(SAVED_FILE, saved)
                    self.message("Sauvegarde", "Thème sauvegardé.")
                bsave.bind(on_release=save_theme)
                row.add_widget(bopen); row.add_widget(bsave); grid.add_widget(row)
            state["shown"] = end
            more.disabled = end >= len(sols)
            more.text = "TOUT AFFICHÉ" if more.disabled else "AFFICHER PLUS"

        def calculate(dt):
            try:
                sols = _search_solutions(item.get("fig_code"), item.get("positions") or [], bool(item.get("stricte", True)), bool(item.get("rare", False)))
                state["sols"] = sols
                header.text = title + f"\nTotal : {len(sols)} solution(s) — recalculé à neuf"
                if not sols:
                    grid.add_widget(Label(text="Aucune solution.", color=(1,1,1,1), size_hint_y=None, height=dp(60)))
                    more.disabled = True
                else:
                    add_rows()
            except Exception as e:
                header.text = "Erreur"
                grid.add_widget(Label(text=str(e)[:500], color=(1,.3,.3,1), size_hint_y=None, height=dp(120)))
                more.disabled = True
        more.bind(on_release=lambda x: add_rows())
        pop.open()
        Clock.schedule_once(calculate, 0.05)

    def popup_recherche_avancee(self):
        selected_fig = [None]
        box = BoxLayout(orientation="vertical", spacing=dp(6), padding=dp(8))
        box.add_widget(Label(text="[b]RECHERCHE AVANCÉE UNIQUE[/b]\nChoisis les critères, ils seront enregistrés dans COMBINAISONS.", markup=True, color=(1,1,1,1), size_hint=(1,None), height=dp(54)))
        box.add_widget(Label(text="1. Figure facultative", color=(0.9,0.9,0.9,1), size_hint=(1,None), height=dp(26)))
        fig_grid = GridLayout(cols=2, spacing=dp(4), size_hint=(1,None)); fig_grid.bind(minimum_height=fig_grid.setter("height"))
        allb = ToggleButton(text="TOUTES LES FIGURES", group="adv_fig_final", state="down", size_hint=(1,None), height=dp(38), background_color=(0.10,0.10,0.10,1))
        allb.bind(on_press=lambda x: selected_fig.__setitem__(0, None)); fig_grid.add_widget(allb)
        for code, data in FIGURES.items():
            tb = ToggleButton(text=f"{data['africain']} / {data['occidental']}", group="adv_fig_final", size_hint=(1,None), height=dp(36), font_size=dp(10))
            tb.bind(on_press=lambda x, c=code: selected_fig.__setitem__(0, c)); fig_grid.add_widget(tb)
        svf = ScrollView(size_hint=(1,None), height=dp(145)); svf.add_widget(fig_grid); box.add_widget(svf)
        box.add_widget(Label(text="2. Maisons facultatives", color=(0.9,0.9,0.9,1), size_hint=(1,None), height=dp(26)))
        mg = GridLayout(cols=4, spacing=dp(4), size_hint=(1,None), height=dp(150)); maison_buttons = {}
        for i in range(1,17):
        ...
        box.add_widget(Label(text="3. Mode", color=(0.9,0.9,0.9,1), size_hint=(1,None), height=dp(24)))
        mr = BoxLayout(orientation="horizontal", spacing=dp(5), size_hint=(1,None), height=dp(40))
        bs = ToggleButton(text="STRICTE", group="adv_mode_final", state="down", background_color=(0.10,0.50,0.70,1))
        bn = ToggleButton(text="NON STRICTE", group="adv_mode_final", background_color=(0.35,0.35,0.35,1))
        mr.add_widget(bs); mr.add_widget(bn); box.add_widget(mr)
        box.add_widget(Label(text="STRICTE = uniquement ces maisons. NON STRICTE = ces maisons + possible ailleurs. Sans maison = recherche globale.", color=(0.75,0.75,0.75,1), size_hint=(1,None), height=dp(54), font_size=dp(10)))
        bc = Button(text="CALCULER + ENREGISTRER DANS COMBINAISONS", size_hint=(1,None), height=dp(48), background_color=(0.0,0.45,0.18,1))
        bf = Button(text="FERMER", size_hint=(1,None), height=dp(42), background_color=(0.25,0.25,0.25,1))
        box.add_widget(bc); box.add_widget(bf)
        pop = Popup(title="Recherche avancée", content=box, size_hint=(0.97,0.97))
        bf.bind(on_release=lambda x: pop.dismiss())
        def go(_):
            pos = sorted([i for i,t in maison_buttons.items() if t.state == "down"])
            fig = selected_fig[0]
            strict = bs.state == "down"
            fig_txt = FIGURES[fig]["africain"] if fig else "Toutes figures"
            pos_txt = "M" + "+M".join(str(p) for p in pos) if pos else "Toutes maisons"
            item = {"kind":"combo", "name":fig_txt + " | " + pos_txt, "fig_code":fig, "positions":pos, "stricte":bool(strict), "search_type":"auto", "rare":False}
            item = _add_combo(item)
            pop.dismiss()
            _open_results(self, item)
        bc.bind(on_release=go)
        pop.open()
    ...
    def popup_solutions_final(self):
        ...
        main = BoxLayout(orientation="vertical", spacing=dp(5), padding=dp(6))
        tabs = BoxLayout(orientation="horizontal", spacing=dp(4), size_hint=(1,None), height=dp(44))
        t_rech = TB(text="RECHERCHES", group="soltabs_final2", state="down", background_color=(0.10,0.55,0.75,1))
        t_save = TB(text="SAUVEGARDE", group="soltabs_final2")
        t_combo = TB(text="COMBINAISONS", group="soltabs_final2")
        tabs.add_widget(t_rech); tabs.add_widget(t_save); tabs.add_widget(t_combo); main.add_widget(tabs)
        zone = BoxLayout(size_hint=(1,1)); main.add_widget(zone)
        close = Button(text="FERMER", size_hint=(1,None), height=dp(44), background_color=(0.1,0.1,0.1,1)); main.add_widget(close)
        pop = Popup(title="SOLUTIONS", content=main, size_hint=(0.95,0.95))
        ...
        def make_rech():
            sv = ScrollView(); grid = GridLayout(cols=1, spacing=dp(6), size_hint_y=None); grid.bind(minimum_height=grid.setter("height")); sv.add_widget(grid)
            b_adv = Button(text="🔎 RECHERCHE AVANCÉE UNIQUE", size_hint_y=None, height=dp(52), background_color=(0.0,0.38,0.28,1)); b_adv.bind(on_release=lambda x: popup_recherche_avancee(self)); grid.add_widget(b_adv)
            grid.add_widget(Label(text="Recherches prédéfinies", color=(0.9,0.9,0.9,1), size_hint_y=None, height=dp(32)))
            for it in PREDEF_SEARCHES:
                row = BoxLayout(orientation="horizontal", spacing=dp(4), size_hint_y=None, height=dp(50))
                b = Button(text=it["name"], size_hint=(0.78,1), background_color=(0.36,0.36,0.36,1))
                bm = Button(text="➜ COMBO", size_hint=(0.22,1), background_color=(0.30,0.18,0.45,1), font_size=dp(10))
                b.bind(on_release=lambda x, item=it: _open_results(self, item))
                bm.bind(on_release=lambda x, item=it: (_add_combo(item), self.message("COMBINAISONS", "Critère ajouté dans COMBINAISONS.")))
                row.add_widget(b); row.add_widget(bm); grid.add_widget(row)
            b_add = Button(text="+ AJOUTER RECHERCHE PERSONNALISÉE", size_hint_y=None, height=dp(44), background_color=(0.05,0.25,0.45,1))
            b_add.bind(on_release=lambda x: popup_recherche_avancee(self))
            grid.add_widget(b_add)
            return sv
        ...
        def make_save():
            ...
            saved = read_json(SAVED_FILE, [])
            if not saved:
                grid.add_widget(Label(text="Aucun thème sauvegardé.", color=(0.7,0.7,0.7,1), size_hint_y=None, height=dp(70))); return sv
            for idx,it in enumerate(saved,1):
                txt=f"{idx}. {it.get('figure_nom','?')} {it.get('positions_str','')} [{it.get('mode','')}]\nM1={it.get('m1','?')} M2={it.get('m2','?')} M3={it.get('m3','?')} M4={it.get('m4','?')}"
                row=BoxLayout(orientation="horizontal", spacing=dp(3), size_hint_y=None, height=dp(58)); b=Button(text=txt, font_size=dp(9.5), size_hint=(0.75,1)); bg=Button(text="GO", size_hint=(0.13,1), background_color=(0.1,0.35,0.7,1)); bd=Button(text="X", size_hint=(0.12,1), background_color=(0.65,0.08,0.08,1))
                def go(btn,item=it): pop.dismiss(); self.apply_solution({"m1":item["m1"],"m2":item["m2"],"m3":item["m3"],"m4":item["m4"],"figure":item.get("figure_bits","1111"),"positions":[],"secondary":""})
        ...
        def make_combo():
            ...
            combos = _combo_items()
            if not combos:
                grid.add_widget(Label(text="Aucune combinaison sauvegardée.\nCrée une recherche avancée ou utilise ➜ COMBO depuis RECHERCHES.", color=(0.7,0.7,0.7,1), size_hint_y=None, height=dp(90))); return sv
            grid.add_widget(Label(text="Mes combinaisons de recherche", color=(0.9,0.9,0.9,1), size_hint_y=None, height=dp(32)))
            for idx,it in enumerate(combos,1):
                row = BoxLayout(orientation="horizontal", spacing=dp(3), size_hint_y=None, height=dp(62))
                b = Button(text=str(idx)+". "+_title(it), font_size=dp(10), size_hint=(0.70,1), background_color=(0.18,0.28,0.45,1))
                bg = Button(text="OUVRIR", size_hint=(0.16,1), background_color=(0.1,0.45,0.3,1), font_size=dp(9))
                bd = Button(text="SUPPR.", size_hint=(0.14,1), background_color=(0.65,0.08,0.08,1), font_size=dp(9))
                b.bind(on_release=lambda x,item=it:_open_results(self,item)); bg.bind(on_release=lambda x,item=it:_open_results(self,item)); bd.bind(on_release=lambda x,item=it:(_remove_combo(item, self), switch(None)))
                row.add_widget(b); row.add_widget(bg); row.add_widget(bd); grid.add_widget(row)
            return sv
        ...
        t_rech.bind(on_press=switch); t_save.bind(on_press=switch); t_combo.bind(on_press=switch)
        zone.add_widget(make_rech()); pop.open()
...
_geostar_combos_criteria_final_patch()



# ============================================================
# PATCH FINAL MENU SOLUTIONS — COMBINAISONS = CRITÈRES UNIQUEMENT
# - ADMIN non modifié
# - Onglet RECHERCHES : outils seulement
# - Onglet COMBINAISONS : combos préenregistrés + recherches avancées/personnalisées
# - Les solutions calculées ne sont JAMAIS stockées dans COMBINAISONS
# ============================================================
def _geostar_menu_combos_criteres_final_patch():
    import os as _os
    ...
    PREDEF_COMBOS = [
        {"kind":"predef", "name":"5-10-16 toutes", "positions":[5,10,16], "rare":False, "stricte":True, "fig_code":None, "search_type":"auto"},
        {"kind":"predef", "name":"5-10-16 rares 7-13/7-15", "positions":[5,10,16], "rare":True, "stricte":True, "fig_code":None, "search_type":"auto"},
        {"kind":"predef", "name":"3-10-15 toutes", "positions":[3,10,15], "rare":False, "stricte":True, "fig_code":None, "search_type":"auto"},
        {"kind":"predef", "name":"10-11-15", "positions":[10,11,15], "rare":False, "stricte":True, "fig_code":None, "search_type":"auto"},
        {"kind":"predef", "name":"3-11-15", "positions":[3,11,15], "rare":False, "stricte":True, "fig_code":None, "search_type":"auto"},
        {"kind":"predef", "name":"2-3-13", "positions":[2,3,13], "rare":False, "stricte":True, "fig_code":None, "search_type":"auto"},
        {"kind":"predef", "name":"10-13-15", "positions":[10,13,15], "rare":False, "stricte":True, "fig_code":None, "search_type":"auto"},
        {"kind":"predef", "name":"2-10-15", "positions":[2,10,15], "rare":False, "stricte":True, "fig_code":None, "search_type":"auto"},
    ]
    ...
    COMBO_FILE = _os.path.join(_base_dir(), "geostar_combos_criteres.json")
    HIDE_PREDEF_FILE = _os.path.join(_base_dir(), "geostar_combos_predef_caches.json")
    SAVED_FILE = _get_saved_file()
    ...
    def _save(path, data):
        write_json(path, data if isinstance(data, list) else [])

    def _norm_item(item):
        item = dict(item or {})
        if "positions" in item and item.get("positions") is not None:
            try:
                item["positions"] = sorted([int(x) for x in item.get("positions", []) if 1 <= int(x) <= 16])
            except Exception:
                item["positions"] = []
        item.setdefault("fig_code", None)
        item.setdefault("stricte", True)
        item.setdefault("rare", False)
        item.setdefault("search_type", "auto")
        return item
    ...
    def _key(item):
        ...
        keep = {
            ...
            "stricte": bool(item.get("stricte", True)),
            "rare": bool(item.get("rare", False)),
            "search_type": item.get("search_type", "auto"),
        }
        return _json.dumps(keep, ensure_ascii=False, sort_keys=True)

    def _combo_items():
        return [_norm_item(x) for x in _items(COMBO_FILE)]

    def _hidden_predefs():
        return set(str(x) for x in _items(HIDE_PREDEF_FILE))

    def _hide_predef(item):
        data = list(_hidden_predefs())
        k = _key(item)
        if k not in data:
            data.append(k)
        _save(HIDE_PREDEF_FILE, data)

    def _add_combo(item):
        item = _norm_item(item)
        if item.get("kind") not in ("predef", "custom"):
            item["kind"] = "advanced"
        data = _combo_items()
        if not any(_key(x) == _key(item) for x in data):
            data.append(item)
            _save(COMBO_FILE, data)
        return item

    def _remove_combo(item, owner=None):
        item = _norm_item(item)
        if item.get("kind") == "predef":
            _hide_predef(item)
            return

        # Supprime dans le fichier COMBINAISONS
        cur_combos = _combo_items()
        _save(COMBO_FILE, [x for x in cur_combos if _key(x) != _key(item)])

        # Supprime aussi les anciennes recherches personnalisées qui réapparaissent via CUSTOM_FILE
        try:
            cur = read_json(CUSTOM_FILE, [])
            kept = []
            for c in cur if isinstance(cur, list) else []:
                c_item = _custom_to_combo(c)
                if _key(c_item) != _key(item):
                    kept.append(c)
            write_json(CUSTOM_FILE, kept)
            if owner is not None and hasattr(owner, "custom_searches"):
                owner.custom_searches = kept
        except Exception:
            pass

    def _custom_to_combo(c):
        c = dict(c or {})
        positions = c.get("positions") or []
        name = c.get("name") or c.get("nom") or ("Recherche personnalisée M" + "+M".join(str(x) for x in positions))
        return _norm_item({
            "kind":"custom",
            "name":name,
            "positions":positions,
            "fig_code":c.get("fig_code"),
            "stricte":c.get("stricte", True),
            "rare":c.get("rare", False),
            "search_type":"auto",
        })

    def _all_combo_items(self):
        """Ce qui doit apparaître dans l'onglet COMBINAISONS :
        1) combos préenregistrés (sauf ceux effacés), 2) recherches avancées, 3) recherches personnalisées.
        """
        hidden = _hidden_predefs()
        out = []
        for p in PREDEF_COMBOS:
            if _key(p) not in hidden:
                out.append(_norm_item(p))
        for x in _combo_items():
            if x.get("kind") != "predef" and not any(_key(x) == _key(y) for y in out):
                out.append(x)
        # ajoute les anciennes recherches personnalisées dans COMBINAISONS si elles existent
        try:
            for c in getattr(self, "custom_searches", []) or read_json(CUSTOM_FILE, []):
                ci = _custom_to_combo(c)
                if not any(_key(ci) == _key(y) for y in out):
                    out.append(ci)
        except Exception:
            pass
        return out
    ...
    def _title(item):
        item = _norm_item(item)
        if item.get("kind") == "predef":
            return item.get("name", "Combo préenregistré")
        fig = item.get("fig_code")
        fig_txt = FIGURES.get(fig, {}).get("africain") if fig else "Toutes figures"
        pos = item.get("positions") or []
        pos_txt = "M" + "+M".join(str(p) for p in pos) if pos else "Toutes maisons"
        mode = "STRICTE" if item.get("stricte", True) else "NON STRICTE"
        nom = item.get("name") or (fig_txt + " | " + pos_txt)
        return nom + " | " + mode + " | AUTO"
                    ...
                    for m4 in TOUTES_LES_FIGURES:
                        ...
                        if positions:
                            ...
                            if target_bits is not None:
                                ...
                                # Recherche globale : on retourne tous les thèmes, sans enregistrer les solutions.
                                fig_bits = h[15]
                                all_pos = [15]
                                ok = True
                        if ok and rare:
    ...
    def _open_results(self, item):
        ...
        def add_rows(count=None):
            ...
            for idx in range(start, end):
                ...
                pos_txt = "M" + "M".join(str(p) for p in pos) if pos else "global"
                txt = f"{idx+1}. {fig} | {pos_txt}\nM1={sol['m1']} M2={sol['m2']} M3={sol['m3']} M4={sol['m4']}"
                bopen = Button(text=txt, font_size=dp(9.5), halign="left", size_hint_y=None, height=dp(56), background_color=(0.10,0.30,0.50,1))
                bopen.bind(on_release=lambda x, s=sol, sl=sols: (pop.dismiss(), self.apply_solution_from_list(s, sl, None)))
                grid.add_widget(bopen)
            state["shown"] = end
            more.disabled = end >= len(sols)
            more.text = "TOUT AFFICHÉ" if more.disabled else "AFFICHER PLUS"
    ...
    def popup_recherche_avancee_final(self):
        selected_fig = [None]
        box = BoxLayout(orientation="vertical", spacing=dp(6), padding=dp(8))
        box.add_widget(Label(text="[b]RECHERCHE AVANCÉE[/b]\nLa combinaison est enregistrée AVANT calcul dans COMBINAISONS.", markup=True, color=(1,1,1,1), size_hint=(1,None), height=dp(58)))
        box.add_widget(Label(text="1. Figure facultative", color=(0.9,0.9,0.9,1), size_hint=(1,None), height=dp(26)))
        fig_grid = GridLayout(cols=2, spacing=dp(4), size_hint=(1,None)); fig_grid.bind(minimum_height=fig_grid.setter("height"))
        allb = ToggleButton(text="TOUTES LES FIGURES", group="adv_fig_last", state="down", size_hint=(1,None), height=dp(38), background_color=(0.10,0.10,0.10,1))
        allb.bind(on_press=lambda x: selected_fig.__setitem__(0, None)); fig_grid.add_widget(allb)
        for code, data in FIGURES.items():
            tb = ToggleButton(text=f"{data['africain']} / {data['occidental']}", group="adv_fig_last", size_hint=(1,None), height=dp(36), font_size=dp(10))
            tb.bind(on_press=lambda x, c=code: selected_fig.__setitem__(0, c)); fig_grid.add_widget(tb)
        svf = ScrollView(size_hint=(1,None), height=dp(145)); svf.add_widget(fig_grid); box.add_widget(svf)
        box.add_widget(Label(text="2. Maisons facultatives", color=(0.9,0.9,0.9,1), size_hint=(1,None), height=dp(26)))
        mg = GridLayout(cols=4, spacing=dp(4), size_hint=(1,None), height=dp(150)); maison_buttons = {}
        ...
        box.add_widget(Label(text="3. Mode", color=(0.9,0.9,0.9,1), size_hint=(1,None), height=dp(24)))
        mr = BoxLayout(orientation="horizontal", spacing=dp(5), size_hint=(1,None), height=dp(40))
        bs = ToggleButton(text="STRICTE", group="adv_mode_last", state="down", background_color=(0.10,0.50,0.70,1))
        bn = ToggleButton(text="NON STRICTE", group="adv_mode_last", background_color=(0.35,0.35,0.35,1))
        mr.add_widget(bs); mr.add_widget(bn); box.add_widget(mr)
        box.add_widget(Label(text="STRICTE = uniquement ces maisons. NON STRICTE = ces maisons + possible ailleurs.", color=(0.75,0.75,0.75,1), size_hint=(1,None), height=dp(44), font_size=dp(10)))
        bc = Button(text="ENREGISTRER DANS COMBINAISONS + CALCULER", size_hint=(1,None), height=dp(48), background_color=(0.0,0.45,0.18,1))
        bf = Button(text="FERMER", size_hint=(1,None), height=dp(42), background_color=(0.25,0.25,0.25,1))
        box.add_widget(bc); box.add_widget(bf)
        pop = Popup(title="Recherche avancée", content=box, size_hint=(0.97,0.97))
        bf.bind(on_release=lambda x: pop.dismiss())
    ...
    def popup_delete_advanced_final(self):
        data = [x for x in _combo_items() if x.get("kind") == "advanced"]
        box = BoxLayout(orientation="vertical", padding=dp(8), spacing=dp(6))
        if not data:
            box.add_widget(Label(text="Aucune recherche avancée sauvegardée.", color=(1,1,1,1), size_hint_y=None, height=dp(70)))
        else:
            box.add_widget(Label(text="Clique une recherche avancée pour la supprimer.", color=(1,1,1,1), size_hint_y=None, height=dp(40)))
            for it in data:
                b = Button(text=_title(it), size_hint_y=None, height=dp(46), background_color=(0.45,0.05,0.05,1))
                b.bind(on_release=lambda x,item=it: (_remove_combo(item), p.dismiss(), self.message("Supprimé", "Recherche avancée supprimée.")))
                box.add_widget(b)
        close = Button(text="FERMER", size_hint_y=None, height=dp(42)); box.add_widget(close)
        p = Popup(title="Supprimer recherche avancée", content=box, size_hint=(0.92,0.75)); close.bind(on_release=lambda x:p.dismiss()); p.open()

    def popup_solutions_clean(self):
        main = BoxLayout(orientation="vertical", spacing=dp(5), padding=dp(6))
        tabs = BoxLayout(orientation="horizontal", spacing=dp(4), size_hint=(1,None), height=dp(44))
        t_rech = ToggleButton(text="RECHERCHES", group="soltabs_clean", state="down", background_color=(0.10,0.55,0.75,1))
        t_save = ToggleButton(text="SAUVEGARDE", group="soltabs_clean")
        t_combo = ToggleButton(text="COMBINAISONS", group="soltabs_clean")
        tabs.add_widget(t_rech); tabs.add_widget(t_save); tabs.add_widget(t_combo); main.add_widget(tabs)
        zone = BoxLayout(size_hint=(1,1)); main.add_widget(zone)
        close = Button(text="FERMER", size_hint=(1,None), height=dp(44), background_color=(0.1,0.1,0.1,1)); main.add_widget(close)
        pop = Popup(title="SOLUTIONS", content=main, size_hint=(0.95,0.95))
        ...
        def make_rech():
            ...
            b_adv = Button(text="🔎 RECHERCHE AVANCÉE", size_hint_y=None, height=dp(56), background_color=(0.0,0.38,0.28,1))
            b_adv.bind(on_release=lambda x: popup_recherche_avancee_final(self))
            box.add_widget(b_adv)
            box.add_widget(Label(text="Les combinaisons sauvegardées se gèrent uniquement dans COMBINAISONS.", color=(0.75,0.75,0.75,1), size_hint_y=None, height=dp(60)))
            box.add_widget(Label(size_hint_y=1))
            return box
        ...
        def make_save():
            ...
            for idx,it in enumerate(saved,1):
                txt=f"{idx}. {it.get('figure_nom','?')} {it.get('positions_str','')} [{it.get('mode','')}]\nM1={it.get('m1','?')} M2={it.get('m2','?')} M3={it.get('m3','?')} M4={it.get('m4','?')}"
                row=BoxLayout(orientation="horizontal", spacing=dp(3), size_hint_y=None, height=dp(58))
                b=Button(text=txt, font_size=dp(9.5), size_hint=(0.75,1)); bg=Button(text="GO", size_hint=(0.13,1), background_color=(0.1,0.35,0.7,1)); bd=Button(text="X", size_hint=(0.12,1), background_color=(0.65,0.08,0.08,1))
                def go(btn,item=it): pop.dismiss(); self.apply_solution({"m1":item["m1"],"m2":item["m2"],"m3":item["m3"],"m4":item["m4"],"figure":item.get("figure_bits","1111"),"positions":[],"secondary":""})
                def dele(btn,item=it): write_json(SAVED_FILE,[x for x in read_json(SAVED_FILE,[]) if x!=item]); switch(None)
                b.bind(on_release=go); bg.bind(on_release=go); bd.bind(on_release=dele); row.add_widget(b); row.add_widget(bg); row.add_widget(bd); grid.add_widget(row)
            return sv
        ...
        def make_combo():
            sv = ScrollView(); grid = GridLayout(cols=1, spacing=dp(5), size_hint_y=None); grid.bind(minimum_height=grid.setter("height")); sv.add_widget(grid)
            combos = _all_combo_items(self)
            if not combos:
                grid.add_widget(Label(text="Aucune combinaison.", color=(0.7,0.7,0.7,1), size_hint_y=None, height=dp(90))); return sv
            grid.add_widget(Label(text="COMBINAISONS = critères de recherche sauvegardés, pas solutions développées", color=(0.9,0.9,0.9,1), size_hint_y=None, height=dp(38)))
            for idx,it in enumerate(combos,1):
                row = BoxLayout(orientation="horizontal", spacing=dp(3), size_hint_y=None, height=dp(62))
                b = Button(text=str(idx)+". "+_title(it), font_size=dp(10), size_hint=(0.70,1), background_color=(0.18,0.28,0.45,1))
                bg = Button(text="OUVRIR", size_hint=(0.16,1), background_color=(0.1,0.45,0.3,1), font_size=dp(9))
    ...
    MoneyRoot.popup_solutions = popup_solutions_clean
    MoneyRoot.popup_recherche_avancee = popup_recherche_avancee_final
    MoneyRoot.popup_search_repetition = popup_recherche_avancee_final
    MoneyRoot.popup_repetition_maisons = popup_recherche_avancee_final
    MoneyRoot.popup_search_by_figure = popup_recherche_avancee_final

_geostar_menu_combos_criteres_final_patch()



# ============================================================
# PATCH DEFINITIF — COMBINAISONS AVEC ONGLET PRE-ENREGISTRE / MES COMBOS
# - Les combos pre-enregistres sont visibles dans COMBINAISONS et NON supprimables
# - Les recherches avancees creees vont dans MES COMBOS et sont supprimables
# - La suppression nettoie aussi les anciens fichiers qui faisaient reapparaitre "Recherche perso"
# - Admin non touche
# ============================================================
def _geostar_combo_grouped_delete_final_patch():
    import os as _os, json as _json
    from kivy.uix.boxlayout import BoxLayout
    from kivy.uix.gridlayout import GridLayout
    from kivy.uix.scrollview import ScrollView
    ...
    BASE = _base_dir()
    COMBO_FILE = _os.path.join(BASE, "geostar_combos_criteres.json")
    OLD_COMBO_FILE = _os.path.join(BASE, "geostar_solution_combos.json")
    ADV_SEARCH_FILE = _os.path.join(BASE, "geostar_advanced_searches.json")
    SAVED_FILE = _get_saved_file()
    ...
    def _norm(item):
        item = dict(item or {})
        try:
            item["positions"] = sorted([int(x) for x in (item.get("positions") or []) if 1 <= int(x) <= 16])
        except Exception:
            item["positions"] = []
        # convertir anciens noms de cle
        if item.get("target_code") and not item.get("fig_code"):
            item["fig_code"] = item.get("target_code")
        item.setdefault("fig_code", None)
        item.setdefault("stricte", True)
        item.setdefault("rare", False)
        item.setdefault("search_type", "auto")
        if not item.get("name"):
            item["name"] = "Recherche perso"
        return item
    ...
    def _same(a, b):
        ...
        if _key(a) == _key(b):
            return True
        # si meme criteres, meme si ancien nom different
        if (a.get("fig_code") == b.get("fig_code") and
            (a.get("positions") or []) == (b.get("positions") or []) and
            bool(a.get("stricte", True)) == bool(b.get("stricte", True)) and
            bool(a.get("rare", False)) == bool(b.get("rare", False))):
            return True
        # vieux bug : Recherche perso sans criteres qui revient
        an = str(a.get("name", "")).strip().lower()
        bn = str(b.get("name", "")).strip().lower()
        if an == bn and an in ("recherche perso", "recherche personnalisée", "recherche personnalisee"):
            return True
        return False
    ...
    def _custom_to_combo(c):
        ...
        return _norm({
            "kind":"custom",
            "name": c.get("name") or c.get("nom") or "Recherche perso",
            "positions": positions,
            "fig_code": c.get("fig_code") or c.get("target_code"),
            "stricte": c.get("stricte", True),
            "rare": c.get("rare", False),
            "search_type":"auto",
        })
    ...
    def _title(item):
        ...
        mode = "STRICTE" if item.get("stricte", True) else "NON STRICTE"
        nom = item.get("name") or (fig_txt + " | " + pos_txt)
        # Evite l'ancien libelle vide qui ne dit rien
        if nom.strip().lower() in ("recherche perso", "recherche personnalisée", "recherche personnalisee"):
            nom = fig_txt + " | " + pos_txt
        return nom + " | " + mode + " | AUTO"
    ...
    def _user_combos():
        out = []
        # nouveau fichier
        for x in _items(COMBO_FILE):
            nx = _norm(x)
            if nx.get("kind") != "predef" and not any(_same(nx, y) for y in out):
                out.append(nx)
        # anciens fichiers qui pouvaient faire reapparaitre les combos
        for path in (OLD_COMBO_FILE, ADV_SEARCH_FILE):
            for x in _items(path):
                nx = _norm(x)
                if nx.get("kind") != "predef" and not any(_same(nx, y) for y in out):
                    out.append(nx)
        # anciennes recherches personnalisees
        try:
            custom_list = getattr(MoneyRoot, "custom_searches", None)
        except Exception:
            custom_list = None
        for c in read_json(CUSTOM_FILE, []):
            ci = _custom_to_combo(c)
            if not any(_same(ci, y) for y in out):
                out.append(ci)
        return out

    def _add_user_combo(item):
        item = _norm(dict(item or {}))
        item["kind"] = "advanced"
        data = [_norm(x) for x in _items(COMBO_FILE) if _norm(x).get("kind") != "predef"]
        if not any(_same(item, x) for x in data):
            data.append(item)
        _save(COMBO_FILE, data)
        return item

    def _remove_user_combo(item, owner=None):
        item = _norm(item)
        # Nettoie tous les emplacements possibles qui peuvent le faire revenir
        for path in (COMBO_FILE, OLD_COMBO_FILE, ADV_SEARCH_FILE):
            cur = [_norm(x) for x in _items(path)]
            kept = [x for x in cur if not _same(x, item)]
            _save(path, kept)
        # Nettoie aussi les anciennes recherches personnalisees
        try:
            cur = read_json(CUSTOM_FILE, [])
            kept = []
            for c in cur if isinstance(cur, list) else []:
                ci = _custom_to_combo(c)
                if not _same(ci, item):
                    kept.append(c)
            write_json(CUSTOM_FILE, kept)
            if owner is not None and hasattr(owner, "custom_searches"):
                owner.custom_searches = kept
                ...
                for m3 in TOUTES_LES_FIGURES:
                    for m4 in TOUTES_LES_FIGURES:
                        ...
                        if positions:
                            ...
                            if target_bits is not None:
                                all_pos = [i for i in range(1, 17) if h[i] == target_bits]
                                ok = len(all_pos) > 0; fig_bits = target_bits
                            else:
                                fig_bits = h[15]; all_pos = [15]; ok = True
                        if ok and rare:
                            ok = (h[7] == h[13] or h[7] == h[15])
                        if not ok: continue
                        key = (m1, m2, m3, m4, fig_bits, tuple(all_pos))
    ...
    def _open_results(self, item):
        ...
        def calculate(dt):
            try:
                ...
                more.disabled = True
        more.bind(on_release=lambda x: add_rows())
        pop.open(); Clock.schedule_once(calculate, 0.05)

    def popup_recherche_avancee_final(self):
        selected_fig = [None]
        box = BoxLayout(orientation="vertical", spacing=dp(6), padding=dp(8))
        box.add_widget(Label(text="[b]RECHERCHE AVANCÉE[/b]\nLa combinaison est enregistrée dans MES COMBOS avant calcul.", markup=True, color=(1,1,1,1), size_hint=(1,None), height=dp(58)))
        box.add_widget(Label(text="1. Figure facultative", color=(0.9,0.9,0.9,1), size_hint=(1,None), height=dp(26)))
        fig_grid = GridLayout(cols=2, spacing=dp(4), size_hint=(1,None)); fig_grid.bind(minimum_height=fig_grid.setter("height"))
        allb = ToggleButton(text="TOUTES LES FIGURES", group="adv_fig_grouped", state="down", size_hint=(1,None), height=dp(38), background_color=(0.10,0.10,0.10,1))
        allb.bind(on_press=lambda x: selected_fig.__setitem__(0, None)); fig_grid.add_widget(allb)
        for code, data in FIGURES.items():
            tb = ToggleButton(text=f"{data['africain']} / {data['occidental']}", group="adv_fig_grouped", size_hint=(1,None), height=dp(36), font_size=dp(10))
            tb.bind(on_press=lambda x, c=code: selected_fig.__setitem__(0, c)); fig_grid.add_widget(tb)
        svf = ScrollView(size_hint=(1,None), height=dp(145)); svf.add_widget(fig_grid); box.add_widget(svf)
        box.add_widget(Label(text="2. Maisons facultatives", color=(0.9,0.9,0.9,1), size_hint=(1,None), height=dp(26)))
        mg = GridLayout(cols=4, spacing=dp(4), size_hint=(1,None), height=dp(150)); maison_buttons = {}
        ...
        box.add_widget(Label(text="3. Mode", color=(0.9,0.9,0.9,1), size_hint=(1,None), height=dp(24)))
        mr = BoxLayout(orientation="horizontal", spacing=dp(5), size_hint=(1,None), height=dp(40))
        bs = ToggleButton(text="STRICTE", group="adv_mode_grouped", state="down", background_color=(0.10,0.50,0.70,1))
        bn = ToggleButton(text="NON STRICTE", group="adv_mode_grouped", background_color=(0.35,0.35,0.35,1))
        mr.add_widget(bs); mr.add_widget(bn); box.add_widget(mr)
        box.add_widget(Label(text="STRICTE = uniquement ces maisons. NON STRICTE = ces maisons + possible ailleurs.", color=(0.75,0.75,0.75,1), size_hint=(1,None), height=dp(44), font_size=dp(10)))
        bc = Button(text="ENREGISTRER DANS MES COMBOS + CALCULER", size_hint=(1,None), height=dp(48), background_color=(0.0,0.45,0.18,1))
        bf = Button(text="FERMER", size_hint=(1,None), height=dp(42), background_color=(0.25,0.25,0.25,1))
        box.add_widget(bc); box.add_widget(bf)
        pop = Popup(title="Recherche avancée", content=box, size_hint=(0.97,0.97)); bf.bind(on_release=lambda x: pop.dismiss())
        def go(_):
            ...
            pos_txt = "M" + "+M".join(str(p) for p in pos) if pos else "Toutes maisons"
            item = {"kind":"advanced", "name":fig_txt + " | " + pos_txt, "fig_code":fig, "positions":pos, "stricte":bool(strict), "search_type":"auto", "rare":False}
            item = _add_user_combo(item)
            pop.dismiss(); _open_results(self, item)
        bc.bind(on_release=go); pop.open()

    def popup_solutions_clean(self):
        main = BoxLayout(orientation="vertical", spacing=dp(5), padding=dp(6))
        tabs = BoxLayout(orientation="horizontal", spacing=dp(4), size_hint=(1,None), height=dp(44))
        t_rech = ToggleButton(text="RECHERCHES", group="soltabs_grouped", state="down", background_color=(0.10,0.55,0.75,1))
        t_save = ToggleButton(text="SAUVEGARDE", group="soltabs_grouped")
        t_combo = ToggleButton(text="COMBINAISONS", group="soltabs_grouped")
        tabs.add_widget(t_rech); tabs.add_widget(t_save); tabs.add_widget(t_combo); main.add_widget(tabs)
        zone = BoxLayout(size_hint=(1,1)); main.add_widget(zone)
        close = Button(text="FERMER", size_hint=(1,None), height=dp(44), background_color=(0.1,0.1,0.1,1)); main.add_widget(close)
        pop = Popup(title="SOLUTIONS", content=main, size_hint=(0.95,0.95))
        ...
        def make_rech():
            ...
            b_adv.bind(on_release=lambda x: popup_recherche_avancee_final(self))
            box.add_widget(b_adv)
            box.add_widget(Label(text="Les combos pré-enregistrés et tes combos sauvegardés sont dans COMBINAISONS.", color=(0.75,0.75,0.75,1), size_hint_y=None, height=dp(60)))
            box.add_widget(Label(size_hint_y=1))
            return box
        ...
        def make_combo():
            outer = BoxLayout(orientation="vertical", spacing=dp(5), padding=dp(2))
            sub = BoxLayout(orientation="horizontal", spacing=dp(4), size_hint=(1,None), height=dp(42))
            t_pre = ToggleButton(text="PRÉENREGISTRÉS", group="combo_subtabs", state="down", background_color=(0.45,0.35,0.05,1))
            t_mes = ToggleButton(text="MES COMBOS", group="combo_subtabs", background_color=(0.15,0.30,0.55,1))
            sub.add_widget(t_pre); sub.add_widget(t_mes); outer.add_widget(sub)
            body = BoxLayout(size_hint=(1,1)); outer.add_widget(body)
            def render(_=None):
                body.clear_widgets()
                sv = ScrollView(); grid = GridLayout(cols=1, spacing=dp(5), size_hint_y=None); grid.bind(minimum_height=grid.setter("height")); sv.add_widget(grid)
                if t_pre.state == "down":
                    grid.add_widget(Label(text="Combos pré-enregistrés — non supprimables", color=(0.9,0.9,0.9,1), size_hint_y=None, height=dp(36)))
                    for it in PREDEF_COMBOS:
                        row = BoxLayout(orientation="horizontal", spacing=dp(3), size_hint_y=None, height=dp(58))
                        b = Button(text=_title(it), font_size=dp(10), size_hint=(0.80,1), background_color=(0.22,0.22,0.22,1))
                        bg = Button(text="OUVRIR", size_hint=(0.20,1), background_color=(0.1,0.45,0.3,1), font_size=dp(9))
                        b.bind(on_release=lambda x,item=it:_open_results(self,item)); bg.bind(on_release=lambda x,item=it:_open_results(self,item))
                        row.add_widget(b); row.add_widget(bg); grid.add_widget(row)
                else:
                    combos = _user_combos()
                    if not combos:
                        grid.add_widget(Label(text="Aucun combo personnel.", color=(0.7,0.7,0.7,1), size_hint_y=None, height=dp(90)))
                    else:
                        grid.add_widget(Label(text="Mes combos — supprimables", color=(0.9,0.9,0.9,1), size_hint_y=None, height=dp(36)))
                        for idx,it in enumerate(combos,1):
                            row = BoxLayout(orientation="horizontal", spacing=dp(3), size_hint_y=None, height=dp(62))
                            b = Button(text=str(idx)+". "+_title(it), font_size=dp(10), size_hint=(0.70,1), background_color=(0.18,0.28,0.45,1))
                            bg = Button(text="OUVRIR", size_hint=(0.16,1), background_color=(0.1,0.45,0.3,1), font_size=dp(9))
                            bd = Button(text="SUPPR.", size_hint=(0.14,1), background_color=(0.65,0.08,0.08,1), font_size=dp(9))
                            b.bind(on_release=lambda x,item=it:_open_results(self,item)); bg.bind(on_release=lambda x,item=it:_open_results(self,item)); bd.bind(on_release=lambda x,item=it:(_remove_user_combo(item, self), render()))
                            row.add_widget(b); row.add_widget(bg); row.add_widget(bd); grid.add_widget(row)
                body.add_widget(sv)
            t_pre.bind(on_press=render); t_mes.bind(on_press=render); render()
            return outer
...
_geostar_combo_grouped_delete_final_patch()



# ============================================================
# PATCH FINAL — fermer toutes les fenêtres SOLUTIONS quand une solution est ouverte
# ============================================================
def _geostar_close_solution_windows_final_patch():
    old_apply_from_list = getattr(MoneyRoot, "apply_solution_from_list", None)
    old_apply_solution = getattr(MoneyRoot, "apply_solution", None)

    def _close_solution_ui(self):
        try:
            if hasattr(self, "close_all_solution_popups"):
                self.close_all_solution_popups()
        except Exception:
            pass

    def apply_solution_from_list_close(self, sol, sol_list, pop=None):
        try:
            if pop is not None:
                pop.dismiss()
        except Exception:
            pass
        _close_solution_ui(self)
        if old_apply_from_list:
            return old_apply_from_list(self, sol, sol_list, None)
        if old_apply_solution:
            self.active_solutions = sol_list
            self.active_solution_index = sol_list.index(sol) if sol in sol_list else 0
            return old_apply_solution(self, sol)

    MoneyRoot.apply_solution_from_list = apply_solution_from_list_close

_geostar_close_solution_windows_final_patch()

# ============================================================
# PATCH FINAL UTILISATEUR — COMBINAISONS PROPRES
# - RECHERCHES : seulement bouton Recherche avancée
# - COMBINAISONS : 2 onglets (PRÉENREGISTRÉS non supprimables / MES COMBOS supprimables)
# - retire le vieux bug "Recherche perso | STRICTE | AUTO" sans critères
# - clic sur une solution ferme les fenêtres pour afficher le thème
# ============================================================
def _geostar_combo_tabs_clean_patch():
    import os as _os, json as _json
    from kivy.uix.boxlayout import BoxLayout
    from kivy.uix.gridlayout import GridLayout
    from kivy.uix.scrollview import ScrollView
    ...
    def _norm(item):
        ...
        try:
            for p in item.get("positions", []) or []:
                n = int(p)
                if 1 <= n <= 16 and n not in pos:
                    pos.append(n)
        except Exception:
            pos = []
        item["positions"] = sorted(pos)
        item["fig_code"] = item.get("fig_code") or item.get("target_code") or None
        item["stricte"] = bool(item.get("stricte", True))
        item["rare"] = bool(item.get("rare", False))
        item["search_type"] = "auto"
        item["name"] = str(item.get("name") or item.get("nom") or "").strip()
        return item

    def _is_empty_bug(item):
        item = _norm(item)
        name = item.get("name", "").strip().lower()
        return (not item.get("positions") and not item.get("fig_code") and name in (
            "recherche perso", "recherche personnalisée", "recherche personnalisee", "perso", ""
        ))
    ...
    def _key(item):
        ...
        keep = {
            ...
            "rare": bool(item.get("rare", False)),
            "search_type": "auto",
            "name": item.get("name") if item.get("kind") == "predef" else "",
            "kind": "predef" if item.get("kind") == "predef" else "user",
        }
        return _json.dumps(keep, ensure_ascii=False, sort_keys=True)
    ...
    def _title(item):
        ...
        mode = "STRICTE" if item.get("stricte", True) else "NON STRICTE"
        name = item.get("name")
        if not name or name.lower() in ("recherche perso", "recherche personnalisée", "recherche personnalisee", "perso"):
            name = fig_txt + " | " + pos_txt
        return name + " | " + mode + " | AUTO"

    def _cleanup_bug_files(owner=None):
        # enlève uniquement le vieux combo vide qui refuse de disparaître
        for path in (COMBO_FILE, OLD_COMBO_FILE, ADV_SEARCH_FILE):
            cur = [_norm(x) for x in _items(path)]
            kept = [x for x in cur if not _is_empty_bug(x)]
            _save(path, kept)
        try:
            cur = read_json(CUSTOM_FILE, [])
            kept = [c for c in (cur if isinstance(cur, list) else []) if not _is_empty_bug(c)]
            write_json(CUSTOM_FILE, kept)
            if owner is not None and hasattr(owner, "custom_searches"):
                owner.custom_searches = kept
        except Exception:
    ...
    def _user_combos(owner=None):
        _cleanup_bug_files(owner)
        out = []
        # nouvelle source + anciennes sources converties, sans doublons
        for path in (COMBO_FILE, OLD_COMBO_FILE, ADV_SEARCH_FILE):
            for x in _items(path):
                nx = _norm(x)
                if nx.get("kind") == "predef" or _is_empty_bug(nx):
                    continue
                nx["kind"] = "user"
                if not any(_same(nx, y) for y in out):
                    out.append(nx)
        return out

    def _save_user_combos(items):
        clean = []
        for x in items:
            nx = _norm(x); nx["kind"] = "user"
            if not _is_empty_bug(nx) and not any(_same(nx, y) for y in clean):
                clean.append(nx)
        _save(COMBO_FILE, clean)
        _save(OLD_COMBO_FILE, [])
        _save(ADV_SEARCH_FILE, [])
        try:
            write_json(CUSTOM_FILE, [])
        except Exception:
            pass

    def _add_user_combo(item, owner=None):
        item = _norm(item); item["kind"] = "user"
        data = _user_combos(owner)
        if not _is_empty_bug(item) and not any(_same(item, x) for x in data):
            data.append(item)
        _save_user_combos(data)
        if owner is not None and hasattr(owner, "custom_searches"):
            owner.custom_searches = []
        return item

    def _remove_user_combo(item, owner=None):
        item = _norm(item)
        data = [x for x in _user_combos(owner) if not _same(x, item)]
        # sécurité spéciale contre l'ancien "Recherche perso" vide
        data = [x for x in data if not _is_empty_bug(x)]
        _save_user_combos(data)
        if owner is not None and hasattr(owner, "custom_searches"):
            owner.custom_searches = []
    ...
    def _open_results(self, item):
        ...
        def add_rows(count=None):
            ...
            for idx in range(start, end):
                ...
                txt = f"{idx+1}. {fig} | {pos_txt}\nM1={sol['m1']} M2={sol['m2']} M3={sol['m3']} M4={sol['m4']}"
                bopen = Button(text=txt, font_size=dp(9.5), halign="left", size_hint_y=None, height=dp(56), background_color=(0.10,0.30,0.50,1))
                def _go(btn, s=sol, sl=sols):
                    try: pop.dismiss()
                    except Exception: pass
                    try: self.close_solution_popups()
                    except Exception: pass
                    self.apply_solution_from_list(s, sl, None)
                bopen.bind(on_release=_go)
                grid.add_widget(bopen)
            state["shown"] = end
            more.disabled = end >= len(sols)
            more.text = "TOUT AFFICHÉ" if more.disabled else "AFFICHER PLUS"
    ...
    def popup_recherche_avancee_clean(self):
        selected_fig = [None]
        box = BoxLayout(orientation="vertical", spacing=dp(6), padding=dp(8))
        box.add_widget(Label(text="[b]RECHERCHE AVANCÉE[/b]\nLe combo est enregistré dans MES COMBOS avant calcul.", markup=True, color=(1,1,1,1), size_hint=(1,None), height=dp(58)))
        box.add_widget(Label(text="1. Figure facultative", color=(0.9,0.9,0.9,1), size_hint=(1,None), height=dp(26)))
        fig_grid = GridLayout(cols=2, spacing=dp(4), size_hint=(1,None)); fig_grid.bind(minimum_height=fig_grid.setter("height"))
        allb = ToggleButton(text="TOUTES LES FIGURES", group="adv_fig_clean", state="down", size_hint=(1,None), height=dp(38), background_color=(0.10,0.10,0.10,1))
        allb.bind(on_press=lambda x: selected_fig.__setitem__(0, None)); fig_grid.add_widget(allb)
        for code, data in FIGURES.items():
            tb = ToggleButton(text=f"{data['africain']} / {data['occidental']}", group="adv_fig_clean", size_hint=(1,None), height=dp(36), font_size=dp(10))
            tb.bind(on_press=lambda x, c=code: selected_fig.__setitem__(0, c)); fig_grid.add_widget(tb)
        svf = ScrollView(size_hint=(1,None), height=dp(145)); svf.add_widget(fig_grid); box.add_widget(svf)
        box.add_widget(Label(text="2. Maisons facultatives", color=(0.9,0.9,0.9,1), size_hint=(1,None), height=dp(26)))
        mg = GridLayout(cols=4, spacing=dp(4), size_hint=(1,None), height=dp(150)); maison_buttons = {}
        ...
        box.add_widget(Label(text="3. Mode", color=(0.9,0.9,0.9,1), size_hint=(1,None), height=dp(24)))
        mr = BoxLayout(orientation="horizontal", spacing=dp(5), size_hint=(1,None), height=dp(40))
        bs = ToggleButton(text="STRICTE", group="adv_mode_clean", state="down", background_color=(0.10,0.50,0.70,1))
        bn = ToggleButton(text="NON STRICTE", group="adv_mode_clean", background_color=(0.35,0.35,0.35,1))
        mr.add_widget(bs); mr.add_widget(bn); box.add_widget(mr)
        box.add_widget(Label(text="STRICTE = uniquement ces maisons. NON STRICTE = ces maisons + possible ailleurs.", color=(0.75,0.75,0.75,1), size_hint=(1,None), height=dp(44), font_size=dp(10)))
        bc = Button(text="ENREGISTRER DANS MES COMBOS + CALCULER", size_hint=(1,None), height=dp(48), background_color=(0.0,0.45,0.18,1))
        bf = Button(text="FERMER", size_hint=(1,None), height=dp(42), background_color=(0.25,0.25,0.25,1))
        ...
        def go(_):
            pos = sorted([i for i,t in maison_buttons.items() if t.state == "down"])
            fig = selected_fig[0]
            item = {"kind":"user", "name":"", "fig_code":fig, "positions":pos, "stricte":(bs.state=="down"), "rare":False, "search_type":"auto"}
            _add_user_combo(item, self)
            pop.dismiss()
            _open_results(self, item)
        bc.bind(on_release=go)
        pop.open()
    ...
    def popup_solutions_clean(self):
        ...
        def make_rech():
            ...
            box.add_widget(Label(size_hint_y=1))
            b = Button(text="🔎 RECHERCHE AVANCÉE", size_hint_y=None, height=dp(60), background_color=(0.0,0.38,0.28,1))
            b.bind(on_release=lambda x: popup_recherche_avancee_clean(self))
            box.add_widget(b)
            box.add_widget(Label(text="Les combos sont enregistrés uniquement dans COMBINAISONS.", color=(0.75,0.75,0.75,1), size_hint_y=None, height=dp(45)))
            box.add_widget(Label(size_hint_y=1))
            return box
        ...
        def make_save():
            ...
            for idx,it in enumerate(saved,1):
                ...
                def go(btn,item=it):
                    pop.dismiss()
                    self.apply_solution({"m1":item["m1"],"m2":item["m2"],"m3":item["m3"],"m4":item["m4"],"figure":item.get("figure_bits","1111"),"positions":[],"secondary":""})
                def dele(btn,item=it):
                    write_json(SAVED_FILE,[x for x in read_json(SAVED_FILE,[]) if x!=item]); switch(None)
                b.bind(on_release=go); bg.bind(on_release=go); bd.bind(on_release=dele)
                row.add_widget(b); row.add_widget(bg); row.add_widget(bd); grid.add_widget(row)
            return sv
        ...
        def make_combo():
            ...
            sub = BoxLayout(orientation="horizontal", spacing=dp(4), size_hint=(1,None), height=dp(42))
            t_pre = ToggleButton(text="PRÉENREGISTRÉS", group="combosub", state="down", background_color=(0.12,0.35,0.55,1))
            t_me = ToggleButton(text="MES COMBOS", group="combosub", background_color=(0.22,0.22,0.22,1))
            sub.add_widget(t_pre); sub.add_widget(t_me); outer.add_widget(sub)
            content = BoxLayout(size_hint=(1,1)); outer.add_widget(content)
            def show_sub(_=None):
                content.clear_widgets()
                sv = ScrollView(); grid = GridLayout(cols=1, spacing=dp(5), size_hint_y=None); grid.bind(minimum_height=grid.setter("height")); sv.add_widget(grid)
                if t_pre.state == "down":
                    grid.add_widget(Label(text="Combos préenregistrés — non supprimables", color=(0.9,0.9,0.9,1), size_hint_y=None, height=dp(32)))
                    for it in PREDEF_COMBOS:
                        row = BoxLayout(orientation="horizontal", spacing=dp(3), size_hint_y=None, height=dp(56))
                        b = Button(text=_title(it), size_hint=(0.82,1), font_size=dp(10), background_color=(0.18,0.18,0.18,1))
                        go = Button(text="OUVRIR", size_hint=(0.18,1), font_size=dp(9), background_color=(0.1,0.45,0.3,1))
                        b.bind(on_release=lambda x,item=it:_open_results(self,item)); go.bind(on_release=lambda x,item=it:_open_results(self,item))
                        row.add_widget(b); row.add_widget(go); grid.add_widget(row)
                else:
                    data = _user_combos(self)
                    if not data:
                        grid.add_widget(Label(text="Aucun combo personnel.", color=(0.7,0.7,0.7,1), size_hint_y=None, height=dp(70)))
                    else:
                        grid.add_widget(Label(text="Mes combos — supprimables", color=(0.9,0.9,0.9,1), size_hint_y=None, height=dp(32)))
                    for idx,it in enumerate(data,1):
                        row = BoxLayout(orientation="horizontal", spacing=dp(3), size_hint_y=None, height=dp(58))
                        b = Button(text=str(idx)+". "+_title(it), font_size=dp(9.5), size_hint=(0.68,1), background_color=(0.15,0.25,0.42,1))
                        go = Button(text="OUVRIR", size_hint=(0.16,1), font_size=dp(9), background_color=(0.1,0.45,0.3,1))
                        bd = Button(text="SUPPR.", size_hint=(0.16,1), font_size=dp(9), background_color=(0.65,0.08,0.08,1))
                        b.bind(on_release=lambda x,item=it:_open_results(self,item)); go.bind(on_release=lambda x,item=it:_open_results(self,item)); bd.bind(on_release=lambda x,item=it:(_remove_user_combo(item,self), show_sub()))
                        row.add_widget(b); row.add_widget(go); row.add_widget(bd); grid.add_widget(row)
                content.add_widget(sv)
            t_pre.bind(on_press=show_sub); t_me.bind(on_press=show_sub); show_sub()
            return outer
    ...
    MoneyRoot.popup_recherche_avancee = popup_recherche_avancee_clean
    MoneyRoot.popup_search_repetition = popup_recherche_avancee_clean
    MoneyRoot.popup_repetition_maisons = popup_recherche_avancee_clean
    MoneyRoot.popup_search_by_figure = popup_recherche_avancee_clean
    MoneyRoot.popup_solutions = popup_solutions_clean

_geostar_combo_tabs_clean_patch()

# ============================================================
# PATCH NETTOYAGE CIBLE — retire le combo bloque M2+M3+M5
# ============================================================
def _geostar_remove_blocked_combo_m235_patch():
    import os as _os, json as _json
    from kivy.clock import Clock
    ...
    BASE = _base_dir()
    FILES = [
        _os.path.join(BASE, "geostar_combos_criteres.json"),
        _os.path.join(BASE, "geostar_solution_combos.json"),
        _os.path.join(BASE, "geostar_advanced_searches.json"),
        CUSTOM_FILE,
    ]

    def _norm_pos(item):
        try:
            return sorted([int(x) for x in (item.get("positions") or []) if 1 <= int(x) <= 16])
        except Exception:
            return []

    def _is_blocked_m235(item):
        item = dict(item or {})
        pos = _norm_pos(item)
        fig = item.get("fig_code") or item.get("target_code") or None
        strict = bool(item.get("stricte", True))
        name = str(item.get("name") or item.get("nom") or "").strip().lower()
        return pos == [2, 3, 5] and fig in (None, "", "None") and strict and (
            name in ("", "recherche perso", "recherche personnalisée", "recherche personnalisee")
            or "toutes figures" in name
            or "m2" in name
        )
        ...
        for path in FILES:
            try:
                ...
                if not isinstance(data, list):
                    continue
                kept = [x for x in data if not _is_blocked_m235(x)]
                if len(kept) != len(data):
                    write_json(path, kept)
            except Exception:
                pass

    old_popup = getattr(MoneyRoot, "popup_solutions", None)
    def popup_solutions_clean_m235(self, *args, **kwargs):
        _clean_files()
        try:
            self.custom_searches = [x for x in getattr(self, "custom_searches", []) if not _is_blocked_m235(x)]
        except Exception:
            pass
        return old_popup(self, *args, **kwargs)

    MoneyRoot.popup_solutions = popup_solutions_clean_m235
    try:
        Clock.schedule_once(lambda dt: _clean_files(), 0.2)
    except Exception:
        _clean_files()

_geostar_remove_blocked_combo_m235_patch()


# ============================================================
# PATCH ULTIME — nettoyage de tous les vieux combos bloqués avant lancement
# ============================================================
def _geostar_force_clean_old_blocked_combos():
    import os as _os
    def _base_dir():
        try:
            from kivy.utils import platform as _p
    ...
    files = [
        ...
        _os.path.join(base, "geostar_solution_combos.json"),
        _os.path.join(base, "geostar_advanced_searches.json"),
        _os.path.join(base, "money_recherches_perso.json"),
    ]
    def is_bad(item):
        try:
            d = dict(item or {})
            pos = sorted([int(x) for x in (d.get("positions") or []) if 1 <= int(x) <= 16])
            fig = d.get("fig_code") or d.get("target_code")
            name = str(d.get("name") or d.get("nom") or "").strip().lower()
            # supprime le combo bloqué M2+M3+M5 et les anciens combos vides Recherche perso
            if pos == [2, 3, 5] and fig in (None, "", "None"):
                return True
            if not pos and fig in (None, "", "None") and name in ("", "recherche perso", "recherche personnalisée", "recherche personnalisee", "perso"):
                return True
        except Exception:
            return False
        return False
    for path in files:
        try:
            ...
            if isinstance(data, list):
                kept = [x for x in data if not is_bad(x)]
                write_json(path, kept)
        except Exception:
            pass

_geostar_force_clean_old_blocked_combos()

# ============================================================
# PATCH FINAL - FERMER TOUTES LES FENETRES QUAND UNE SOLUTION EST CHOISIE
# ============================================================
# Objectif : quand l'utilisateur clique sur une solution, toutes les fenêtres
# du menu SOLUTIONS se ferment vraiment avant l'affichage du thème.
# ============================================================
try:
    from kivy.uix.popup import Popup as _GeoPopup
    _GEOSTAR_OPEN_POPUPS = []
    _geo_popup_open_original = _GeoPopup.open
    _geo_popup_dismiss_original = _GeoPopup.dismiss

    def _geo_popup_open_patched(self, *args, **kwargs):
        try:
            if self not in _GEOSTAR_OPEN_POPUPS:
                _GEOSTAR_OPEN_POPUPS.append(self)
        except Exception:
            pass
        return _geo_popup_open_original(self, *args, **kwargs)

    def _geo_popup_dismiss_patched(self, *args, **kwargs):
        try:
            if self in _GEOSTAR_OPEN_POPUPS:
                _GEOSTAR_OPEN_POPUPS.remove(self)
        except Exception:
            pass
        return _geo_popup_dismiss_original(self, *args, **kwargs)

    _GeoPopup.open = _geo_popup_open_patched
    _GeoPopup.dismiss = _geo_popup_dismiss_patched

    def _geostar_close_every_popup_now():
        for _p in list(_GEOSTAR_OPEN_POPUPS):
            try:
                _p.dismiss()
            except Exception:
                pass
    ...
    def _close_all_solution_popups_final(self):
        try:
            if hasattr(self, '_solution_popups'):
                for _p in list(self._solution_popups):
                    try:
                        _p.dismiss()
                    except Exception:
                        pass
            ...
            pass
        _geostar_close_every_popup_now()

    def _apply_solution_final(self, sol):
        _close_all_solution_popups_final(self)
        self.active_notes = []
        self.active_note_index = -1
        self.mother_codes = [sol['m1'], sol['m2'], sol['m3'], sol['m4']]
        self.afficher_theme(developper_theme(*self.mother_codes))

    def _apply_solution_from_list_final(self, sol, sol_list, pop=None):
        self.active_solutions = sol_list or []
        try:
            self.active_solution_index = self.active_solutions.index(sol)
        except Exception:
            self.active_solution_index = 0
        _apply_solution_final(self, sol)

    MoneyRoot.close_all_solution_popups = _close_all_solution_popups_final
    MoneyRoot.apply_solution = _apply_solution_final
    MoneyRoot.apply_solution_from_list = _apply_solution_from_list_final
except Exception:
    pass
