...
# ============================================================
# PATCH GEOSTAR — VRAI ENREGISTREUR AUDIO ANDROID + SUPABASE
# ============================================================

def _geostar_real_audio_note_patch():
    ...
    def _headers(prefer="return=representation"):
        return {
            ...
            "Content-Type": "application/json",
            "Prefer": prefer,
            "User-Agent": "GEOSTAR-AUDIO/1.0"
        }

    def _http_json(method, url, payload=None, prefer="return=representation"):
        data = json.dumps(payload).encode("utf-8") if payload is not None else None
    ...
    def _fetch_notes(theme):
        url = SUPABASE_URL + "/rest/v1/notes_communaute?select=*&statut=eq.valide&theme_key=eq." + urllib.parse.quote(str(theme)) + "&order=date_validation.desc"
        data = _http_json("GET", url)
        return data if isinstance(data, list) else []

    def _send_note(theme, auteur, texte, audio_url):
        payload = {
            "theme_key": str(theme),
            "auteur": auteur or "Anonyme",
            "code_licence": "",
        ...
        return _http_json("POST", SUPABASE_URL + "/rest/v1/notes_communaute", payload, prefer="return=minimal")

    def _ask_delete(note_id):
        payload = {"statut": "demande_suppression", "date_validation": datetime.now().isoformat()}
        url = SUPABASE_URL + "/rest/v1/notes_communaute?id=eq." + urllib.parse.quote(str(note_id))
        return _http_json("PATCH", url, payload, prefer="return=minimal")

    def _upload_audio(file_path, theme):
        # Upload direct Supabase Storage.
        name = os.path.basename(file_path)
        object_path = str(theme).replace("/", "_") + "/" + name
    ...
    def popup_notes_audio(self):
        theme = _theme_key(self)
        recorder = AndroidRecorder()
        audio_state = {"path": "", "url": ""}
        ...
        b_refresh = Button(text="ACTUALISER", background_color=(0.05, 0.25, 0.50, 1))
        b_add = Button(text="AJOUTER NOTE / VOCAL", background_color=(0, 0.42, 0.10, 1))
        top.add_widget(b_refresh)
        top.add_widget(b_add)
        main.add_widget(top)
            ...
            def worker():
                ...
                def done(dt):
                    ...
                    for n in notes:
                        ...
                        card = BoxLayout(orientation="vertical", padding=dp(7), spacing=dp(3), size_hint_y=None, height=dp(190))
                        card.add_widget(Label(text="[b]" + auteur + "[/b]  " + date, markup=True, color=(1, 0.82, 0.1, 1), size_hint=(1, None), height=dp(26)))
                        card.add_widget(Label(text=texte[:300], color=(1, 1, 1, 1), size_hint=(1, None), height=dp(62)))

                        if audio:
                            card.add_widget(Label(text="VOCAL DISPONIBLE : " + audio[:95], color=(0.75, 0.85, 1, 1), size_hint=(1, None), height=dp(34)))

                        bdel = Button(text="DEMANDER SUPPRESSION", size_hint=(1, None), height=dp(38), background_color=(0.45, 0.10, 0.10, 1))
                        bdel.bind(on_release=lambda x, nid=note_id: ask_delete(nid))
                        card.add_widget(bdel)
        ...
        def ask_delete(nid):
            try:
                _ask_delete(nid)
                _msg(self, "SUPPRESSION", "Demande envoyée.\nUn admin doit accepter ou refuser.")
                refresh()
            except Exception as e:
                _msg(self, "ERREUR", str(e)[:300])
        ...
        def add_note(*_):
            ...
            audio_info = Label(text="Aucun vocal enregistré", color=(0.85, 0.85, 0.85, 1), size_hint=(1, None), height=dp(32))

            row_audio = BoxLayout(size_hint=(1, None), height=dp(46), spacing=dp(6))
            b_start = Button(text="DÉMARRER VOCAL", background_color=(0, 0.35, 0.05, 1))
            b_stop = Button(text="STOP VOCAL", background_color=(0.55, 0.28, 0, 1))
            b_delete = Button(text="SUPPRIMER VOCAL", background_color=(0.45, 0.05, 0.05, 1))
            row_audio.add_widget(b_start)
            row_audio.add_widget(b_stop)
            row_audio.add_widget(b_delete)

            send = Button(text="ENVOYER POUR VALIDATION", size_hint=(1, None), height=dp(46), background_color=(0, 0.42, 0.10, 1))
            cancel = Button(text="FERMER", size_hint=(1, None), height=dp(42), background_color=(0.25, 0.25, 0.25, 1))

            for w in [auteur, texte, audio_info, row_audio, send, cancel]:
                box.add_widget(w)

            p2 = Popup(title="Nouvelle note", content=box, size_hint=(0.92, 0.82))

            def start_rec(*__):
                ok, res = recorder.start()
                if ok:
                    audio_state["path"] = res
                    audio_info.text = "Enregistrement en cours..."
                else:
                    audio_info.text = "Erreur micro : " + str(res)[:120]

            def stop_rec(*__):
                ok, res = recorder.stop()
                if ok:
                    audio_state["path"] = res
                    audio_info.text = "Vocal enregistré : " + os.path.basename(res)
                else:
                    audio_info.text = "Erreur stop : " + str(res)[:120]

            def delete_rec(*__):
                ok, res = recorder.delete()
                audio_state["path"] = ""
                audio_state["url"] = ""
                audio_info.text = "Vocal supprimé" if ok else "Erreur suppression : " + str(res)[:120]

            def do_send(*__):
                if not texte.text.strip() and not audio_state.get("path"):
                    _msg(self, "NOTE", "Écris une note ou enregistre un vocal avant d’envoyer.")
                    return
                ...
                def worker():
                    try:
                        audio_url = ""
                        if audio_state.get("path"):
                            audio_url = _upload_audio(audio_state["path"], theme)
                            audio_state["url"] = audio_url

                        _send_note(theme, auteur.text.strip() or "Anonyme", texte.text.strip(), audio_url)
                        err = ""
                    except Exception as e:
                        err = str(e)[:450]
                    ...
                    def done(dt):
                        ...
                        if err:
                            _msg(self, "ERREUR", err)
                        else:
                            p2.dismiss()
                            _msg(self, "NOTE", "Note envoyée.\nElle apparaîtra après validation admin.")
                    Clock.schedule_once(done, 0)
            ...
            b_delete.bind(on_release=delete_rec)
    ...
    MoneyRoot.popup_notes_current = popup_notes_audio
    MoneyRoot.ouvrir_communaute = lambda self, *args: self.popup_notes_current()

_geostar_real_audio_note_patch()
