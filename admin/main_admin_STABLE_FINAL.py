...
# ============================================================
# PATCH ADMIN STABLE — ALIAS + NOTES PUBLIQUES
# ============================================================

def _admin_stable_notes_patch():
    from kivy.uix.boxlayout import BoxLayout
    from kivy.metrics import dp

    def ui_label(txt, size=13, h=120, color=(1,1,1,1)):
        if "label" in globals():
            return globals()["label"](txt, size=size, h=h, color=color)
        return globals()["make_label"](txt, size=size, h=h, color=color)

    def ui_button(txt, bg, h=44):
        if "button" in globals():
            return globals()["button"](txt, bg=bg, h=h)
        return globals()["make_btn"](txt, bg=bg, h=h)

    def set_status(note_id, st):
        if "note_status" in globals():
            return globals()["note_status"](note_id, st)
        return globals()["update_note_status"](note_id, st)

    def note_card_fixed(self, n, reload):
        ...
        card = BoxLayout(orientation="vertical", padding=dp(8), spacing=dp(4), size_hint_y=None, height=dp(250))
        contenu = "[b]" + auteur + "[/b] | " + statut.upper() + "\nThème : " + theme
        if audio:
            contenu += "\nVocal : " + audio[:90]
        contenu += "\n\n" + texte[:260]
        card.add_widget(ui_label(contenu, size=13, h=145))

        row = BoxLayout(size_hint_y=None, height=dp(44), spacing=dp(6))

        def change(st):
            try:
                set_status(note_id, st)
                reload()
            except Exception as e:
                alert("Erreur", str(e)[:300])

        if statut == "demande_suppression":
            b1 = ui_button("ACCEPTER SUPPR.", (0.50,0.05,0.05,1))
            b2 = ui_button("REFUSER SUPPR.", (0,0.45,0.05,1))
            b1.bind(on_release=lambda x: change("supprime"))
            b2.bind(on_release=lambda x: change("valide"))
            ...
            for txt, st, col in [
                ...
                b = ui_button(txt, col)
                b.bind(on_release=lambda x, s=st: change(s))
                row.add_widget(b)

        card.add_widget(row)
        return card

    AdminApp.note_card = note_card_fixed

_admin_stable_notes_patch()
