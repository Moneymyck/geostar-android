...
def popup_audio_player(audio_url):
    global _CURRENT_SOUND, _CURRENT_AUDIO_FILE
    box = BoxLayout(orientation='vertical', padding=dp(12), spacing=dp(10))
    status = lab('Préparation du vocal...', 14, (1, 1, 1, 1), 70)
        ...
        try:
            if _CURRENT_SOUND:
                try:
                    ...
                    pass
                _CURRENT_SOUND = None
            _CURRENT_AUDIO_FILE = _download_audio_to_cache(audio_url)
            _CURRENT_SOUND = SoundLoader.load(_CURRENT_AUDIO_FILE)
            if _CURRENT_SOUND:
                status.text = 'Vocal chargé. Appuie sur LIRE.'
            else:
                status.text = 'Fichier téléchargé, mais le lecteur Kivy ne reconnaît pas ce format.\nEssaie un enregistrement .mp3, .m4a ou .wav.'
        except Exception as e:
            status.text = 'Impossible de charger le vocal :\n' + str(e)[:220]
    ...
    p.bind(on_dismiss=lambda *a: stop(None))
    p.open()
...
class AdminApp(App):
    ...
    def note_card(self, n, reload):
        ...
        if audio_url:
            ...
            bd = btn('SUPPR. AUDIO', (.5,.05,.05,1), 44)
            ba.bind(on_release=lambda x, u=audio_url: popup_audio_player(u))
            bd.bind(on_release=lambda x, u=audio_url: (delete_audio_from_storage(u), msg('Audio','Suppression audio demandée.'), reload()))
            row_audio.add_widget(ba); row_audio.add_widget(bd); card.add_widget(row_audio)
        row = BoxLayout(size_hint_y=None, height=dp(44), spacing=dp(6))
        for state_txt, col in [('attente',(.55,.28,0,1)), ('valide',(0,.45,.05,1)), ('rejete',(.5,.05,.05,1)), ('supprime',(.18,.18,.18,1))]:
            if state_txt == 'supprime':
                b = btn('SUPPR.', col, 44)
                b.bind(on_release=lambda x, nn=n: (delete_audio_from_storage(str(nn.get('audio_url','') or '')), delete_note_real(nn.get('id')), reload()))
            else:
