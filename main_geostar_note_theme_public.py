...
def _geostar_remote_update_and_validated_notes_patch():
    ...
    def _send_note(auteur, texte, theme_key, langue="fr"):
        payload = {
...
# ============================================================
# PATCH GEOSTAR — NOTE PUBLIQUE PAR THÈME VIA SUPABASE
# ============================================================

def _geostar_note_theme_public_only_patch():
    ...
    def _headers(prefer="return=representation"):
        return {
            ...
            "Content-Type": "application/json",
            "Prefer": prefer,
            "User-Agent": "GEOSTAR-NOTE-THEME/2.0"
        }

    def _http(method, url, payload=None, prefer="return=representation"):
    ...
    def _fetch_public_notes(theme_key):
        url = (
            SUPABASE_URL
            + "/rest/v1/notes_communaute?select=*&statut=eq.valide&theme_key=eq."
            + urllib.parse.quote(str(theme_key))
            + "&order=date_validation.desc"
        )
        data = _http("GET", url)
        return data if isinstance(data, list) else []

    def _send_note(theme_key, auteur, texte, audio_url=""):
        payload = {
        ...
        return _http("POST", SUPABASE_URL + "/rest/v1/notes_communaute", payload=payload, prefer="return=minimal")

    def _ask_delete(note_id):
        payload = {
            "statut": "demande_suppression",
            "date_validation": datetime.now().isoformat()
        }
        url = SUPABASE_URL + "/rest/v1/notes_communaute?id=eq." + urllib.parse.quote(str(note_id))
        return _http("PATCH", url, payload=payload, prefer="return=minimal")

    def _message(self, title, body):
        try:
            self.message(title, body)
        except Exception:
            pop = Popup(title=title, content=Label(text=str(body)), size_hint=(0.85, 0.45))
            pop.open()

    def popup_notes_current_theme_public(self):
        key = _theme_key(self)

        main = BoxLayout(orientation="vertical", padding=dp(8), spacing=dp(7))

        main.add_widget(Label(
            text="[b]NOTES PUBLIQUES DU THÈME[/b]\n" + str(key),
            markup=True,
            color=(1, 1, 1, 1),
            size_hint=(1, None),
            height=dp(58)
        ...
        row_top = BoxLayout(size_hint=(1, None), height=dp(46), spacing=dp(6))
        btn_refresh = Button(text="ACTUALISER", background_color=(0.05, 0.25, 0.50, 1))
        btn_write = Button(text="AJOUTER UNE NOTE", background_color=(0.0, 0.42, 0.10, 1))
        row_top.add_widget(btn_refresh)
        row_top.add_widget(btn_write)
        main.add_widget(row_top)
            ...
            def worker():
                ...
                def done(dt):
                    ...
                    for n in notes:
                        ...
                        card = BoxLayout(orientation="vertical", padding=dp(7), spacing=dp(3), size_hint_y=None, height=dp(155))
                        card.add_widget(Label(
                            text="[b]" + auteur + "[/b]  " + date,
                            markup=True,
                            color=(1, 0.82, 0.1, 1),
                            size_hint=(1, None),
                            height=dp(26)
                        ...
                        def do_delete(_, nid=note_id):
                            try:
                                _ask_delete(nid)
                                _message(self, "SUPPRESSION", "Demande envoyée.\nUn admin doit accepter ou refuser.")
                                refresh_notes()
                            except Exception as e:
                                _message(self, "ERREUR", str(e)[:300])
        ...
        def write_note(*_):
            box = BoxLayout(orientation="vertical", padding=dp(10), spacing=dp(8))
            box.add_widget(Label(text="Ajouter une note publique au thème\n" + str(key), color=(1, 1, 1, 1), size_hint=(1, None), height=dp(52)))
            ...
            audio = TextInput(hint_text="Lien ou mémo vocal optionnel", multiline=False, size_hint=(1, None), height=dp(44))
            btn_send = Button(text="ENVOYER POUR VALIDATION", size_hint=(1, None), height=dp(46), background_color=(0.0, 0.42, 0.10, 1))
            btn_cancel = Button(text="FERMER", size_hint=(1, None), height=dp(42), background_color=(0.25, 0.25, 0.25, 1))
            for w in [auteur, texte, audio, btn_send, btn_cancel]:
                box.add_widget(w)

            p2 = Popup(title="Nouvelle note", content=box, size_hint=(0.92, 0.78))

            def send(*__):
                if not texte.text.strip():
                    _message(self, "NOTE", "Écris une note avant d’envoyer.")
                    return
                ...
                def worker():
                    try:
                        _send_note(key, auteur.text.strip() or "Anonyme", texte.text.strip(), audio.text.strip())
                        err = ""
                    except Exception as e:
                        err = str(e)[:350]
                    ...
                    def done(dt):
                        ...
                        btn_send.text = "ENVOYER POUR VALIDATION"
                        if err:
                            _message(self, "ERREUR", err)
                        else:
                            p2.dismiss()
                            _message(self, "NOTE", "Note envoyée.\nElle apparaîtra après validation admin.")
                    Clock.schedule_once(done, 0)

                threading.Thread(target=worker, daemon=True).start()

            btn_send.bind(on_release=send)
            btn_cancel.bind(on_release=lambda x: p2.dismiss())
            p2.open()

        btn_refresh.bind(on_release=refresh_notes)
        btn_write.bind(on_release=write_note)
        close.bind(on_release=lambda x: pop.dismiss())

        pop.open()
        refresh_notes()

    MoneyRoot.popup_notes_current = popup_notes_current_theme_public

    def ouvrir_communaute_redirect(self, *args):
        return self.popup_notes_current()

    MoneyRoot.ouvrir_communaute = ouvrir_communaute_redirect

    # Si le bouton COMM existe, il ouvre NOTE. On le laisse inutile ou on peut le masquer selon l’ancienne interface.
    try:
        old_build_ui = MoneyRoot.build_ui
        def build_ui_note_only(self):
            old_build_ui(self)
            try:
                for child in list(self.children):
                    if isinstance(child, BoxLayout):
                        for w in list(child.children):
                            if getattr(w, "text", "") == "COMM.":
                                child.remove_widget(w)
            except Exception:
                pass
        MoneyRoot.build_ui = build_ui_note_only
    except Exception:
        pass

_geostar_note_theme_public_only_patch()
