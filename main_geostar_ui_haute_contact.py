...
# ============================================================
# PATCH VERROUILLAGE GEOSTAR — UI HAUTE + CONTACT EMAIL
# ============================================================

GEOSTAR_CODES_URL = "https://raw.githubusercontent.com/Moneymyck/geostar-android/main/codes_geostar.json"

def _geostar_lock_ui_top_patch():
    import os, json, re, ssl, urllib.request, threading
    from datetime import datetime
    from kivy.app import App
    from kivy.uix.screenmanager import ScreenManager, Screen, FadeTransition
    from kivy.uix.floatlayout import FloatLayout
    ...
    CODE_RE = re.compile(r"^GEO-[A-Z0-9]{4}$")
    ...
    def _licence_path():
        return os.path.join(_base_dir(), ".geostar_licence.json")

    def _clear_local_code():
        try:
            os.remove(_licence_path())
        except Exception:
            pass
    ...
    def _fetch_codes():
        ...
        req = urllib.request.Request(
            GEOSTAR_CODES_URL,
            headers={"User-Agent": "GEOSTAR-CLIENT-LOCK/1.0"}
        )
        with urllib.request.urlopen(req, timeout=10, context=ctx) as r:
            return json.loads(r.read().decode("utf-8"))

    def _verify_code_online(code):
        if not CODE_RE.match(code):
            return False, "Format invalide. Exemple : GEO-A7K9", ""
        ...
        if code in bloques:
            return False, "Code bloqué par l'administrateur.", ""

        if code not in valides:
            return False, "Code inconnu ou non synchronisé.", ""

        info = valides.get(code, {})
        expire = info.get("expire", "")

        if expire:
            try:
                if datetime.now() > datetime.strptime(expire, "%Y-%m-%d"):
                    return False, "Code expiré le " + expire, expire
            except Exception:
                pass

        return True, "Code valide", expire

    def _rounded_bg(widget, color, border=None):
        with widget.canvas.before:
            Color(*color)
            widget._bg_rect = RoundedRectangle(pos=widget.pos, size=widget.size, radius=[dp(8)])
            if border:
                Color(*border)
                widget._border = Line(rounded_rectangle=(widget.x, widget.y, widget.width, widget.height, dp(8)), width=1.2)

        def upd(*_):
            widget._bg_rect.pos = widget.pos
            widget._bg_rect.size = widget.size
            if border and hasattr(widget, "_border"):
    ...
    def locked_build(self):
        self.title = "GEOSTAR"
        _clear_local_code()

        sm = ScreenManager(transition=FadeTransition(duration=0.25))
        activation = Screen(name="activation")
        ...
        root.bind(pos=lambda s, v: setattr(s._bg, "pos", v))
        root.bind(size=lambda s, v: setattr(s._bg, "size", v))
        ...
        code_input = TextInput(
            ...
            pos_hint={"x": 0.03, "top": 0.755}
        )
        root.add_widget(code_input)
        ...
        status.bind(width=lambda s, w: setattr(s, "text_size", (w, None)))
        root.add_widget(status)

        btn = Button(
            text="ACTIVER ET ACCÉDER",
            size_hint=(0.94, None),
            height=dp(52),
            bold=True,
            font_size=dp(15),
            background_color=(0.0, 0.20, 0.07, 1),
            pos_hint={"x": 0.03, "top": 0.635}
        )
        root.add_widget(btn)

        info = Label(
            text="[b]Vous n'avez pas de code ?[/b]\nContactez : anthom1253@gmail.com",
            markup=True,
            font_size=dp(13),
            color=(1, 1, 1, 1),
            size_hint=(0.94, None),
            height=dp(58),
            halign="center",
            pos_hint={"x": 0.03, "top": 0.555}
        )
        info.bind(width=lambda s, w: setattr(s, "text_size", (w, None)))
        ...
        mail_btn = Button(
            ...
            pos_hint={"x": 0.03, "top": 0.475}
        )
        root.add_widget(mail_btn)

        important = Label(
            text="Le code est demandé à chaque connexion.",
            font_size=dp(12),
            color=(1, 1, 1, 1),
            size_hint=(0.94, None),
            height=dp(34),
            halign="center",
            pos_hint={"x": 0.03, "top": 0.405}
        )
        important.bind(width=lambda s, w: setattr(s, "text_size", (w, None)))
        root.add_widget(important)

        footer = Label(
            text="GEOSTAR - Sécurisé, fiable, protégé",
            font_size=dp(11),
            color=(1, 1, 1, 0.9),
            size_hint=(1, None),
            height=dp(26),
        ...
        def open_real_app():
            if not sm.has_screen("app"):
                app_screen = Screen(name="app")
                app_screen.add_widget(MoneyRoot())
                sm.add_widget(app_screen)
            sm.current = "app"

        def do_check(_):
            code = code_input.text.strip().upper()
            if code and not code.startswith("GEO-") and len(code) == 4:
                code = "GEO-" + code
                code_input.text = code

            status.text = "Vérification en ligne..."
            status.color = (1, 1, 0.2, 1)
            btn.disabled = True

            def worker():
                try:
                    ok, msg, expire = _verify_code_online(code)
                except Exception:
                    ok, msg, expire = False, "Connexion impossible. Vérifie internet.", ""

                def finish(dt):
                    btn.disabled = False
                    if ok:
                        status.text = "Code valide. Ouverture..."
                        status.color = (0.2, 1, 0.3, 1)
                        Clock.schedule_once(lambda d: open_real_app(), 0.3)
                    else:
                        status.text = msg
                        status.color = (1, 0.25, 0.25, 1)
        ...
        def demander_code(_):
            try:
                if platform == "android":
                    ...
                    Uri = autoclass("android.net.Uri")
                    PythonActivity = autoclass("org.kivy.android.PythonActivity")
                    sujet = "Demande de code GEOSTAR"
                    corps = "Bonjour,%0A%0AJe souhaite obtenir un code d'activation pour GEOSTAR.%0A%0ANom :%0ATelephone :%0ADuree souhaitee : 1 mois / 1 an / a vie%0A%0AMerci."
                    uri = Uri.parse("mailto:anthom1253@gmail.com?subject=" + sujet.replace(" ", "%20") + "&body=" + corps)
                    intent = Intent(Intent.ACTION_SENDTO, uri)
                    PythonActivity.mActivity.startActivity(intent)
                else:
                    status.text = "Contact : anthom1253@gmail.com"
                    status.color = (1, 1, 1, 1)
            except Exception:
                status.text = "Contact : anthom1253@gmail.com"
                status.color = (1, 1, 1, 1)

        btn.bind(on_release=do_check)
        mail_btn.bind(on_release=demander_code)

        activation.add_widget(root)
        sm.add_widget(activation)
        sm.current = "activation"
        return sm

    MoneyApp.build = locked_build

_geostar_lock_ui_top_patch()
