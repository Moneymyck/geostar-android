...
# ============================================================
# MODULE SUPABASE — VALIDATION DES NOTES
# ============================================================
...
SUPABASE_TABLE = "notes_communaute"


def _supa_req(method, endpoint, data=None):
    ...
    body = _js.dumps(data).encode("utf-8") if data else None
...
def supa_get_notes_attente():
    return _supa_req("GET", f"{SUPABASE_TABLE}?statut=eq.attente&order=date_soumission.asc") or []


def supa_valider_note(note_id, valider=True):
    from datetime import datetime
    import urllib.request as _ur, ssl as _ssl, json as _js
    _ctx = _ssl.SSLContext(_ssl.PROTOCOL_TLS_CLIENT)
    _ctx.check_hostname = False
    _ctx.verify_mode = _ssl.CERT_NONE
    data = {"statut": "valide" if valider else "rejete",
             "date_validation": datetime.now().isoformat()}
    url = f"{SUPABASE_URL}/rest/v1/{SUPABASE_TABLE}?id=eq.{note_id}"
    headers = {"apikey": SUPABASE_KEY,
               "Authorization": f"Bearer {SUPABASE_KEY}",
               "Content-Type": "application/json",
               "Prefer": "return=representation"}
    req = _ur.Request(url, data=_js.dumps(data).encode(), method="PATCH", headers=headers)
    try:
        with _ur.urlopen(req, timeout=10, context=_ctx) as resp:
            return _js.loads(resp.read().decode("utf-8"))
    except Exception:
...
class MainScreen(Screen):
    ...
    def _show_notes_attente(self):
        """Popup des notes en attente de validation depuis Supabase."""
        notes = supa_get_notes_attente()
            ...
            for idx, note in enumerate(notes_filtrees):
                ...
                if filtre == "attente":
                    ...
                    def do_valider(_, n=note, i=idx):
                        ...
                        import threading
                        def _val():
                            supa_valider_note(note_id, valider=True)
                            Clock.schedule_once(lambda dt: (
                                show_toast("Note validee !"),
                                log_action(App.get_running_app().current_admin or "admin",
                                          "Validation note", n.get("theme_key","?")),
                                refresh_notes("attente")
                            ), 0)
                        threading.Thread(target=_val, daemon=True).start()

                    def do_rejeter(_, n=note, i=idx):
                        note_id = n.get("id","")
                        import threading
                        def _rej():
                            supa_valider_note(note_id, valider=False)
                            Clock.schedule_once(lambda dt: (
                                show_toast("Note rejetee"),
                                log_action(App.get_running_app().current_admin or "admin",
                                          "Rejet note", n.get("theme_key","?")),
                                refresh_notes("attente")
                            ), 0)
