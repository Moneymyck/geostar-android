...
# ============================================================
# PATCH GEOSTAR — BOUTON FERMER VISIBLE SUR POPUP SOLUTIONS
# ============================================================

def _geostar_solution_close_button_patch():
    from kivy.uix.popup import Popup
    from kivy.uix.button import Button
    from kivy.metrics import dp
    from kivy.clock import Clock
    ...
    def open_with_solution_close(self, *args, **kwargs):
        ...
        try:
            ...
            if title and "SOLUTION" in title.upper():
                ...
                # Eviter doublon
                deja = False
                if content is not None and hasattr(content, "children"):
                    for child in content.children:
                        if getattr(child, "text", "") == "FERMER":
                            deja = True

                if content is not None and hasattr(content, "add_widget") and not deja:
                    btn_close = Button(
                        text="FERMER",
                        size_hint=(1, None),
                        height=dp(52),
                    ...
                    # Ajouter en bas du contenu existant
                    try:
    ...
    Popup.open = open_with_solution_close

_geostar_solution_close_button_patch()
...
def _geostar_verify_at_startup():
    ...
    if remote is not None:
        ...
        valides = remote.get("codes_valides", {})
        # Si codes_valides est vide -> pas encore de codes créés -> forcer activation
        if not valides:
            try:
                os.remove(_geostar_licence_path())
            except Exception:
    ...
    # MAIS seulement si le code a déjà été validé au moins une fois
    derniere = local.get("derniere_verif", "")
    if not derniere:
        # Jamais vérifié en ligne -> forcer activation
        return (False, "first_run")
    try:
...
def _geostar_show_activation_screen(app_root):
    """Affiche l'écran d'activation en remplacant toute l'UI."""
    from kivy.core.window import Window

    # Creer un widget plein ecran qui recouvre tout
    overlay = FloatLayout(size=Window.size)

    # Fond noir opaque
    with overlay.canvas.before:
        from kivy.graphics import Color as GColor, Rectangle as GRect
        GColor(0.05, 0.05, 0.08, 1)
        overlay._bg = GRect(pos=overlay.pos, size=overlay.size)

    overlay.bind(
        pos=lambda s, v: setattr(s._bg, "pos", v),
    ...
    # Marquer l'overlay pour éviter les doublons
    overlay._is_geostar_activation = True
    # Ajouter directement dans Window (pas un Popup - impossible a ignorer)
    ...
    def do_activate(_):
        ...
        status_label.text = "Verification en cours..."
...
def _geostar_force_activation_check():
    """Version robuste : vérifie plusieurs fois si le popup s'est bien affiché."""
    from kivy.uix.popup import Popup
    from kivy.core.window import Window

    # Vérifier si le popup d'activation est déjà ouvert
    for w in Window.children:
        if isinstance(w, Popup) and "Activation" in str(getattr(w, "title", "")):
            return  # Déjà ouvert

    # Sinon relancer la vérification
    _geostar_licence_startup_check()


# Fonction principale de vérification de licence au démarrage
def _geostar_lancer_verification(root):
    """Vérifie la licence et affiche l'écran d'activation si nécessaire.
    Appelée depuis App.build() après création de l'UI."""
    from kivy.core.window import Window

    # Si l'overlay d'activation est déjà affiché, ne rien faire
    for w in list(Window.children):
        if getattr(w, "_is_geostar_activation", False):
            return

    autoriser, message = _geostar_verify_at_startup()
    if autoriser:
        return  # OK, pas besoin d'activation

    # Afficher l'écran d'activation
    _geostar_show_activation_screen(root)

# Hook build_ui (garde la compatibilité)
...
def _geostar_v2_patch_apply():
    ...
    # Stocker référence originale pour apply_from_list_v2
    MoneyRoot._orig_apply_solution = _old_apply_solution

    def apply_solution_v2(self, sol):
        # Fermer immédiatement tous les popups
        _geostar_v2_close_every_popup()
        # Fermer aussi la popup principale SOLUTIONS si elle existe
        pop_main = getattr(self, "_popup_solutions_principale", None)
        if pop_main is not None:
            try:
                pop_main.dismiss()
            except Exception:
                pass
            self._popup_solutions_principale = None
        # Quitter le mode favoris (sans toucher active_solutions)
        self.active_notes = []
        self.active_note_index = -1
        # NE PAS vider active_solutions ici
    ...
    def apply_from_list_v2(self, sol, sol_list, pop=None):
        _geostar_v2_close_every_popup()
        # Sauvegarder la liste AVANT d'appeler apply_solution
        self.active_solutions = sol_list
        try:
            self.active_solution_index = sol_list.index(sol)
        except ValueError:
        ...
        self.active_notes = []
        self.active_note_index = -1
        # Appeler apply_solution_v2 directement (pas via self.apply_solution)
        # pour éviter que clear_active_solutions() soit appelé en chemin
        _old_orig_apply = MoneyRoot.__dict__.get("_orig_apply_solution")
        if _old_orig_apply:
            _old_orig_apply(self, sol)
        else:
            self.mother_codes = [sol["m1"], sol["m2"], sol["m3"], sol["m4"]]
            self.afficher_theme(developper_theme(*self.mother_codes))
        # Re-restaurer active_solutions après afficher_theme (qui peut les vider)
        self.active_solutions = sol_list
        try:
            self.active_solution_index = sol_list.index(sol)
        except ValueError:
