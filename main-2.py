...
# ============================================================
# PATCH V12-FIX-SOLUTIONS — Fermeture forcée des popups
# ============================================================
# Quand on clique sur une solution dans le menu SOLUTIONS,
# tous les popups doivent se fermer pour laisser voir le thème.
# Le code original utilisait Popup._popup_stack qui n'existe pas
# en Kivy moderne, donc rien ne se fermait.
#
# Ce patch ferme TOUS les popups ouverts via Window.children.
# ============================================================

def _geostar_close_all_popups_force():
    """Ferme TOUS les popups Kivy actuellement ouverts."""
    from kivy.uix.popup import Popup
    from kivy.core.window import Window
    try:
        # Window.children contient tous les widgets racine, popups inclus
...
def _geostar_patch_apply_solution_close():
    ...
    def apply_solution_close_popups(self, sol):
        # Étape 1 : fermer TOUS les popups via Window
        _geostar_close_all_popups_force()
        # Étape 2 : fermer via le tracker dédié (au cas où)
        try:
            self.close_all_solution_popups()
        except Exception:
            pass
        # Étape 3 : appliquer la solution normalement
        _old_apply_solution(self, sol)

    MoneyRoot.apply_solution = apply_solution_close_popups

    # Même chose pour apply_solution_from_list (si appelée directement)
    if hasattr(MoneyRoot, "apply_solution_from_list"):
        _old_afl = MoneyRoot.apply_solution_from_list
        ...
        def apply_solution_from_list_close(self, sol, sol_list, pop=None):
            ...
            try:
                ...
                pass
            _old_afl(self, sol, sol_list, pop)

        MoneyRoot.apply_solution_from_list = apply_solution_from_list_close
        MoneyRoot.apply_solution_from_list_v105 = apply_solution_from_list_close
        MoneyRoot.apply_solution_from_list_v1093 = apply_solution_from_list_close

_geostar_patch_apply_solution_close()
